...
void main() {
  ...
  for (final dark in [false, true]) {
    ...
    testWidgets('customer quotes and payments $mode', (tester) async {
      await _phone(tester, dark: dark);
      ...
      await _wait(tester);
      await _shot(tester, 'customer-$mode-10a-quotes');
      await tester.tap(find.text('Kolkata – Birgunj ICD'));
      await _wait(tester);
      await _shot(tester, 'customer-$mode-10b-quote');
      await tester.tap(find.byType(SheetCloseButton).last);
      await _wait(tester);
      await tester.tap(find.byType(SheetCloseButton).last);
      await _wait(tester);
      await tester.scrollUntilVisible(find.text('Bikram Sambat (BS)'), 300, scrollable: find.byType(Scrollable).hitTestable().first);
      await _shot(tester, 'customer-$mode-10c-settings');
      ...
      await _wait(tester);
      await _shot(tester, 'customer-$mode-10d-invoice-bs');
      await tester.tap(find.text('Pay online'));
      await _wait(tester);
      await _shot(tester, 'customer-$mode-10e-pay');
      await tester.tap(find.text('Khalti'));
      await _wait(tester);
      await _shot(tester, 'customer-$mode-10f-pay-waiting');
      await tester.tap(find.byType(SheetCloseButton).last);
      await _wait(tester);
      await controller.setCalendar(DateCalendar.gregorian);
      await tester.pumpWidget(const SizedBox());
      await _wait(tester);
    }, variant: iPhone);

    testWidgets('ops delivery and actions $mode', (tester) async {
      await _phone(tester, dark: dark);
      ...
      await _wait(tester);
      await tester.scrollUntilVisible(find.text('Start delivery'), 300, scrollable: find.byType(Scrollable).last);
      await _shot(tester, 'ops-$mode-6a-job-delivery');
      await tester.tap(find.text('Start delivery'));
      await _wait(tester);
      await _shot(tester, 'ops-$mode-6b-start-delivery');
      await tester.tap(find.text('Start delivery').last);
      await _wait(tester, 20);
      await tester.enterText(find.byType(TextField).first, 'Ram Bahadur Thapa');
      await tester.tap(find.text('Consignee'));
      await _wait(tester, 3);
      await _shot(tester, 'ops-$mode-6c-record-delivery');
      await tester.tap(find.text('Not delivered'));
      await _wait(tester);
      await _shot(tester, 'ops-$mode-6d-not-delivered');
      await tester.tap(find.byType(SheetCloseButton).last);
      await _wait(tester);
      await tester.scrollUntilVisible(find.text('New task'), -300, scrollable: find.byType(Scrollable).last);
      ...
      await _wait(tester, 3);
      await _shot(tester, 'ops-$mode-6e-new-task');
      await tester.tap(find.text('Nobody yet'));
      await _wait(tester);
      await _shot(tester, 'ops-$mode-6f-staff');
      await tester.tap(find.byType(SheetCloseButton).last);
      await _wait(tester);
      await tester.tap(find.byType(SheetCloseButton).last);
      await _wait(tester);
      await tester.scrollUntilVisible(find.text('Close job…'), 600, scrollable: find.byType(Scrollable).last);
      await tester.tap(find.text('Close job…'));
      await _wait(tester);
      await _shot(tester, 'ops-$mode-6g-close');
      await tester.pumpWidget(const SizedBox());
      await _wait(tester);
    }, variant: iPhone);
