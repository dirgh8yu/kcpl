...
test("final admin design system is loaded last and uses the Figma identity tokens", async () => {
  ...
  const overviewImport = layout.indexOf('import "./admin/operations-overview-responsive.css";');
  const commercialImport = layout.indexOf('import "./admin/commercial-detail-refinement.css";');
  assert.ok(systemImport > overviewImport, "admin design system must load after Overview refinements");
