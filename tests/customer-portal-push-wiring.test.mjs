...
test("push reuses the notification key, so it cannot send what email would not", async () => {
  ...
  // Each of the five facts (milestones, document releases, document requests,
  // free time, invoices) computes one key and hands it to both transports.
  assert.equal((sweep.match(/await pushOnce\(\{/g) ?? []).length, 5);
  assert.equal((sweep.match(/const key = portalNotificationKey\(\{/g) ?? []).length, 5);
