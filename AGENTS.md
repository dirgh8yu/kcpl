...
- KCPL is a whole product experience, not a collection of isolated admin pages. The staff system must feel like one premium cargo operations product: calm and confident, fast to scan, precise in action hierarchy, and operationally clear from first glance.
...
- **Approved Figma Make UI is the visual design source of truth for the whole KCPL staff product.** When implementing or repairing a screen, read the relevant Figma Make design first and reproduce its layout, hierarchy, spacing, surface treatment, density, typography, icon language and interaction model in the real application. Do not reinterpret an approved Figma screen into an older repository layout merely because legacy components already exist. The backend and server policies remain the functional source of truth.
- The target interaction quality is informed by Linear-level scan speed, Stripe-level clarity, Flexport/project44 shipment visibility and Ramp-level approval/action UX, but the final result must remain unmistakably KCPL: crimson/black/neutral palette, Inter interface text, restrained surfaces, and operational hierarchy over decorative dashboard styling.
...
- Keep shared UI in the canonical stylesheet. Legacy compatibility styles may be modified or removed as part of an intentional route migration. Preserve behaviour, migrate consumers to the canonical system, validate the affected workflows, and do not create new compatibility layers. Do not create another hotfix/refinement stylesheet, use `!important`, target Tailwind class strings in CSS, or add arbitrary literal colours/sizes/shadows to components.
