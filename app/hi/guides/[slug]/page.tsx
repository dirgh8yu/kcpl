...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const guide = guides.hi[slug as GuideSlug];
  return createPageMetadata({ title: guide.title, description: guide.description, path: `/hi/guides/${slug}` });
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  if (!guideSlugs.includes(slug as GuideSlug)) notFound();
  return <FreightGuidePage locale="hi" slug={slug as GuideSlug}/>;
}
