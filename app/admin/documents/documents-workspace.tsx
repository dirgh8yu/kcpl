...
export function DocumentsWorkspace({ dashboard, role, currentUserEmail }: { dashboard: DocumentVaultDashboard; role: KcplStaffRole; currentUserEmail: string }) {
  ...
  return <OpsPage><main className="min-h-[calc(100vh-64px)] bg-[var(--admin-canvas)] text-[var(--admin-ink)]"><div className="mx-auto w-full max-w-[1320px] px-4 pb-16 pt-8 sm:px-6 lg:px-8">
    <header className="grid gap-6 border-b border-[#101010] pb-7 md:grid-cols-[minmax(0,1fr)_auto] md:items-end"><div><p className="text-[length:var(--app-label-size)] uppercase tracking-[0.11em] text-[var(--admin-crimson)]">Operations · Evidence control</p><h1 className="mt-3 text-[clamp(36px,4vw,52px)] font-normal leading-[1.04] tracking-[-0.04em]">Document Vault</h1><p className="mt-3 max-w-2xl text-[14px] leading-6 text-[var(--admin-muted)]">Control shipment evidence, review state, expiry and version history. Only verified, current files satisfy readiness.</p><p className="mt-3 text-[length:var(--app-label-size)] uppercase tracking-[0.05em] text-[var(--admin-muted)]">Generated {dateTime(dashboard.generated_at)}</p></div><div className="flex flex-wrap gap-2"><Link href="/admin/freight-documents" className="inline-flex h-10 items-center border border-[var(--admin-line-strong)] px-3 text-[12px] hover:border-[#101010] hover:bg-[var(--admin-surface-muted)]">Freight Documents</Link><Link href="/admin/customs" className="inline-flex h-10 items-center border border-[var(--admin-line-strong)] px-3 text-[12px] hover:border-[#101010] hover:bg-[var(--admin-surface-muted)]">Customs</Link><button type="button" onClick={() => router.refresh()} className="inline-flex h-10 items-center gap-2 border border-[var(--admin-crimson)] bg-[var(--admin-crimson)] px-4 text-[12px] font-medium text-white hover:border-[var(--admin-crimson-dark)] hover:bg-[var(--admin-crimson-dark)]"><RefreshCw size={13}/>Refresh vault</button></div></header>

    <section className="grid border-b border-[var(--admin-line)] sm:grid-cols-3 lg:grid-cols-6"><Metric label="Active" value={dashboard.active_count} active={status === "active"} onClick={() => setStatus("active")}/><Metric label="Verified" value={dashboard.verified_count} active={status === "verified"} onClick={() => setStatus(status === "verified" ? "active" : "verified")}/><Metric label="Review queue" value={dashboard.review_count} alert active={status === "received" || status === "under_review"} onClick={() => setStatus(status === "received" ? "active" : "received")}/><Metric label="Rejected" value={dashboard.rejected_count} alert active={status === "rejected"} onClick={() => setStatus(status === "rejected" ? "active" : "rejected")}/><Metric label="Expired" value={dashboard.expired_count} alert active={status === "expired"} onClick={() => setStatus(status === "expired" ? "active" : "expired")}/><Metric label="History" value={dashboard.deleted_count} active={status === "deleted"} onClick={() => setStatus(status === "deleted" ? "active" : "deleted")}/></section>
    ...
    <section className="mt-6 border-y border-[var(--admin-line)]">
      <div className="grid gap-3 border-b border-[#101010] py-4 md:grid-cols-[minmax(260px,1fr)_190px_220px_170px_auto]">
        <label className="flex h-10 items-center border border-[var(--admin-line-strong)] bg-white px-3"><Search size={13} className="mr-2 text-[var(--admin-muted)]"/><input className="min-w-0 flex-1 bg-transparent text-[12px] outline-none placeholder:text-[var(--admin-muted)]" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search shipment, customer, filename, reviewer or hash"/></label>
      ...
      <div className="hidden min-h-11 grid-cols-[minmax(240px,1.4fr)_190px_210px_190px_auto] items-center gap-4 border-b border-[#101010] px-3 text-[length:var(--app-label-size)] uppercase tracking-[0.06em] text-[var(--admin-muted)] lg:grid"><span>Document / shipment</span><span>Status</span><span>Received</span><span>Control</span><span className="text-right">Actions</span></div>
...
function DocumentRow({ row, role, currentUserEmail, busy, onDelete, onNotice }: { row: DocumentVaultRow; role: KcplStaffRole; currentUserEmail: string; busy: boolean; onDelete: () => void; onNotice: (notice: Notice) => void }) {
  ...
  return <article className={`border-b border-[var(--admin-line)] last:border-b-0 ${inactive ? "opacity-65" : ""}`}>
    <div className="grid gap-4 px-3 py-4 transition-colors hover:bg-[var(--admin-surface-muted)] lg:grid-cols-[minmax(240px,1.4fr)_190px_210px_190px_auto] lg:items-start">
      <div className="min-w-0"><p className="truncate text-[13px] font-medium">{row.filename}</p><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]"><Link href={`/admin/jobs/${encodeURIComponent(row.shipment_reference)}`} className="font-medium hover:text-[var(--admin-crimson)]"><OpsMono>{row.shipment_reference}</OpsMono></Link> · {row.customer_id ? <Link href={`/admin/crm/${encodeURIComponent(row.customer_id)}`} className="hover:text-[var(--admin-crimson)]">{row.customer_name}</Link> : row.customer_name}</p><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{row.origin} → {row.destination} · {row.mode} · {bytes(row.size_bytes)}</p><div className="mt-2 flex flex-wrap gap-1.5"><OpsBadge>{shipmentDocumentTypeLabels[row.document_type]}</OpsBadge>{row.customer_safe ? <OpsBadge tone="info">Customer-safe</OpsBadge> : <OpsBadge>Internal</OpsBadge>}{row.branch ? <OpsBadge>{row.branch}</OpsBadge> : <OpsBadge tone="warning">Branch repair needed</OpsBadge>}</div></div>
      <div><OpsBadge tone={statusTone(row.effective_status)} dot>{statusLabel(row.effective_status)}</OpsBadge><p className="mt-2 text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]">{row.sha256 ? `SHA ${row.sha256.slice(0, 12)}…` : "Hash unavailable"}</p></div>
      <div><p className="text-[11px] font-medium">{dateTime(row.uploaded_at)}</p><p className="mt-1 text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]">{row.uploaded_by}{row.uploaded_by_email ? ` · ${row.uploaded_by_email}` : ""}</p></div>
      <div><p className="text-[11px] font-medium">{row.expires_on ? `Expires ${dateOnly(row.expires_on)}` : "No expiry"}</p><p className="mt-1 text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]">{row.reviewed_by ? `Reviewed by ${row.reviewed_by}` : "Awaiting first review"}</p>{row.review_note ? <p className="mt-1 text-[length:var(--app-label-size)] leading-4 text-[#666660]">{row.review_note}</p> : null}{row.storage_delete_pending ? <p className="mt-1 text-[length:var(--app-label-size)] font-medium text-[var(--admin-crimson)]">Storage cleanup pending</p> : null}</div>
    ...
    {reviewOpen ? <form onSubmit={saveReview} className="grid gap-3 border-t border-[var(--admin-line-strong)] bg-[var(--admin-surface-muted)] px-4 py-5 md:grid-cols-2 xl:grid-cols-[180px_180px_minmax(280px,1fr)_220px_auto] xl:items-end"><label className="grid gap-1.5 text-[length:var(--app-label-size)] font-medium text-[var(--admin-muted)]">Review status<select name="status" defaultValue={row.review_status} className="ops-select"><option value="received">Received</option><option value="under_review">Under review</option>{canSelfVerify ? <option value="verified">Verified</option> : null}<option value="rejected">Rejected</option></select></label><label className="grid gap-1.5 text-[length:var(--app-label-size)] font-medium text-[var(--admin-muted)]">Expiry date<input name="expiresOn" type="date" defaultValue={row.expires_on ?? ""} className="ops-input"/></label><label className="grid gap-1.5 text-[length:var(--app-label-size)] font-medium text-[var(--admin-muted)]">Review note<input name="reviewNote" defaultValue={row.review_note ?? ""} className="ops-input" placeholder="Required when rejecting; useful for verification notes"/></label><label className="flex min-h-10 items-center gap-2 text-[length:var(--app-label-size)] font-medium text-[var(--admin-muted)]"><input name="customerSafe" type="checkbox" defaultChecked={row.customer_safe}/>Safe for customer-facing use</label><OpsButton type="submit" variant="primary" size="sm"><CheckCircle2 size={11}/>Save review</OpsButton></form> : null}
  </article>;
}
