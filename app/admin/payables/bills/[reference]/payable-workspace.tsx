...
import { Banknote, BriefcaseBusiness, CheckCircle2, Landmark, ReceiptText, Trash2 } from "lucide-react";
      ...
      <div className="ops-grid-main">
        <div className="ops-stack">
          <OpsSurface eyebrow={openingBalance ? "Supplier opening balance" : "Supplier bill"} title={bill.supplier_name} description={openingBalance ? `Ledger opening as at ${dateLabel(asOfDate)}.` : bill.supplier_bill_reference ? `Vendor reference ${bill.supplier_bill_reference}` : "No vendor bill reference supplied."}>
            <div className="grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3"><Fact label="Status" value={payableStatusLabels[bill.status]}/><Fact label={openingBalance ? "Opening date" : "Bill date"} value={dateLabel(openingBalance ? asOfDate : bill.bill_date)}/><Fact label="Due date" value={dateLabel(bill.due_date)}/><Fact label={openingBalance ? "Accounting treatment" : "Category"} value={openingBalance ? "Ledger opening only" : jobCostCategoryLabels[bill.category]}/><Fact label="Branch" value={bill.branch}/><Fact label="Currency" value={bill.currency}/></div>
...
function Metric({label,value,tone="neutral"}:{label:string;value:string;tone?:"neutral"|"success"|"warning"}) { return <div className="rounded-[var(--app-radius)] border border-[#eae2dc] bg-[var(--admin-surface-muted)] p-3"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.07em] text-[var(--admin-muted)]">{label}</p><strong className={`mt-1.5 block text-[11px] ${tone === "success" ? "text-[#66806b]" : tone === "warning" ? "text-[#9a682f]" : "text-[var(--admin-ink)]"}`}>{value}</strong></div>; }
