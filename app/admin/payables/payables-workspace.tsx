  ...
  return <OpsPage>
    ...
    <div className="ops-content-wide ops-stack">
      ...
      <OpsSurface eyebrow="Payables register" title="Supplier bill ledger" description={`${filtered.length} of ${dashboard.bills.length} bills shown.`} flush>
        <div className="ops-toolbar"><OpsSearch value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search partner, bill, shipment, branch or reference"/><select className="ops-select" value={status} onChange={(event) => setStatus(event.target.value as "all" | PayableStatus)}><option value="all">All statuses</option>{payableStatuses.map((item) => <option key={item} value={item}>{payableStatusLabels[item]}</option>)}</select><OpsButton variant="ghost" size="sm" onClick={() => { setQuery(""); setStatus("all"); }}>Reset</OpsButton></div>
        <div className="ops-table-wrap"><table className="ops-table min-w-[1100px]"><thead><tr><th>Bill</th><th>Supplier</th><th>Job / branch</th><th>Bill / due</th><th>Total</th><th>Balance</th><th>Status</th></tr></thead><tbody>{filtered.length ? filtered.map((bill) => <tr key={bill.reference}><td><Link href={`/admin/payables/bills/${encodeURIComponent(bill.reference)}`}><OpsMono>{bill.reference}</OpsMono></Link><p className="mt-1.5 text-[10px] text-[#8d837b]">{bill.supplier_bill_reference || "No vendor reference"}</p></td><td><strong>{bill.supplier_name}</strong><p className="mt-1.5 text-[10px] text-[#8d837b]">{bill.supplier_id || "Unregistered supplier"} · {jobCostCategoryLabels[bill.category]}</p></td><td><span>{bill.shipment_reference ? <OpsMono>{bill.shipment_reference}</OpsMono> : "General payable"}</span><p className="mt-1.5 flex items-center gap-1 text-[10px] text-[#8d837b]"><BriefcaseBusiness size={10}/>{bill.branch}</p></td><td><span>{dateLabel(bill.bill_date)}</span><p className={`mt-1.5 text-[10px] ${bill.status === "overdue" ? "font-bold text-[#b65355]" : "text-[#8d837b]"}`}>Due {dateLabel(bill.due_date)}</p></td><td className="font-bold text-[#514840]">{money(bill.total, bill.currency)}</td><td className="font-bold text-[#514840]">{money(bill.balance_due, bill.currency)}</td><td><OpsBadge tone={statusTone(bill.status)} dot>{payableStatusLabels[bill.status]}</OpsBadge></td></tr>) : <tr><td colSpan={7}><OpsEmptyState title="No supplier bills match" description="Change the filters or create a new payable."/></td></tr>}</tbody></table></div>
      </OpsSurface>
    </div>
  </OpsPage>;
}
