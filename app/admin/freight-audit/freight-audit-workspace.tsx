  ...
  return <div className="ops-content ops-stack">
    ...
    <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
      <OpsSurface eyebrow="Audit queue" title="Supplier invoices">
        {!rows.length ? <OpsEmptyState icon={<BadgeCheck size={18}/>} title="No supplier bills to audit" description="New supplier bills linked to TMS shipments will appear here automatically."/> : <div className="space-y-2">{rows.map((row) => <button type="button" key={row.payable_reference} onClick={() => setSelectedReference(row.payable_reference)} className={`w-full rounded-[var(--app-radius)] border p-3 text-left transition ${selectedReference === row.payable_reference ? "border-[#d5ae8f] bg-[#fff9f4]" : "border-[var(--admin-line)] bg-white hover:bg-[var(--admin-surface)]"}`}>
          <div className="flex items-start justify-between gap-3"><div><div className="text-[12px] font-bold text-[#443d38]">{row.supplier_name}</div><div className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{row.payable_reference}{row.supplier_bill_reference ? ` · Invoice ${row.supplier_bill_reference}` : ""}</div></div><OpsBadge tone={tone(row.status)}>{freightAuditStatusLabels[row.status]}</OpsBadge></div>
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[length:var(--app-label-size)] text-[#756c65]"><span>Booked {money(row.booked_currency, row.booked_cost)}</span><span>Invoice {money(row.invoice_currency, row.invoice_subtotal)}</span>{row.variance_amount !== null ? <span className={row.variance_amount > 0 ? "text-[#a35b4c]" : "text-[#58725f]"}>Variance {row.variance_amount >= 0 ? "+" : ""}{row.variance_amount.toFixed(2)}</span> : null}</div>
        </button>)}</div>}
      </OpsSurface>
      ...
      <OpsSurface eyebrow="Three-way match" title={selected ? selected.payable_reference : "Select a supplier bill"} description={selected ? `${selected.supplier_name}${selected.shipment_reference ? ` · ${selected.shipment_reference}` : ""}` : "Choose an invoice from the audit queue."}>
        {!selected ? <OpsEmptyState icon={<CircleDollarSign size={18}/>} title="No audit selected" description="Select a supplier invoice to inspect its booking match."/> : <div className="space-y-4">
          ...
          {selected.expected_linehaul !== null ? <div className="rounded-[var(--app-radius)] border border-[#e4ddd6] bg-[var(--admin-surface)] p-4"><div className="mb-3"><div className="text-[11px] font-bold text-[var(--admin-ink)]">Booked rate-card baseline</div><div className="mt-1 text-[length:var(--app-label-size)] leading-5 text-[#857b73]">Reconstructed from the selected Partner rate card and the booked order quantity. This breakdown is hidden when a negotiated counter-offer replaced the original rate-card economics.</div></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><Metric label="Linehaul" value={money(selected.booked_currency, selected.expected_linehaul)}/><Metric label="Fuel surcharge" value={money(selected.booked_currency, selected.expected_fuel_surcharge)}/><Metric label="Accessorials" value={money(selected.booked_currency, selected.expected_accessorials)}/><Metric label="Rating unit" value={selected.expected_rate_unit?.replaceAll("_", " ") ?? "Not available"}/><Metric label="Booked quantity" value={selected.expected_quantity === null ? "Not available" : selected.expected_quantity.toLocaleString("en-AU")}/><Metric label="Minimum charge" value={selected.minimum_applied === null ? "Not available" : selected.minimum_applied ? "Applied" : "Not applied"}/></div></div> : null}

          <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-4"><div className="flex items-center justify-between gap-3"><div><div className="text-[11px] font-bold text-[var(--admin-ink)]">Match-Pay status</div><div className="mt-1 text-[length:var(--app-label-size)] text-[#857b73]">{selected.booked_partner_name ?? "No TMS carrier snapshot"}{selected.carrier_reference ? ` · Booking ${selected.carrier_reference}` : ""}</div></div><OpsBadge tone={tone(selected.status)}>{freightAuditStatusLabels[selected.status]}</OpsBadge></div></div>

          <div><div className="mb-2 text-[11px] font-bold text-[var(--admin-ink)]">Audit findings</div>{selected.issues.length ? <div className="space-y-2">{selected.issues.map((item) => <div key={item.code} className={`rounded-[var(--app-radius)] border p-3 ${item.severity === "blocking" ? "border-[#e8c8bf] bg-[#fff7f4]" : "border-[#eadfca] bg-[#fffaf0]"}`}><div className="flex gap-2"><AlertTriangle size={13} className="mt-0.5 shrink-0"/><div><div className="text-[11px] font-bold text-[#4e4640]">{item.title}</div><div className="mt-1 text-[length:var(--app-label-size)] leading-5 text-[#796f68]">{item.detail}</div></div></div></div>)}</div> : <OpsNotice tone="success">Booked provider, currency and freight subtotal are within the configured match tolerance.</OpsNotice>}</div>
          ...
          <label className="block"><span className="mb-1 block text-[length:var(--app-label-size)] font-semibold text-[#746b64]">Decision / dispute note</span><textarea className="ops-textarea min-h-24" value={note} onChange={(event) => setNote(event.target.value)} placeholder="Required for dispute, rejection or approved variance..."/></label>
...
function Metric({ label, value }: { label: string; value: string }) { return <div className="rounded-[var(--app-radius)] border border-[#ebe4de] bg-white p-3"><div className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.12em] text-[#968b82]">{label}</div><div className="mt-1 text-[12px] font-bold capitalize text-[#4a423c]">{value}</div></div>; }
