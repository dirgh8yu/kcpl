...
export const freightAuditStatuses = [
  "pending",
  "matched",
  "review_required",
  "disputed",
...
export const freightAuditStatusLabels: Record<FreightAuditStatus, string> = {
  ...
  disputed: "Disputed",
  approved_variance: "Variance approved",
  rejected: "Rejected",
...
export type FreightAuditIssue = {
  code: FreightAuditIssueCode;
...
export type FreightAuditQueueRow = FreightAuditRecord & {
  payable_status: string;
  customer_name: string | null;
  carrier_reference: string | null;
};

export type FreightAuditSummary = {
  total: number;
  matched: number;
  review_required: number;
  disputed: number;
  approved_variance: number;
  blocked_from_payment: number;
};

export const DEFAULT_FREIGHT_AUDIT_TOLERANCE_PERCENT = 1;
export const DEFAULT_FREIGHT_AUDIT_TOLERANCE_AMOUNT = 1;

export function normalizeAuditReference(value: string | null | undefined) {
...
export function freightVarianceWithinTolerance(input: {
...
export function freightAuditPaymentAllowed(status: FreightAuditStatus) {
  return status === "matched" || status === "approved_variance" || status === "not_applicable";
}

export function freightAuditRequiresManagement(status: FreightAuditStatus) {
  return status === "review_required" || status === "disputed";
}
