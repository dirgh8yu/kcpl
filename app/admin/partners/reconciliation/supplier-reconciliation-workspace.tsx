        ...
        <div className="ops-table-wrap"><table className="ops-table min-w-[1320px]"><thead><tr><th>Supplier bill</th><th>Current supplier identity</th><th>Job / branch</th><th>Amount</th><th>Due / status</th><th>Partner match</th><th></th></tr></thead><tbody>
          {filtered.length ? filtered.map((bill) => {
            ...
            return <Fragment key={bill.reference}>
              <tr>
                <td><Link href={`/admin/payables/bills/${encodeURIComponent(bill.reference)}`} className="font-bold text-[var(--admin-ink)] hover:underline"><OpsMono>{bill.reference}</OpsMono></Link><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Vendor ref: {bill.supplier_bill_reference || "Not recorded"}</p></td>
                ...
                <td>{bill.shipment_reference ? <Link href={`/admin/shipments/${encodeURIComponent(bill.shipment_reference)}`} className="font-semibold hover:underline"><OpsMono>{bill.shipment_reference}</OpsMono></Link> : <span>General payable</span>}<p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{bill.branch}</p></td>
                <td><strong>{money(bill.total, bill.currency)}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{money(bill.balance_due, bill.currency)} outstanding</p></td>
