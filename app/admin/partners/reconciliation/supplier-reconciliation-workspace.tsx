        ...
        <div className="ops-table-wrap"><table className="ops-table min-w-[1320px]"><thead><tr><th>Supplier bill</th><th>Current supplier identity</th><th>Job / branch</th><th>Amount</th><th>Due / status</th><th>Partner match</th><th></th></tr></thead><tbody>
          {filtered.length ? filtered.map((bill) => {
            ...
            return <Fragment key={bill.reference}>
              <tr>
                <td><Link href={`/admin/payables/bills/${encodeURIComponent(bill.reference)}`} className="font-bold text-[var(--admin-ink)] hover:underline"><OpsMono>{bill.reference}</OpsMono></Link><p className="mt-1 text-[length:var(--app-label-size)] text-[#8f857d]">Vendor ref: {bill.supplier_bill_reference || "Not recorded"}</p></td>
                ...
                <td>{bill.shipment_reference ? <Link href={`/admin/shipments/${encodeURIComponent(bill.shipment_reference)}`} className="font-semibold hover:underline"><OpsMono>{bill.shipment_reference}</OpsMono></Link> : <span>General payable</span>}<p className="mt-1 text-[length:var(--app-label-size)] text-[#8f857d]">{bill.branch}</p></td>
                <td><strong>{money(bill.total, bill.currency)}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[#8f857d]">{money(bill.balance_due, bill.currency)} outstanding</p></td>
              ...
              {confirming === bill.reference && selectedPartner ? <tr><td colSpan={7} className="bg-[#fbf7f3] p-0"><div className="m-3 rounded-[var(--app-radius)] border border-[#e6d8cf] bg-white p-4"><p className="text-[11px] font-bold text-[var(--admin-ink)]">Confirm accounting identity change</p><div className="mt-3 grid gap-3 md:grid-cols-3"><Review label="Before" value={`${bill.supplier_name}${bill.supplier_id ? ` · ${bill.supplier_id}` : " · no supplier ID"}`}/><Review label="After" value={`${selectedPartner.name} · ${selectedPartner.id}`}/><Review label="Bill values" value={`${money(bill.total, bill.currency)} · ${payableStatusLabels[bill.status]} · amounts unchanged`}/></div><p className="mt-3 text-[length:var(--app-label-size)] leading-5 text-[#7e746d]">This writes the Partner ID to the supplier bill and updates the linked Job File cost identity if one exists. It does not change the bill amount, payments, status, currency or due date.</p><div className="mt-4 flex gap-2"><OpsButton variant="primary" size="sm" disabled={busy} onClick={() => void confirmLink(bill)}>{busy ? "Linking…" : "Confirm Partner link"}</OpsButton><OpsButton variant="ghost" size="sm" disabled={busy} onClick={() => setConfirming(null)}>Cancel</OpsButton></div></div></td></tr> : null}
            </Fragment>;
...
function Review({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[var(--app-radius)] border border-[#eee5df] bg-[var(--admin-surface)] p-3"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.07em] text-[var(--admin-muted)]">{label}</p><p className="mt-1.5 text-[11px] font-semibold leading-5 text-[var(--admin-ink)]">{value}</p></div>;
}
