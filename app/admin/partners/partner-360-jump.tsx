...
export function Partner360Jump({ partners, canReconcile = false }: { partners: Array<{ id: string; display_name: string }>; canReconcile?: boolean }) {
  ...
  return <div className="border-b border-[var(--admin-line)] bg-[var(--admin-surface-muted)]">
    <div className="ops-content-wide flex min-h-[54px] flex-wrap items-center gap-2 py-2">
      {partners.length ? <><span className="flex items-center gap-2 text-[11px] font-medium text-[var(--admin-muted)]"><Search size={12} className="text-[var(--admin-crimson)]"/>Open Partner 360</span>
      ...
      <OpsButton variant="secondary" size="sm" disabled={!partnerId} onClick={() => partnerId && router.push(`/admin/partners/${encodeURIComponent(partnerId)}`)}>Open <ArrowRight size={11}/></OpsButton></> : <span className="flex-1 text-[11px] text-[var(--admin-muted)]">No Partner records are available in your current scope.</span>}
