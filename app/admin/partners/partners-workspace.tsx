...
export function PartnersWorkspace({ dashboard, canEdit }: { dashboard: PartnerDashboard; canEdit: boolean }) {
  ...
  return <OpsPage>
    ...
    {dashboard.partners.length ? <OpsStatStrip><OpsStat label="Active network" value={dashboard.active_count} icon={<Handshake size={13}/>} tone="success"/><OpsStat label="Preferred" value={dashboard.preferred_count} icon={<BadgeCheck size={13}/>} tone="accent"/><OpsStat label="Countries" value={dashboard.country_count} icon={<Globe2 size={13}/>} /><OpsStat label="Unlinked supplier bills" value={dashboard.unlinked_supplier_bills} icon={<TriangleAlert size={13}/>} tone={dashboard.unlinked_supplier_bills ? "warning" : "neutral"}/><OpsStat label="Partner records" value={dashboard.partners.length} icon={<Building2 size={13}/>} /></OpsStatStrip> : null}
