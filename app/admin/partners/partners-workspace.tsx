    ...
    <div className="ops-content-wide ops-stack">
      ...
      {formOpen && canEdit ? <OpsSurface eyebrow="Network record" title={form.id ? `Edit ${form.displayName}` : "Add partner or vendor"} description="Capture the service footprint and working contact first. Legal, contract and commercial detail can stay tucked away until needed." action={<button type="button" onClick={() => setFormOpen(false)} className="grid h-8 w-8 place-items-center rounded-[var(--app-radius)] text-[var(--admin-muted)] hover:bg-[var(--admin-surface-soft)]" aria-label="Close partner form"><X size={13}/></button>}>
        <form onSubmit={savePartner} className="ops-stack">
          ...
          <button type="button" onClick={() => setAdvancedOpen(!advancedOpen)} className="flex items-center justify-between rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] px-4 py-3 text-left"><span><strong className="block text-[11px] text-[var(--admin-ink)]">Commercial, compliance & contract details</strong><small className="mt-1 block text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Optional fields that do not need to crowd everyday network work</small></span><span className="text-[length:var(--app-label-size)] font-bold text-[var(--admin-crimson)]">{advancedOpen ? "Hide" : "Add details"}</span></button>
          ...
          <div className="flex gap-2"><OpsButton type="submit" variant="primary" disabled={busy}>{busy ? "Saving…" : form.id ? "Save partner" : "Create partner"}</OpsButton><OpsButton type="button" variant="ghost" onClick={() => setFormOpen(false)}>Cancel</OpsButton></div>
        </form>
      </OpsSurface> : null}
      ...
      {dashboard.partners.length ? <OpsSurface eyebrow="Network register" title="Partners & vendors" description={`${filtered.length} of ${dashboard.partners.length} records shown. Search covers saved names, identifiers, countries, cities, ports, services, contacts and relationship types.`} flush>
        ...
        <div className="ops-table-wrap"><table className={`ops-table ${financialVisible ? "min-w-[1280px]" : "min-w-[1080px]"}`}><thead><tr><th>Partner</th><th>Operating footprint</th><th>Services</th><th>Contact</th><th>KCPL owner</th>{financialVisible ? <th>Supplier exposure</th> : null}<th>Activity</th><th></th></tr></thead><tbody>{filtered.length ? filtered.map((p) => <tr key={p.id}><td><div className="flex items-center gap-2"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-[var(--app-radius)] bg-[var(--admin-accent-bg)] text-[var(--admin-crimson)]"><Building2 size={14}/></span><div><div className="flex items-center gap-1.5"><strong>{p.display_name}</strong>{p.preferred ? <Star size={11} fill="currentColor" className="text-[var(--admin-crimson)]"/> : null}</div><div className="mt-1.5 flex max-w-[240px] flex-wrap gap-1"><OpsBadge tone={statusTone(p.status)}>{partnerStatusLabels[p.status]}</OpsBadge>{p.types.slice(0,3).map((type) => <OpsBadge key={type}>{partnerTypeLabels[type]}</OpsBadge>)}{p.types.length > 3 ? <OpsBadge>+{p.types.length - 3}</OpsBadge> : null}</div><p className="mt-1.5 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{p.id}</p></div></div></td><td><div className="min-w-[220px]"><span className="flex items-center gap-1.5 text-[11px] font-semibold text-[var(--admin-ink)]"><Globe2 size={11} className="text-[var(--admin-muted)]"/>{p.country}</span><FootprintLine label="Serves" values={p.countries_served}/><FootprintLine label="Cities" values={p.cities_served}/><FootprintLine label="Ports" values={p.ports_served}/></div></td><td><div className="flex max-w-[210px] flex-wrap gap-1">{p.modes.length ? p.modes.slice(0,5).map((mode) => <OpsBadge key={mode} tone="info">{partnerModeLabels[mode]}</OpsBadge>) : <span className="text-[length:var(--app-label-size)] text-[var(--admin-muted)]">No services selected</span>}{p.modes.length > 5 ? <OpsBadge tone="info">+{p.modes.length - 5}</OpsBadge> : null}</div></td><td>{p.primary_contact_name ? <strong>{p.primary_contact_name}</strong> : <span className="text-[length:var(--app-label-size)] text-[var(--admin-muted)]">No primary contact</span>}{p.primary_email ? <p className="mt-1.5 flex items-center gap-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]"><Mail size={10}/>{p.primary_email}</p> : null}{p.primary_phone ? <p className="mt-1 flex items-center gap-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]"><Phone size={10}/>{p.primary_phone}</p> : null}{p.whatsapp ? <p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">WhatsApp · {p.whatsapp}</p> : null}</td><td>{p.owner_branch ? <strong>{p.owner_branch}</strong> : <OpsBadge tone="warning">Needs owner repair</OpsBadge>}{commercialVisible ? <p className="mt-1.5 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Terms {p.payment_terms_days}d · {p.preferred_currency}</p> : <p className="mt-1.5 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Commercial terms restricted</p>}</td>{financialVisible ? <td>{p.payable_open.length ? p.payable_open.map((amount) => <p key={amount.currency} className="text-[length:var(--app-label-size)] font-semibold text-[var(--admin-ink)]">{money(amount.amount, amount.currency)} open</p>) : <span className="text-[length:var(--app-label-size)] text-[var(--admin-success)]">No open payable</span>}<p className="mt-1.5 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{p.bill_count} bills · {p.shipment_count} jobs</p></td> : null}<td><span className="text-[length:var(--app-label-size)]">{dateLabel(p.last_activity_at)}</span>{financialVisible && p.overdue_bill_count ? <p className="mt-1.5 text-[length:var(--app-label-size)] font-bold text-[var(--admin-danger)]">{p.overdue_bill_count} overdue bill{p.overdue_bill_count === 1 ? "" : "s"}</p> : null}</td><td>{canEditRecord(p) ? <OpsButton variant="ghost" size="sm" onClick={() => startEdit(p)}><Pencil size={11}/>Edit</OpsButton> : p.owner_branch === null && canEdit ? <OpsBadge tone="warning">Management repair</OpsBadge> : null}</td></tr>) : <tr><td colSpan={financialVisible ? 8 : 7}><OpsEmptyState kind="search" icon={<Handshake size={18}/>} title="No partners match" description="Try a partner name, identifier, country, city, port, service, contact or relationship type, or change the filters."/></td></tr>}</tbody></table></div>
      </OpsSurface> : null}
    </div>
  </OpsPage>;
}
...
function FootprintLine({ label, values }: { label: string; values: string[] }) {
  ...
  return <p className="mt-1 max-w-[240px] text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]"><span className="font-bold text-[var(--admin-muted)]">{label}</span> · {values.join(", ")}</p>;
}

function PartnerKind({ title, detail }: { title: string; detail: string }) { return <div className="border-l-2 border-[var(--admin-line)] pl-3"><strong className="text-[12px] text-[var(--admin-ink)]">{title}</strong><p className="mt-1 text-[11px] leading-5 text-[var(--admin-muted)]">{detail}</p></div>; }
