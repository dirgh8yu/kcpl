...
function FootprintLine({ label, values }: { label: string; values: string[] }) {
  ...
  return <p className="mt-1 max-w-[240px] text-[length:var(--app-label-size)] leading-4 text-[#817970]"><span className="font-bold text-[#665f59]">{label}</span> · {values.join(", ")}</p>;
}

function PartnerKind({ title, detail }: { title: string; detail: string }) { return <div className="border-l-2 border-[#d8d2cc] pl-3"><strong className="text-[12px] text-[var(--admin-ink)]">{title}</strong><p className="mt-1 text-[11px] leading-5 text-[#756e67]">{detail}</p></div>; }
