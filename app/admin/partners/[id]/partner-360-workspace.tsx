...
export function Partner360Workspace({ snapshot, commercialVisible, financialVisible }: {
  ...
  return <OpsPage>
    ...
    <div className="ops-content-wide ops-stack">
      {financialVisible && snapshot.legacy_name_linked_bill_count > 0 ? <div className="rounded-[var(--app-radius)] border border-[#ead3b7] bg-[#fff9ef] px-4 py-3"><div className="flex items-start gap-3"><TriangleAlert size={15} className="mt-0.5 shrink-0 text-[#b77735]"/><div><strong className="text-[11px] text-[#6c4f2e]">Legacy supplier links need reconciliation</strong><p className="mt-1 text-[length:var(--app-label-size)] leading-5 text-[#806b54]">{snapshot.legacy_name_linked_bill_count} non-void bill{snapshot.legacy_name_linked_bill_count === 1 ? "" : "s"} match this partner by name but do not carry its Partner ID. They remain visible to Accounts, but they are not treated as confirmed operational job links.</p></div></div></div> : null}
      {financialVisible && snapshot.finance_integrity_warning_count > 0 ? <div className="rounded-[var(--app-radius)] border border-[#e7c9c5] bg-[#fff5f4] px-4 py-3"><div className="flex items-start gap-3"><TriangleAlert size={15} className="mt-0.5 shrink-0 text-[#b65355]"/><div><strong className="text-[11px] text-[#844a4b]">Finance data needs repair</strong><p className="mt-1 text-[length:var(--app-label-size)] leading-5 text-[#806363]">{snapshot.finance_integrity_warning_count} branch or currency integrity issue{snapshot.finance_integrity_warning_count === 1 ? "" : "s"} were found in linked payable records. Values with invalid currency are excluded from currency totals.</p></div></div></div> : null}
      ...
      <div className="ops-grid-main">
        <div className="ops-stack">
          ...
          <OpsSurface eyebrow="Relationship trail" title="Partner activity" description="Network changes and, for authorised Finance roles, supplier bill lifecycle events.">
            {snapshot.activity.length ? <div className="divide-y divide-[#eee7e1]">{snapshot.activity.slice(0, 30).map((item) => <article key={item.id} className="py-3.5"><div className="flex items-start gap-2.5"><span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#c6755d]"/><div className="min-w-0"><strong className="text-[11px] text-[var(--admin-ink)]">{item.title}</strong>{item.detail ? <p className="mt-1.5 text-[length:var(--app-label-size)] leading-5 text-[#756d66]">{item.detail}</p> : null}<p className="mt-1.5 text-[length:var(--app-label-size)] text-[#968c84]">{dateTime(item.created_at)} NPT{item.actor_name || item.actor_email ? ` · ${item.actor_name || item.actor_email}` : ""}</p></div></div></article>)}</div> : <OpsEmptyState title="No partner activity yet" description="Partner updates and linked financial lifecycle events will build this relationship trail over time."/>}
          </OpsSurface>
        </div>
        ...
        <aside className="ops-stack xl:sticky xl:top-[76px]">
          ...
          <OpsSurface eyebrow="Compliance" title="Registration & contract">
            ...
            {partner.document_url ? <a href={partner.document_url} target="_blank" rel="noreferrer" className="mt-4 flex items-center gap-2 border-t border-[var(--admin-line)] pt-4 text-[length:var(--app-label-size)] font-semibold text-[#a85f4a] hover:underline"><ExternalLink size={11}/>Open contract / document</a> : null}
          </OpsSurface>

          {partner.internal_notes ? <OpsSurface eyebrow="Internal" title="Partner notes"><p className="whitespace-pre-wrap text-[length:var(--app-label-size)] leading-5 text-[#756d66]">{partner.internal_notes}</p></OpsSurface> : null}
        </aside>
      </div>
...
function PartnerFinance({ summaries, bills }: { summaries: PartnerFinanceSummary[]; bills: Partner360Snapshot["bills"] }) {
  return <div className="ops-stack">
    <div><p className="ops-eyebrow">Accounts Payable</p><h2 className="mt-1 text-[18px] font-[720] tracking-[-.025em] text-[#443b35]">Supplier exposure</h2><p className="mt-1 max-w-3xl text-[11px] leading-5 text-[var(--admin-muted)]">Each currency stands alone. KCPL does not convert or combine supplier exposure using invented exchange rates.</p></div>
