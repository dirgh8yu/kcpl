  ...
  return (
    ...
    >
      <form onSubmit={calculate} className={`grid gap-3 p-3.5 ${compact ? "xl:grid-cols-[1fr_1fr_1fr_auto]" : "xl:grid-cols-[1fr_1fr_1fr_220px]"}`}>
        ...
        <label className="block"><span className="text-[length:var(--app-label-size)] font-medium text-[#69717a]">Via stops <span className="text-[#a0a6ac]">optional</span></span><textarea rows={compact ? 1 : 2} value={via} onChange={(event) => setVia(event.target.value)} className={`mt-1.5 w-full resize-none px-3 py-2 ${compact ? "h-[34px]" : "min-h-[64px]"}`} placeholder="Raxaul, India&#10;Birgunj, Nepal"/></label>
        <div className="flex items-end gap-2 xl:flex-col xl:items-stretch xl:justify-end"><label className="flex min-h-[34px] flex-1 cursor-pointer items-center gap-2 rounded-[var(--app-radius)] border border-[#dfe2e6] bg-[#fbfbfb] px-3 text-[length:var(--app-label-size)] font-medium text-[#616a73]"><input type="checkbox" checked={trafficAware} onChange={(event) => setTrafficAware(event.target.checked)} className="h-3.5 w-3.5 accent-[#5367d9]"/><span>Use live traffic <span className="text-[#7f88b3]">Pro</span></span></label><OpsButton tone="primary" type="submit" disabled={loading}>{loading ? "Calculating…" : "Calculate route"}</OpsButton></div>
      </form>
