...
export function GooglePlaceInput({
  ...
  return (
    <label className="relative block">
      <span className="mb-1.5 block text-[length:var(--app-label-size)] font-semibold text-[var(--ops-text-secondary)]">{label}</span>
      <div className="flex h-9 items-center gap-2 rounded-[var(--app-radius)] border border-[var(--ops-border)] bg-[var(--ops-surface-subtle)] px-2.5 focus-within:border-[#9aa6e5] focus-within:bg-white focus-within:shadow-[0_0_0_3px_rgba(83,103,217,.08)]">
      ...
      {showMenu ? (
        <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-50 overflow-hidden rounded-[var(--app-radius)] border border-[var(--ops-border)] bg-white shadow-[var(--ops-shadow-float)]">
          ...
          {!loading && !error ? suggestions.map((suggestion) => (
            ...
            >
              ...
              {suggestion.secondary_text ? <span className="mt-0.5 block truncate text-[length:var(--app-label-size)] text-[#858c94]">{suggestion.secondary_text}</span> : null}
            </button>
          )) : null}
          <div className="border-t border-[#eef0f2] bg-[#fafafa] px-3 py-1.5 text-right text-[length:var(--app-label-size)] text-[#91979e]">
