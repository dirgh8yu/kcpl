        ...
        <div id={listboxId} role="listbox" className="absolute inset-x-0 top-full z-40 mt-1 max-h-80 overflow-y-auto rounded-[var(--app-radius)] border border-[#d9dee3] bg-white p-1.5 shadow-[0_18px_45px_rgba(16,38,63,.18)]">
          ...
          {suggestions.map((suggestion) => (
            <button
              ...
              className="flex w-full items-start gap-3 rounded-[var(--app-radius)] px-3 py-2.5 text-left transition hover:bg-[#f3f5f6]"
            >
              <span className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-[var(--app-radius)] bg-[#eef1f3] text-[#87672f]"><MapPin size={13}/></span>
              <span className="min-w-0">
                <strong className="block text-xs text-[var(--admin-ink)]">{suggestion.label}</strong>
                <span className="mt-0.5 block text-[length:var(--app-label-size)] text-[#7e8992]">{suggestion.kind}{suggestion.detail ? ` · ${suggestion.detail}` : ""}</span>
              </span>
            </button>
          ))}
          <div className="border-t border-[#edf0f2] px-3 py-2 text-[length:var(--app-label-size)] leading-4 text-[#9aa2a9]">Locations supplied directly by the SeaRates Freight Index dictionary.</div>
        </div>
      ) : null}
    </div>
  );
    ...
    <div className="mx-auto grid max-w-[1500px] gap-5 p-4 sm:p-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:p-8">
      <section className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-5 shadow-sm sm:p-7">
        ...
        <form onSubmit={calculate} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Mode</span><select className={inputClass} value={form.mode} onChange={(event) => changeMode(event.target.value as EstimateMode)}>{modes.map((mode) => <option key={mode} value={mode}>{modeLabels[mode]}</option>)}</select></label>
          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Transport unit</span><select className={inputClass} value={form.loadType} onChange={(event) => setForm((current) => ({ ...current, loadType: event.target.value as LoadType }))}>{relevantLoadTypes.map((item) => <option key={item} value={item}>{loadTypeLabels[item]}</option>)}</select></label>
          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Quantity</span><input min="1" max="99" step="1" type="number" className={inputClass} value={form.quantity} onChange={(event) => setForm((current) => ({ ...current, quantity: event.target.value }))}/></label>
          <div><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Provider currency</span><div className={`${inputClass} flex items-center font-bold`}>USD</div></div>

          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Cargo weight <span className="font-normal text-[#9aa2a9]">(context)</span></span><div className="flex gap-2"><input min="0.01" step="0.01" type="number" className={inputClass} value={form.weight} onChange={(event) => setForm((current) => ({ ...current, weight: event.target.value }))} placeholder="Optional"/><select className={`${inputClass} max-w-24`} value={form.weightUnit} onChange={(event) => setForm((current) => ({ ...current, weightUnit: event.target.value }))}><option value="kg">kg</option><option value="lb">lb</option><option value="ton">ton</option></select></div></label>
          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Dimension unit <span className="font-normal text-[#9aa2a9]">(context)</span></span><select className={inputClass} value={form.dimensionUnit} onChange={(event) => setForm((current) => ({ ...current, dimensionUnit: event.target.value }))}><option value="cm">cm</option><option value="m">m</option><option value="inch">inch</option></select></label>
          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Length <span className="font-normal text-[#9aa2a9]">(optional)</span></span><input min="0.01" step="0.01" type="number" className={inputClass} value={form.length} onChange={(event) => setForm((current) => ({ ...current, length: event.target.value }))}/></label>
          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Width <span className="font-normal text-[#9aa2a9]">(optional)</span></span><input min="0.01" step="0.01" type="number" className={inputClass} value={form.width} onChange={(event) => setForm((current) => ({ ...current, width: event.target.value }))}/></label>
          <label><span className="text-[11px] font-semibold text-[var(--admin-muted)]">Height <span className="font-normal text-[#9aa2a9]">(optional)</span></span><input min="0.01" step="0.01" type="number" className={inputClass} value={form.height} onChange={(event) => setForm((current) => ({ ...current, height: event.target.value }))}/></label>
      ...
      <aside className="space-y-4">
        {configurationError ? <div className="rounded-[var(--app-radius)] border border-amber-200 bg-amber-50 p-5 text-amber-900"><div className="flex items-start gap-3"><KeyRound size={18} className="mt-0.5 shrink-0"/><div><p className="text-sm font-semibold">SeaRates API key required</p><p className="mt-1 text-xs leading-5">{configurationError}</p><p className="mt-3 text-[length:var(--app-label-size)] leading-5 text-amber-800">The credential stays server-side. Nothing is exposed to the browser.</p></div></div></div> : null}
