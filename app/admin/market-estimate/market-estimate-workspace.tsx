    ...
    <div className="relative md:col-span-1 xl:col-span-2">
      ...
      {open && value.trim().length >= 2 ? (
        <div id={listboxId} role="listbox" className="absolute inset-x-0 top-full z-40 mt-1 max-h-80 overflow-y-auto rounded-[var(--app-radius)] border border-[#d9dee3] bg-white p-1.5 shadow-[0_18px_45px_rgba(16,38,63,.18)]">
          ...
          {!searching && combined.length === 0 ? <div className="px-3 py-3"><p className="text-xs font-bold text-[#42505e]">No dropdown match yet.</p><p className="mt-1 text-[length:var(--app-label-size)] leading-4 text-[#8a949d]">Try city + country, or enter a 3-letter IATA airport code / 5-character UN/LOCODE directly.</p></div> : null}
          {combined.map((suggestion) => (
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="mt-0.5 block truncate text-[length:var(--app-label-size)] text-[#7e8992]">{suggestion.kind}{suggestion.detail ? ` · ${suggestion.detail}` : ""}</span>
              </span>
            </button>
          ))}
          <div className="border-t border-[#edf0f2] px-3 py-2 text-[length:var(--app-label-size)] leading-4 text-[#9aa2a9]">
  ...
  return <main className="min-h-screen bg-[var(--admin-canvas)] text-[var(--admin-ink)]">
    <header className="border-b border-[var(--admin-line)] bg-white px-4 py-5 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-[1500px] flex-wrap items-start justify-between gap-4">
        ...
        <span className="rounded-full border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] px-3 py-2 text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.08em] text-[#68747f]">{roleLabel}</span>
      </div>
    </header>
    ...
    <div className="mx-auto grid max-w-[1500px] gap-5 p-4 sm:p-6 lg:grid-cols-[minmax(0,1fr)_420px] lg:p-8">
      <section className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-5 shadow-sm sm:p-7">
        ...
        <form onSubmit={calculate} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <p className="md:col-span-2 xl:col-span-4 -mt-1 text-[length:var(--app-label-size)] leading-5 text-[#7a858f]">Select a dropdown location whenever possible. Freightos also accepts exact 3-letter IATA airport codes and 5-character UN/LOCODE seaport codes.</p>
          ...
          {!containerMode ? <p className="md:col-span-2 xl:col-span-4 text-[length:var(--app-label-size)] leading-5 text-[#7a858f]">Freightos requires weight plus length, width and height for boxes, crates and pallets.</p> : null}
      ...
      <aside className="space-y-4">
        ...
        {estimate ? <section className="overflow-hidden rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white shadow-sm">
          ...
          <div className="space-y-5 p-5">
            <div>
              <p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.12em] text-[#89939c]">Estimated freight range</p>
            ...
            <div className="border-t border-[#edf0f2] pt-4 text-[length:var(--app-label-size)] leading-5 text-[#88929a]">
...
function Mini({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[var(--app-radius)] border border-[#e4e7ea] bg-[var(--admin-surface-muted)] p-3"><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.1em] text-[#929ba3]">{label}</p><p className="mt-1.5 text-sm font-semibold text-[var(--admin-ink)]">{value}</p></div>;
}
