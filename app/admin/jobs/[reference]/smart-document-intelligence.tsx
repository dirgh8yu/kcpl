    ...
    >
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.25fr)_minmax(280px,.75fr)]">
        <div>
          ...
          <div className="grid gap-2 md:grid-cols-2">{relevant.map((item) => <div key={item.document_type} className={`rounded-[var(--app-radius)] border p-3 ${item.present ? "border-[#d6e1d6] bg-[#f5f9f5]" : item.required ? "border-[#ead7b7] bg-[#fff9ef]" : "border-[#e5dfd9] bg-[var(--admin-surface-muted)]"}`}>
            <div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2">{item.present ? <CheckCircle2 size={12} className="shrink-0 text-[#66806b]"/> : <FileCheck2 size={12} className="shrink-0 text-[#a46b38]"/>}<strong className="truncate text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{item.label}</strong></div><OpsBadge tone={item.present ? "success" : item.required ? "warning" : "neutral"}>{item.present ? "Present" : item.required ? "Required" : "Review"}</OpsBadge></div>
            <p className="mt-2 text-[length:var(--app-label-size)] leading-4 text-[#837970]">{item.reason}</p>
        ...
        <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-4">
          ...
          <div className="mt-3 grid gap-2 text-[length:var(--app-label-size)] text-[#655c55]"><p><strong>Lane:</strong> {workflow.document_intelligence.origin || "Origin not set"} → {workflow.document_intelligence.destination || "Destination not set"}</p><p><strong>Mode:</strong> {workflow.document_intelligence.mode || "Not set"}</p><p><strong>Cargo:</strong> {workflow.document_intelligence.cargo_type || "Not specified"}</p></div>
          ...
          {workflow.document_intelligence.advisories.length ? <div className="mt-4 rounded-[var(--app-radius)] border border-[#ead7b7] bg-[#fffaf2] p-3"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.07em] text-[#8f6835]">Verify with customs</p>{workflow.document_intelligence.advisories.map((advisory) => <p key={advisory} className="mt-2 flex items-start gap-1.5 text-[length:var(--app-label-size)] leading-4 text-[#806a50]"><AlertTriangle size={9} className="mt-0.5 shrink-0"/>{advisory}</p>)}</div> : null}
          {missing.length ? <p className="mt-4 text-[length:var(--app-label-size)] leading-4 text-[#8e5d3b]">Missing required documents block controlled final-mile progression unless Management records an audited override.</p> : <p className="mt-4 text-[length:var(--app-label-size)] leading-4 text-[#617564]">All smart-required operational documents are present.</p>}
          {error ? <p className="mt-3 text-[length:var(--app-label-size)] leading-4 text-[#a04d4d]">{error}</p> : null}
        </div>
      </div>
    </OpsSurface>
  </div>;
