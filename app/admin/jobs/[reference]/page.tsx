...
export default async function JobFilePage({ params }: { params: Promise<{ reference: string }> }) {
  ...
  return <>
    ...
    {staff.permissions.canManageFinance ? <div className="fixed bottom-5 left-5 z-50 flex flex-col gap-2"><Link href={`/admin/finance/new/${encodeURIComponent(result.job.reference)}`} className="rounded-2xl bg-emerald-700 px-4 py-3 text-[10px] font-black uppercase tracking-[.1em] text-white shadow-lg">Create invoice</Link><Link href="/admin/finance" className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-[9px] font-black uppercase tracking-[.1em] text-emerald-800 shadow-lg">Finance & AR</Link></div> : null}
  </>;
}
