    ...
    <OpsSurface eyebrow="Final mile" title="Delivery & POD Control" description="Physical delivery evidence, POD verification and canonical shipment completion are separate controlled facts." action={<div className="flex items-center gap-2"><OpsBadge tone={canonicalDelivered ? "success" : "info"}>KCPL {pretty(shipmentStatus)}</OpsBadge><OpsButton size="sm" variant="ghost" onClick={() => { setBusy(true); refresh().catch((error) => setNotice({ tone: "danger", text: error instanceof Error ? error.message : "Refresh failed." })).finally(() => setBusy(false)); }} disabled={busy}><RefreshCw size={12}/>Refresh</OpsButton></div>}>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="space-y-4">
          {canonicalDelivered ? <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-4"><div className="flex items-center gap-2"><PackageCheck size={14}/><h3 className="text-[12px] font-bold text-[var(--admin-ink)]">Canonical Delivered</h3></div><p className="mt-2 text-[length:var(--app-label-size)] leading-5 text-[#7f756d]">KCPL has completed the authoritative delivery transition. New normal delivery attempts are disabled.</p></div> : !externalDeliveredAvailable ? <form onSubmit={scheduleAttempt} className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-4">
        ...
        <div className="space-y-4">
          ...
          {deliveredAttempt ? <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-4"><div className="flex items-center justify-between"><div><h3 className="text-[12px] font-bold text-[var(--admin-ink)]">Proof of Delivery</h3><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Physical delivery {dateTime(deliveredAttempt.event_time)} · {deliveredAttempt.recipient_name || "Recipient not supplied by adopted provider evidence"}</p></div><OpsBadge tone={podTone}>{pretty(podStatus)}</OpsBadge></div>
            ...
            <div className="mt-4 space-y-2">{!attemptEvidence.length ? <OpsEmptyState icon={<Camera size={16}/>} title="No POD evidence yet" description="Upload a delivery photo, signature or POD document. Files remain private in Firebase Storage."/> : attemptEvidence.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--app-radius)] border border-[#ece5df] bg-[var(--admin-surface)] p-3"><div><div className="text-[11px] font-bold text-[#514943]">{item.filename}</div><div className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{item.kind} · {(item.size_bytes / 1024).toFixed(1)} KB · SHA-256 {item.sha256.slice(0, 12)}…</div></div><div className="flex items-center gap-2"><OpsBadge tone={item.review_status === "verified" ? "success" : item.review_status === "rejected" ? "danger" : "warning"}>{item.review_status}</OpsBadge><a href={`/api/admin/jobs/${encodeURIComponent(reference)}/delivery/evidence?evidenceId=${encodeURIComponent(item.id)}`} target="_blank" rel="noreferrer" className="ops-button" data-variant="ghost" data-size="sm">View</a></div></div>)}</div>
            {canReview && podStatus !== "verified" && attemptEvidence.length ? <div className="mt-4 border-t border-[#eee6e0] pt-4"><div className="flex items-center gap-2"><ShieldCheck size={13}/><h4 className="text-[11px] font-bold text-[#514943]">POD verification</h4></div><OpsField label="Review note"><textarea className="ops-textarea" value={reviewNote} onChange={(event) => setReviewNote(event.target.value)} placeholder="Record any verification or rejection note."/></OpsField><label className="mt-2 flex items-center gap-2 text-[length:var(--app-label-size)] text-[#6f665f]"><input type="checkbox" checked={customerSafe} onChange={(event) => setCustomerSafe(event.target.checked)}/>This verified POD packet may be exposed to a future authenticated customer portal.</label><div className="mt-3 flex flex-wrap gap-2"><OpsButton variant="primary" onClick={() => review("verify")} disabled={busy}><FileCheck2 size={12}/>Verify & reconcile</OpsButton><OpsButton variant="danger" onClick={() => review("reject")} disabled={busy}>Reject evidence</OpsButton></div></div> : null}
