      ...
      <div className="ops-content-wide ops-stack">
        <OpsSurface eyebrow="Commercial result" title="Revenue → cost → gross profit" description="Each currency remains independent. KCPL does not manufacture a consolidated margin by silently applying an FX rate.">
          {currencies.length ? <div className="grid gap-3 lg:grid-cols-2">{currencies.map((currency) => {
            ...
            return <div key={currency} className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-5"><div className="flex items-center justify-between gap-3"><strong className="text-[13px] text-[var(--admin-ink)]">{currency}</strong><OpsBadge tone={profit >= 0 ? "success" : "danger"}>{margin === undefined ? "Margin N/A" : `${margin.toFixed(1)}% margin`}</OpsBadge></div><div className="mt-4 grid grid-cols-3 gap-2"><MoneyCell label="Revenue" value={money(revenue,currency)}/><MoneyCell label="Cost" value={money(cost,currency)}/><MoneyCell label="Gross profit" value={money(profit,currency)} positive={profit >= 0}/></div></div>;
        ...
        <div className="ops-grid-2">
          <OpsSurface eyebrow="Cost ledger" title="Recognised job costs" description="Manual Job File costs and approved supplier bills share one traceable cost ledger.">
            {job.costs.length ? <div className="divide-y divide-[#eee7e1]">{job.costs.map((cost) => <div key={cost.id} className="flex items-start justify-between gap-4 py-3.5"><div className="min-w-0"><strong className="text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{cost.label}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[#958b83]">{jobCostCategoryLabels[cost.category]}{cost.vendor ? ` · ${cost.vendor}` : ""}</p>{cost.source_type === "payable" && cost.source_reference ? <Link href={`/admin/payables/bills/${encodeURIComponent(cost.source_reference)}`} className="mt-2 inline-flex items-center gap-1 text-[length:var(--app-label-size)] font-bold text-[#b5654f]"><FileText size={10}/>AP <OpsMono>{cost.source_reference}</OpsMono></Link> : <p className="mt-2 text-[length:var(--app-label-size)] text-[#aaa098]">Manual Job File cost</p>}</div><strong className="shrink-0 text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{money(cost.amount,cost.currency)}</strong></div>)}</div> : <OpsEmptyState icon={<WalletCards size={17}/>} title="No recognised costs" description="Add costs in the Digital Job File or approve a supplier bill."/>}
          </OpsSurface>
          ...
          <OpsSurface eyebrow="Accounts Payable" title="Supplier bills for this job" description={staff.permissions.canManageFinance ? "Open AP records linked to the shipment." : "Supplier payment detail is restricted to Management and Accounts."}>
            {staff.permissions.canManageFinance ? bills.length ? <div className="divide-y divide-[#eee7e1]">{bills.map((bill) => <Link key={bill.reference} href={`/admin/payables/bills/${encodeURIComponent(bill.reference)}`} className="flex items-start justify-between gap-4 py-3.5"><div><strong className="text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{bill.supplier_name}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[#958b83]"><OpsMono>{bill.reference}</OpsMono> · {payableStatusLabels[bill.status]}</p></div><div className="text-right"><strong className="text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{money(bill.total,bill.currency)}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[#958b83]">{money(bill.balance_due,bill.currency)} due</p></div></Link>)}</div> : <OpsEmptyState title="No supplier bills linked" description="Create a payable from this job when a carrier, agent or supplier cost arrives."/> : <div className="rounded-[var(--app-radius)] bg-[var(--admin-surface-muted)] p-4 text-[length:var(--app-label-size)] leading-5 text-[#80766e]">The operational job remains available, but supplier settlements are not exposed to this role.</div>}
          </OpsSurface>
        </div>
      </div>
    </OpsPage>
