...
function stageTone(state: ShipmentWorkflowReadiness["stages"][number]["state"]) {
  if (state === "complete") return "border-[var(--admin-success-line)] bg-[var(--admin-success-bg)] text-[var(--admin-success)]";
  if (state === "blocked") return "border-[var(--admin-danger-line)] bg-[var(--admin-danger-bg)] text-[var(--admin-danger)]";
  if (state === "current") return "border-[var(--admin-warning-line)] bg-[var(--admin-warning-bg)] text-[var(--admin-warning)]";
...
export function WorkflowSpine({ initialWorkflow, initialJob, canOverride }: { initialWorkflow: ShipmentWorkflowReadiness; initialJob: DigitalJobFile; canOverride: boolean }) {
  ...
  return <div className="shipment-workflow-spine ops-content-wide pt-5">
    ...
    >
      ...
      <div className="overflow-x-auto pb-1">
        <div className="flex min-w-[820px] items-start">
          {workflow.stages.map((stage, index) => <div key={stage.id} className="relative min-w-0 flex-1 px-1">
            {index < workflow.stages.length - 1 ? <span className={`absolute left-[calc(50%+13px)] right-[calc(-50%+13px)] top-[13px] h-[2px] ${stage.state === "complete" ? "bg-[var(--admin-success)]" : "bg-[var(--admin-line)]"}`} aria-hidden="true"/> : null}
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <span className="mt-2 text-[length:var(--app-label-size)] font-bold text-[var(--admin-ink)]">{stage.label}</span>
      ...
      <div className="mt-5 rounded-[var(--app-radius)] border border-[var(--admin-warning-line)] bg-[var(--admin-warning-bg)] p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="ops-eyebrow">Next operational action</p><h3 className="mt-1 text-[14px] font-[740] text-[var(--admin-ink)]">{nextActionTitle(nextAction, workflow)}</h3><p className="mt-1 max-w-3xl text-[length:var(--app-label-size)] leading-5 text-[var(--admin-muted)]">{nextActionDetail(nextAction, workflow)}</p></div>
        ...
        <div className="mt-4">
          ...
          {nextAction.kind === "customs" ? <div className="grid gap-2">{openCustoms.slice(0, 4).map((step) => <div key={step.id} className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-3"><div><strong className="text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{step.title}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-faint)]">{step.branch}{step.detail ? ` · ${step.detail}` : ""}</p></div><OpsButton variant="secondary" size="sm" disabled={Boolean(busy)} onClick={() => completeCustoms(step.id, step.title)}><ShieldCheck size={11}/>{busy === `customs:${step.id}` ? "Saving…" : "Mark complete"}</OpsButton></div>)}</div> : null}
          ...
          {nextAction.kind === "milestone" ? <div className="flex flex-wrap items-center gap-3"><OpsButton variant="primary" disabled={Boolean(busy)} onClick={() => advanceShipment(nextAction.status)}><ArrowRight size={12}/>{busy === "milestone" ? "Updating…" : `Move to ${shipmentStatusLabels[nextAction.status]}`}</OpsButton><span className="text-[length:var(--app-label-size)] text-[var(--admin-faint)]">Current location, ETA and carrier are preserved from this Job File.</span></div> : null}

          {nextAction.kind === "tasks" ? <div className="grid gap-2">{openTasks.slice(0, 4).map((task) => <div key={task.id} className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-3"><div><strong className="text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{task.title}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-faint)]">{task.branch}{task.assigned_to_name || task.assigned_to_email ? ` · ${task.assigned_to_name || task.assigned_to_email}` : " · Unassigned"}</p></div><OpsButton variant="secondary" size="sm" disabled={Boolean(busy)} onClick={() => completeTask(task.id, task.title)}><ClipboardCheck size={11}/>{busy === `task:${task.id}` ? "Saving…" : "Complete"}</OpsButton></div>)}{openTasks.length > 4 ? <p className="text-[length:var(--app-label-size)] text-[var(--admin-faint)]">+ {openTasks.length - 4} more open tasks in the detailed Job File below.</p> : null}</div> : null}
          ...
          {nextAction.kind === "customer" ? <div className="flex items-start gap-2 rounded-[var(--app-radius)] border border-[var(--admin-danger-line)] bg-white p-3 text-[length:var(--app-label-size)] leading-5 text-[var(--admin-danger)]"><AlertTriangle size={12} className="mt-0.5 shrink-0"/><span>Return to the linked enquiry or Customer 360 and confirm the CRM customer. Controlled final-mile milestones remain blocked until ownership is established.</span></div> : null}
          {nextAction.kind === "exception" ? <div className="flex items-start gap-2 rounded-[var(--app-radius)] border border-[var(--admin-danger-line)] bg-white p-3 text-[length:var(--app-label-size)] leading-5 text-[var(--admin-danger)]"><AlertTriangle size={12} className="mt-0.5 shrink-0"/><span>This shipment is in Exception. Resolve the operational issue, then use the Shipment workspace to return it to the appropriate movement stage.</span></div> : null}
          {nextAction.kind === "closed" ? <div className="flex items-start gap-2 rounded-[var(--app-radius)] border border-[var(--admin-success-line)] bg-white p-3 text-[length:var(--app-label-size)] leading-5 text-[var(--admin-success)]"><LockKeyhole size={12} className="mt-0.5 shrink-0"/><span>This operational job is closed. Its shipment, documents, customs controls and audit trail remain available as the permanent record.</span></div> : null}
          {nextAction.kind === "review" ? <div className="flex items-start gap-2 rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-3 text-[length:var(--app-label-size)] leading-5 text-[var(--admin-muted)]"><AlertTriangle size={12} className="mt-0.5 shrink-0"/><span>Review the remaining closeout controls below. The workflow is not yet ready for an automatic next milestone.</span></div> : null}
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-[1.15fr_.85fr]">
        <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-4">
          ...
          {workflow.close_blockers.length ? <div className="mt-4 grid gap-2">{workflow.close_blockers.map((blocker) => <div key={blocker} className="flex items-start gap-2 rounded-[var(--app-radius)] border border-[var(--admin-danger-line)] bg-[var(--admin-danger-bg)] p-2.5 text-[length:var(--app-label-size)] leading-4 text-[var(--admin-danger)]"><AlertTriangle size={11} className="mt-0.5 shrink-0"/><span>{blocker}</span></div>)}</div> : <div className="mt-4 flex items-start gap-2 rounded-[var(--app-radius)] border border-[var(--admin-success-line)] bg-[var(--admin-success-bg)] p-3 text-[length:var(--app-label-size)] leading-4 text-[var(--admin-success)]"><FileCheck2 size={12} className="mt-0.5 shrink-0"/><span>All operational closeout controls are satisfied. This job is ready to close.</span></div>}
          {workflow.warnings.length ? <div className="mt-3 space-y-1">{workflow.warnings.map((warning) => <p key={warning} className="text-[length:var(--app-label-size)] leading-4 text-[var(--admin-faint)]">• {warning}</p>)}</div> : null}
        </div>

        <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-4">
          <div className="flex items-center gap-2"><Landmark size={13} className="text-[var(--admin-crimson)]"/><div><p className="ops-eyebrow">Finance lane</p><h3 className="mt-1 text-[12px] font-bold text-[var(--admin-ink)]">Parallel, not a cargo blocker</h3></div></div>
          ...
          {workflow.customer_id ? <p className="mt-2 text-[length:var(--app-label-size)] text-[var(--admin-faint)]">Customer <OpsMono>{workflow.customer_id}</OpsMono></p> : null}
        </div>
      </div>
    </OpsSurface>
  </div>;
...
function Metric({ label, value }: { label: string; value: number }) {
  return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-2.5"><p className="text-[7px] font-bold uppercase tracking-[.07em] text-[var(--admin-muted)]">{label}</p><p className="mt-1 text-[13px] font-bold text-[var(--admin-ink)]">{value}</p></div>;
}
