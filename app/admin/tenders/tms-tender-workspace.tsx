      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
        <OpsSurface eyebrow="Planning orders" title="Orders ready for procurement" description="Choose an order that already has a selected Partner buy rate.">
          <div className="space-y-2">
            {orders.length ? orders.map((order) => <button key={order.id} type="button" onClick={() => chooseOrder(order.id)} className={`w-full rounded-[var(--app-radius)] border p-3 text-left ${selectedOrderId === order.id ? "border-[#d48a76] bg-[#fff8f4]" : "border-[var(--admin-line)] bg-white"}`}>
              ...
              <p className="mt-1 text-[length:var(--app-label-size)] text-[#887f78]">{order.customer_name || "Customer not linked"} · {order.branch}</p>
          ...
          {!selectedOrder ? <OpsSurface><OpsEmptyState title="Choose a transport order" description="Tender controls appear after an order is selected."/></OpsSurface> : <>
            <OpsSurface eyebrow="Order control" title={`${selectedOrder.origin} → ${selectedOrder.destination}`} description={`${selectedOrder.id} · ${selectedOrder.mode} · ${selectedOrder.branch}`}>
              <div className="grid gap-3 md:grid-cols-3">
                <div><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.08em] text-[#938980]">Selected procurement</p><p className="mt-1 text-[12px] font-bold text-[#403933]">{money(selectedOrder.selected_cost, selectedOrder.selected_currency)}</p></div>
                <div><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.08em] text-[#938980]">Customer</p><p className="mt-1 text-[12px] font-bold text-[#403933]">{selectedOrder.customer_name || "Not linked"}</p></div>
                <div><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.08em] text-[#938980]">State</p><div className="mt-1"><OpsBadge tone={selectedOrder.status === "booked" ? "success" : selectedOrder.status === "tendering" ? "info" : "warning"}>{selectedOrder.status}</OpsBadge></div></div>
              </div>
            ...
            <OpsSurface eyebrow="Tender history" title="Procurement decisions" description="Every response and booking is retained against the transport order.">
              <div className="space-y-3">
                {selectedTenders.length ? selectedTenders.map((tender) => <div key={tender.id} className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><OpsMono>{tender.tender_reference}</OpsMono><OpsBadge tone={statusTone(tender.status)}>{tmsTenderStatusLabels[tender.status]}</OpsBadge>{tender.channel === "email" ? <OpsBadge tone="info"><Mail size={10}/> Email</OpsBadge> : <OpsBadge>Manual</OpsBadge>}</div><strong className="mt-2 block text-[12px] text-[#403933]">{tender.partner_name}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[#887f78]">Offered {money(tender.offered_cost, tender.currency)} · due {dateTime(tender.response_due_at)}</p></div>{tender.shipment_reference ? <Link href={`/admin/jobs/${encodeURIComponent(tender.shipment_reference)}`} className="ops-button" data-variant="primary" data-size="sm">Open Job File <ArrowRight size={12}/></Link> : null}</div>

                  {tender.counter_cost !== null ? <p className="mt-3 rounded-[var(--app-radius)] bg-[#fff8ee] px-3 py-2 text-[length:var(--app-label-size)] text-[#795d34]">Counter-offer: <strong>{money(tender.counter_cost, tender.counter_currency)}</strong>{tender.response_note ? ` · ${tender.response_note}` : ""}</p> : tender.response_note ? <p className="mt-3 text-[length:var(--app-label-size)] leading-5 text-[#766d66]">{tender.response_note}</p> : null}
