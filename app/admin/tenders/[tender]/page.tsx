    ...
    <div className="mx-auto w-full max-w-[1152px]">
      <header className="border-b border-[var(--admin-line)] pb-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="text-[23px] font-semibold leading-[31px] tracking-[-.02em]">{tender.booking_reference || "Booking confirmed"}</h1><span className="inline-flex rounded-[var(--app-radius)] bg-[var(--admin-success-bg)] px-[7px] py-[3px] text-[11px] font-medium text-[var(--admin-success)]">Booked</span></div><p className="mt-1 text-[14px] font-medium">{tender.origin} → {tender.destination}</p><p className="mt-1 text-[12px] text-[var(--admin-muted)]">{order?.customer_name || "Customer not linked"} · {tender.partner_name} · {titleCase(tender.mode)}</p><p className="mt-1 text-[11px] text-[var(--admin-muted)]">{tender.tender_reference} · {tender.order_id} · Booked {dateTime(tender.booked_at)}</p></div>
          <div className="flex flex-wrap gap-2"><Link href={`/admin/tenders?tender=${encodeURIComponent(tender.tender_reference)}`} className="inline-flex h-8 items-center rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] px-3 text-[12px] font-semibold">Tender workspace</Link>{tender.shipment_reference ? <Link href={`/admin/jobs/${encodeURIComponent(tender.shipment_reference)}`} className="inline-flex h-8 items-center rounded-[var(--app-radius)] bg-[var(--admin-crimson)] px-3 text-[12px] font-semibold text-[var(--admin-on-crimson)]">Open shipment</Link> : null}</div>
        </div>
        <nav className="ops-scroll-x mt-5 flex h-9 items-end gap-5 overflow-x-auto text-[12px] font-medium"><span className="relative flex h-8 shrink-0 items-center text-[var(--admin-ink)]">Overview<span className="absolute inset-x-0 bottom-0 h-0.5 bg-[var(--admin-crimson)]"/></span><Link href={`/admin/rating/${encodeURIComponent(tender.order_id)}`} className="flex h-8 shrink-0 items-center text-[var(--admin-muted)] hover:text-[var(--admin-ink)]">Transport Order</Link><Link href={`/admin/tenders?tender=${encodeURIComponent(tender.tender_reference)}`} className="flex h-8 shrink-0 items-center text-[var(--admin-muted)] hover:text-[var(--admin-ink)]">Tender</Link>{tender.shipment_reference ? <Link href={`/admin/jobs/${encodeURIComponent(tender.shipment_reference)}`} className="flex h-8 shrink-0 items-center text-[var(--admin-muted)] hover:text-[var(--admin-ink)]">Shipment / Job File</Link> : null}</nav>
      </header>

      <div className="grid lg:grid-cols-[minmax(0,800px)_351px]">
        <div className="min-w-0 lg:border-r lg:border-[var(--admin-line)] lg:pr-7">
          <Section title="Booking basis"><Grid><Datum label="Originating order" value={tender.order_id}/><Datum label="Accepted tender" value={tender.tender_reference}/><Datum label="Partner / carrier" value={tender.partner_name}/><Datum label="Commercial outcome" value={commercialBasis}/><Datum label="Agreed procurement" value={bookedAmount}/><Datum label="Rate card snapshot" value={tender.rate_card_id}/></Grid><p className="mt-4 text-[12px] leading-[19px] text-[var(--admin-muted)]">Booking was requested from the accepted tender outcome. Later editable Partner rates do not replace this recorded procurement basis.</p></Section>
          ...
          <Section title="Partner response"><Grid><Datum label="Channel" value={tender.channel}/><Datum label="Sent" value={dateTime(tender.sent_at)}/><Datum label="Responded" value={dateTime(tender.responded_at)}/><Datum label="Response deadline" value={dateTime(tender.response_due_at)}/></Grid>{tender.response_note ? <p className="mt-4 whitespace-pre-wrap text-[12px] leading-[19px] text-[var(--admin-muted)]">{tender.response_note}</p> : null}</Section>
        </div>

        <aside className="bg-[var(--admin-surface)] px-6 py-5 lg:min-h-[720px]">
          <p className="text-[11px] font-medium text-[var(--admin-muted)]">NEXT ACTION</p><h2 className="mt-1 text-[15px] font-semibold leading-[22px]">Move booking into execution</h2><p className="mt-1 text-[12px] leading-[19px] text-[var(--admin-muted)]">The commercial handoff is complete. Continue through the linked shipment, pickup and Job File rather than mutating booking authority here.</p>
          <div className="mt-5 border-t border-[var(--admin-line)] py-4"><p className="text-[11px] font-medium text-[var(--admin-muted)]">OWNERSHIP</p><DetailRow label="Branch" value={order?.branch || "Not exposed"}/><DetailRow label="Created by" value={tender.created_by_name || tender.created_by_email}/><DetailRow label="Customer" value={order?.customer_name || "Not linked"}/></div>
          <div className="border-t border-[var(--admin-line)] py-4"><p className="text-[11px] font-medium text-[var(--admin-muted)]">LINKED RECORDS</p><LinkRow label="Transport Order" value={tender.order_id} href={`/admin/rating/${encodeURIComponent(tender.order_id)}`}/><LinkRow label="Tender" value={tender.tender_reference} href={`/admin/tenders?tender=${encodeURIComponent(tender.tender_reference)}`}/>{tender.shipment_reference ? <LinkRow label="Shipment / Job File" value={tender.shipment_reference} href={`/admin/jobs/${encodeURIComponent(tender.shipment_reference)}`}/> : <DetailRow label="Shipment" value="Not linked"/>}</div>
          <div className="border-t border-[var(--admin-line)] py-4"><p className="text-[11px] font-medium text-[var(--admin-muted)]">EXECUTION</p>{tender.shipment_reference ? <div className="mt-3 flex flex-col gap-2"><Link href={`/admin/jobs/${encodeURIComponent(tender.shipment_reference)}`} className="inline-flex h-8 items-center justify-center rounded-[var(--app-radius)] bg-[var(--admin-crimson)] px-3 text-[12px] font-semibold text-[var(--admin-on-crimson)]">Open shipment</Link><Link href={`/admin/pickups?shipment=${encodeURIComponent(tender.shipment_reference)}`} className="inline-flex h-8 items-center justify-center rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] px-3 text-[12px] font-semibold">Open pickup workflow</Link></div> : <p className="mt-2 text-[12px] leading-[19px] text-[var(--admin-muted)]">No shipment reference is currently available from the booking record.</p>}</div>
          <div className="border-t border-[var(--admin-line)] py-4"><p className="text-[11px] font-medium text-[var(--admin-muted)]">AUDIT FACTS</p><DetailRow label="Booked" value={dateTime(tender.booked_at)}/><DetailRow label="Updated" value={dateTime(tender.updated_at)}/></div>
        </aside>
      </div>
    </div>
  </main>;
...
function NotBooked({ tender }: { tender: TmsTender }) {
  return <main className="min-h-[calc(100vh-54px)] bg-[var(--admin-canvas)] px-4 py-8 text-[var(--admin-ink)] sm:px-6 lg:px-7"><div className="mx-auto max-w-[900px] border-y border-[var(--admin-line)] bg-[var(--admin-surface)] px-6 py-8"><p className="text-[11px] font-semibold uppercase tracking-[.08em] text-[var(--admin-muted)]">{tender.tender_reference}</p><div className="mt-2 flex flex-wrap items-center gap-2"><h1 className="text-[22px] font-semibold">Booking not confirmed</h1><span className="rounded-[var(--app-radius)] bg-[var(--admin-warning-bg)] px-[7px] py-[3px] text-[11px] font-medium text-[var(--admin-warning)]">{titleCase(tender.status)}</span></div><p className="mt-3 max-w-2xl text-[13px] leading-6 text-[var(--admin-muted)]">This tender has not produced an authoritative booking. Only an accepted or valid counter-offer can request booking, and the server decides whether that transition is valid.</p><div className="mt-6 flex gap-2"><Link href={`/admin/tenders?tender=${encodeURIComponent(tender.tender_reference)}`} className="inline-flex h-8 items-center rounded-[var(--app-radius)] bg-[var(--admin-crimson)] px-3 text-[12px] font-semibold text-[var(--admin-on-crimson)]">Return to tender</Link><Link href={`/admin/rating/${encodeURIComponent(tender.order_id)}`} className="inline-flex h-8 items-center rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] px-3 text-[12px] font-semibold">Transport Order</Link></div></div></main>;
}
...
function Datum({ label, value }: { label: string; value: string }) { return <div><dt className="text-[11px] font-medium text-[var(--admin-muted)]">{label}</dt><dd className="mt-1 break-words text-[13px] font-medium leading-[19px]">{value}</dd></div>; }
...
function Gate({ title, detail, embedded = false }: { title: string; detail: string; embedded?: boolean }) { return <main className={`grid place-items-center bg-[var(--admin-canvas)] p-6 text-[var(--admin-ink)] ${embedded ? "min-h-[calc(100vh-54px)]" : "min-h-screen"}`}><section className="w-full max-w-xl border-y border-[var(--admin-line)] bg-[var(--admin-surface)] p-8"><p className="text-[11px] font-semibold uppercase tracking-[.08em] text-[var(--admin-crimson)]">KCPL Booking</p><h1 className="mt-3 text-[22px] font-semibold">{title}</h1><p className="mt-3 text-[13px] leading-6 text-[var(--admin-muted)]">{detail}</p><div className="mt-6 flex gap-2"><Link href="/admin/tenders" className="inline-flex h-8 items-center rounded-[var(--app-radius)] bg-[var(--admin-crimson)] px-3 text-[12px] font-semibold text-[var(--admin-on-crimson)]">Tender Workspace</Link><Link href="/admin/rating" className="inline-flex h-8 items-center rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] px-3 text-[12px] font-semibold">Transport Orders</Link></div></section></main>; }
