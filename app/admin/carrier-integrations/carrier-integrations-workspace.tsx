...
function stateTone(state: CarrierProviderDashboard["state"]) {
  ...
  return "border-[#e5ded8] bg-[#faf8f6] text-[var(--admin-muted)]";
}
...
export function CarrierIntegrationsWorkspace({
  ...
  return <div className="ops-content-wide py-5">
    ...
    {message ? <div className="mt-4 rounded-[var(--app-radius)] border border-[#cfe1d3] bg-[#f4faf5] px-3 py-2 text-[length:var(--app-label-size)] text-[#55755d]">{message}</div> : null}
    {error ? <div className="mt-4 rounded-[var(--app-radius)] border border-[#edc8c4] bg-[#fff4f2] px-3 py-2 text-[length:var(--app-label-size)] text-[#a6534d]">{error}</div> : null}

    <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
      {stats.map(({ label, value, Icon }) => <div key={label} className="rounded-[var(--app-radius)] border border-[#e3ddd7] bg-white p-3.5"><div className="flex items-center gap-2 text-[#b45f4b]"><Icon size={13}/><span className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em]">{label}</span></div><p className="mt-2 text-[22px] font-[760] tracking-[-.04em] text-[#443d38]">{value}</p></div>)}
    </div>

    <section className="mt-5 grid gap-3 lg:grid-cols-2">
      {providers.map((provider) => <article key={provider.id} className="rounded-[var(--app-radius)] border border-[#e2dbd5] bg-[var(--admin-surface)] p-4 shadow-[0_8px_24px_rgba(54,43,34,.035)]">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">{provider.capabilities.map((capability) => <span key={capability} className={`rounded-full border px-2 py-1 text-[length:var(--app-label-size)] font-bold uppercase tracking-[.06em] ${provider.active_capabilities.includes(capability) ? "border-[#dfc9bd] bg-[#fff8f3] text-[#a65f4c]" : "border-[#e7e1dc] bg-white text-[var(--admin-muted)]"}`}>{capability}</span>)}</div>
        <p className="mt-3 text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]">{provider.docs_note}</p>
        <div className="mt-3 grid gap-1.5 text-[length:var(--app-label-size)] text-[#746b64] sm:grid-cols-2"><p>Last success: <strong>{fmt(provider.last_success_at)}</strong></p><p>Last failure: <strong>{fmt(provider.last_failure_at)}</strong></p><p>Last action: <strong>{provider.last_action || "None"}</strong></p><p>Latency: <strong>{provider.last_latency_ms === null ? "—" : `${provider.last_latency_ms} ms`}</strong></p></div>
        {provider.last_message ? <p className="mt-3 rounded-[var(--app-radius)] border border-[#ebe5df] bg-white px-2.5 py-2 text-[length:var(--app-label-size)] leading-4 text-[#756c65]">{provider.last_message}</p> : null}
        {provider.id === "maersk_ocean" ? <p className="mt-3 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Webhook endpoint: <code className="rounded bg-[#f4f0ec] px-1 py-0.5">/api/integrations/carriers/maersk</code></p> : null}
      </article>)}
    </section>

    {canViewCommercial ? <section className="mt-5 rounded-[var(--app-radius)] border border-[#e2dbd5] bg-white p-4">
      <div className="flex items-center gap-2 text-[#b45f4b]"><Ship size={14}/><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em]">Maersk commercial schedules</p></div>
