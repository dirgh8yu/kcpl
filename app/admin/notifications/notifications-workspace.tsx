...
export function NotificationsWorkspace() {
  ...
  return <OpsPage><main className="min-h-[calc(100vh-64px)] bg-[var(--admin-canvas)] text-[var(--admin-ink)]"><div className="mx-auto w-full max-w-[1320px] px-4 pb-16 pt-8 sm:px-6 lg:px-8">
    <header className="grid gap-6 border-b border-[#101010] pb-7 md:grid-cols-[minmax(0,1fr)_auto] md:items-end"><div><p className="text-[length:var(--app-label-size)] uppercase tracking-[0.11em] text-[var(--admin-crimson)]">Operations · Signal log</p><h1 className="mt-3 text-[clamp(36px,4vw,52px)] font-normal leading-[1.04] tracking-[-0.04em]">Notification Centre</h1><p className="mt-3 max-w-2xl text-[14px] leading-6 text-[var(--admin-muted)]">Your retained operational signal history across assignments and automation alerts. Unread work stays visible without turning the page into a transient topbar feed.</p><p className="mt-3 text-[length:var(--app-label-size)] uppercase tracking-[0.05em] text-[var(--admin-muted)]">Auto-refresh · 30 seconds · Nepal time</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => void load()} disabled={loading} className="inline-flex h-10 items-center gap-2 border border-[var(--admin-line-strong)] px-3 text-[12px] hover:border-[#101010] hover:bg-[var(--admin-surface-muted)] disabled:opacity-50"><RefreshCw size={13} className={loading ? "animate-spin" : ""}/>Refresh</button>{counts.unread ? <button type="button" onClick={() => void markAllRead()} disabled={busy} className="inline-flex h-10 items-center gap-2 border border-[var(--admin-crimson)] bg-[var(--admin-crimson)] px-4 text-[12px] font-medium text-white hover:border-[var(--admin-crimson-dark)] hover:bg-[var(--admin-crimson-dark)] disabled:opacity-50"><CheckCheck size={13}/>{busy ? "Updating…" : "Mark all read"}</button> : null}</div></header>

    <section className="grid border-b border-[var(--admin-line)] sm:grid-cols-4"><Metric label="Unread" value={counts.unread} alert active={state === "unread"} onClick={() => setState(state === "unread" ? "all" : "unread")}/><Metric label="Critical" value={counts.critical} alert active={severity === "critical"} onClick={() => setSeverity(severity === "critical" ? "all" : "critical")}/><Metric label="Warnings" value={counts.warning} alert active={severity === "warning"} onClick={() => setSeverity(severity === "warning" ? "all" : "warning")}/><Metric label="Resolved" value={counts.resolved} active={state === "resolved"} onClick={() => setState(state === "resolved" ? "all" : "resolved")}/></section>
    ...
    <section className="mt-6 border-y border-[var(--admin-line)]">
      <div className="grid gap-3 border-b border-[#101010] py-4 lg:grid-cols-[minmax(260px,1fr)_190px_150px_160px_auto]"><label className="flex h-10 items-center border border-[var(--admin-line-strong)] bg-white px-3"><Search size={13} className="mr-2 text-[var(--admin-muted)]"/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search notification, branch, reference or detail" className="min-w-0 flex-1 bg-transparent text-[12px] outline-none placeholder:text-[var(--admin-muted)]"/></label><select className="ops-select" value={category} onChange={(event) => setCategory(event.target.value as "all" | NotificationCategory)}><option value="all">All categories</option>{notificationCategories.map((item) => <option key={item} value={item}>{notificationCategoryLabels[item]}</option>)}</select><select className="ops-select" value={state} onChange={(event) => setState(event.target.value as StateFilter)}><option value="all">All states</option><option value="unread">Unread</option><option value="read">Read</option><option value="resolved">Resolved</option></select><select className="ops-select" value={severity} onChange={(event) => setSeverity(event.target.value as SeverityFilter)}><option value="all">All severities</option><option value="critical">Critical</option><option value="warning">Warning</option><option value="info">Info</option></select><button type="button" onClick={reset} className="h-10 border border-[var(--admin-line-strong)] px-3 text-[12px] hover:border-[#101010] hover:bg-[var(--admin-surface-muted)]">Reset</button></div>

      <div className="hidden min-h-11 grid-cols-[110px_170px_minmax(260px,1fr)_180px_150px_24px] items-center gap-4 border-b border-[#101010] px-3 text-[length:var(--app-label-size)] uppercase tracking-[0.06em] text-[var(--admin-muted)] lg:grid"><span>State</span><span>Category</span><span>Signal</span><span>Branch / source</span><span>Time</span><span/></div>
      {loading && !data ? <div className="py-12 text-center text-[11px] text-[var(--admin-muted)]">Loading notification history…</div> : filtered.length ? <div>{filtered.map((item) => {
        const unread = !item.read_at && !item.resolved;
        return <button key={item.id} type="button" onClick={() => void openNotification(item)} className={`group relative grid w-full gap-3 border-b border-[var(--admin-line)] px-3 py-4 text-left last:border-b-0 lg:grid-cols-[110px_170px_minmax(260px,1fr)_180px_150px_24px] lg:items-start ${unread ? "bg-white hover:bg-[var(--admin-surface-muted)]" : "opacity-75 hover:bg-[var(--admin-surface-muted)]"}`}>{unread ? <span className="absolute bottom-2 left-0 top-2 w-[2px] bg-[var(--admin-crimson)]"/> : null}<div className="flex flex-wrap gap-1.5">{item.resolved ? <OpsBadge tone="success">Resolved</OpsBadge> : unread ? <OpsBadge tone="accent">Unread</OpsBadge> : <OpsBadge>Read</OpsBadge>}<OpsBadge tone={severityTone(item.severity)}>{item.severity}</OpsBadge></div><p className="text-[length:var(--app-label-size)] font-medium uppercase tracking-[0.04em] text-[var(--admin-muted)]">{notificationCategoryLabels[item.category]}</p><div className="min-w-0"><strong className={`block text-[13px] tracking-[-0.01em] ${unread ? "font-medium text-[var(--admin-ink)]" : "font-normal text-[#4F4F4A]"}`}>{item.title}</strong><p className="mt-1 text-[11px] leading-5 text-[#666660]">{item.detail}</p></div><div className="text-[length:var(--app-label-size)] leading-5 text-[#666660]"><p>{item.branch || "No branch"}</p><p className="mt-1"><OpsMono>{item.source_id}</OpsMono></p></div><p className="text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]">{dateTime(item.created_at)}</p><ChevronRight size={14} className="text-[#A0A09A] transition-transform group-hover:translate-x-0.5 group-hover:text-[var(--admin-crimson)]"/></button>;
    ...
    <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#101010] pt-4 text-[length:var(--app-label-size)] uppercase tracking-[0.05em] text-[var(--admin-muted)]"><span>{filtered.length} of {notifications.length} retained signals shown</span><span>{data?.email_configured ? "Email channel configured" : "In-app channel active"}</span></div>
  </div></main></OpsPage>;
}
