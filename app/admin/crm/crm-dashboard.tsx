  ...
  return (
    <main className="min-h-[calc(100vh-58px)] bg-[var(--admin-canvas)]">
      <section className="border-b border-[var(--admin-line)] bg-[var(--admin-surface)]/72 px-5 py-6 backdrop-blur-xl lg:px-8">
        <div className="mx-auto flex max-w-[1680px] flex-wrap items-end justify-between gap-5"><div><p className="ops-eyebrow">Relationships</p><h1 className="mt-2 text-[31px] font-[730] tracking-[-.045em] text-[var(--admin-ink)]">Customers</h1><p className="mt-2 max-w-2xl text-[11px] leading-5 text-[#887e76]">Customer accounts that buy KCPL freight and logistics services. Carriers, agents, transporters, suppliers and overseas counterparts live in Partners.</p></div><div className="flex items-center gap-2"><span className="hidden text-[length:var(--app-label-size)] font-semibold text-[var(--admin-muted)] sm:inline">Working as {userName}</span><OpsButton variant="primary" onClick={openNew}><Plus size={13}/>New record</OpsButton></div></div>
      </section>
...
function CustomerOverview({ customer, onNew, commercialVisible }: { customer: CrmCustomerSummary; onNew: () => void; commercialVisible: boolean }) {
  ...
  return <div className="mx-auto max-w-6xl ops-stack">
    <div className="flex flex-wrap items-start justify-between gap-5"><div><div className="flex flex-wrap items-center gap-2"><OpsMono className="text-[length:var(--app-label-size)] text-[#9b7060]">{customer.id}</OpsMono><OpsBadge tone={statusTone(customer.account_status)} dot>{crmAccountStatusLabels[customer.account_status]}</OpsBadge>{customer.relationship_types.map((type) => <OpsBadge key={type}>{crmRelationshipLabels[type]}</OpsBadge>)}</div><h2 className="mt-3 text-[30px] font-[735] tracking-[-.045em] text-[#39312c]">{customer.display_name}</h2><p className="mt-2 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{customer.primary_email || "No primary email"}{customer.primary_phone ? ` · ${customer.primary_phone}` : ""} · {customer.country}</p></div><div className="flex gap-2"><OpsButton variant="secondary" onClick={onNew}><Plus size={12}/>New record</OpsButton><Link href={`/admin/crm/${encodeURIComponent(customer.id)}`} className="ops-button" data-variant="primary" data-size="md">Open Customer 360 <ArrowRight size={12}/></Link></div></div>
  ...
  return <div className="mx-auto max-w-5xl ops-stack">
    ...
    <form onSubmit={(event) => onSubmit(event, false)} className="ops-stack">
      <OpsSurface eyebrow="Identity" title="Who is this?">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2"><OpsBadge tone="info">Customer</OpsBadge><span className="text-[length:var(--app-label-size)] text-[#756e67]">This workspace is for buyers of KCPL services. Operational suppliers and counterparts belong in Partners.</span></div>
      </OpsSurface>
...
function MiniStat({ icon, label, value, warn = false }: { icon: React.ReactNode; label: string; value: number; warn?: boolean }) { return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-4"><div className={`flex items-center gap-2 ${warn ? "text-[#a46a2f]" : "text-[#958b83]"}`}>{icon}<span className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em]">{label}</span></div><strong className={`mt-2 block text-[21px] tracking-[-.035em] ${warn ? "text-[#9a682f]" : "text-[var(--admin-ink)]"}`}>{value}</strong></div>; }
function Fact({ label, value }: { label: string; value: string }) { return <div><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em] text-[var(--admin-muted)]">{label}</p><p className="mt-1.5 text-[length:var(--app-label-size)] font-semibold text-[#5b524b]">{value}</p></div>; }
function MoneyLine({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) { return <div className="flex items-center justify-between gap-4 py-3 text-[length:var(--app-label-size)]"><span className="text-[var(--admin-muted)]">{label}</span><strong className={strong ? "text-[12px] text-[#66806b]" : "text-[var(--admin-ink)]"}>{value}</strong></div>; }
