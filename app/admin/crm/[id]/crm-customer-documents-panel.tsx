    ...
    <section className="bg-[var(--admin-surface-muted)] px-5 pb-6 lg:px-8">
      <div className="mx-auto max-w-[1500px] rounded-[var(--app-radius)] border border-black/10 bg-white shadow-sm">
        ...
        <form onSubmit={upload} className="mx-6 mt-5 grid gap-3 rounded-[var(--app-radius)] border border-black/10 bg-[var(--admin-surface-muted)] p-4 sm:mx-8 md:grid-cols-[220px_1fr_auto] md:items-end">
          ...
          <label><span className="mb-1.5 block text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.13em] text-black/40">File · max 15 MB</span><input ref={fileRef} type="file" className="block w-full rounded-[var(--app-radius)] border border-black/10 bg-white px-3 py-2.5 text-xs" accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx,.xls,.xlsx,.csv,.txt" /></label>
          <button type="submit" disabled={busy || !storageAvailable} className="flex items-center justify-center gap-2 rounded-[var(--app-radius)] bg-[var(--admin-crimson)] px-4 py-3 text-xs font-semibold text-white disabled:opacity-50"><Upload size={14} />{busy ? "Working…" : "Upload"}</button>
        </form>
        ...
        <div className="p-6 sm:p-8">
          {documents.length ? <div className="divide-y divide-black/10 rounded-[var(--app-radius)] border border-black/10">{documents.map((document) => <div key={document.id} className="flex flex-wrap items-center justify-between gap-4 p-4">
            <div className="flex min-w-0 items-center gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-[var(--app-radius)] bg-[var(--admin-crimson)] text-white"><FileText size={16} /></div><div className="min-w-0"><strong className="block truncate text-sm">{document.filename}</strong><p className="mt-1 text-[length:var(--app-label-size)] font-bold text-black/40">{crmCustomerDocumentTypeLabels[document.document_type]} · {formatBytes(document.size_bytes)} · {formatDate(document.uploaded_at)} · {document.uploaded_by}</p></div></div>
            <div className="flex gap-2"><a href={`/api/admin/crm/customers/${encodeURIComponent(customerId)}/documents/${document.id}`} className="flex items-center gap-1 rounded-[var(--app-radius)] border border-black/10 bg-[var(--admin-surface-muted)] px-2.5 py-2 text-[length:var(--app-label-size)] font-semibold"><Download size={11} />Download</a><button type="button" disabled={busy} onClick={() => remove(document)} className="flex items-center gap-1 rounded-[var(--app-radius)] border border-rose-200 bg-rose-50 px-2.5 py-2 text-[length:var(--app-label-size)] font-semibold text-rose-700"><Trash2 size={11} />Delete</button></div>
