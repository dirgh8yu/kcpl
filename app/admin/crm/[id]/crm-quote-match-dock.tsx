...
function QuoteRow({ item, action }: { item: CrmQuoteLinkItem; action: React.ReactNode }) {
  return <div className="rounded-[var(--app-radius)] border border-black/10 bg-[var(--admin-surface-muted)] p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><strong className="text-xs">{item.reference}</strong><p className="mt-1 flex items-center gap-1 text-[length:var(--app-label-size)] font-bold text-black/45"><span className="truncate">{item.origin}</span><ArrowRight size={10} /><span className="truncate">{item.destination}</span></p></div>{action}</div><p className="mt-2 truncate text-[length:var(--app-label-size)] text-black/45">{item.company_name || item.contact_name} · {formatDate(item.created_at)}</p>{item.match_reason ? <p className="mt-1 text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.08em] text-amber-700">Matched by {item.match_reason.replace("_", " ")}</p> : null}</div>;
}
