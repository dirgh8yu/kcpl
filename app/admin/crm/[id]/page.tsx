...
export default async function Customer360Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    ...
    >
      ...
      <div className="ops-page-body ops-stack pt-0">
...
function CustomerGate({ title, detail }: { title: string; detail: string }) {
  return <main className="grid min-h-screen place-items-center bg-[#f7f7f5] p-6 text-[#1c2025]"><section className="w-full max-w-xl rounded-xl border border-[#e2e5e8] bg-white p-8"><p className="text-[10px] font-semibold uppercase tracking-[.1em] text-[#717a86]">KCPL Customer 360</p><h1 className="mt-3 text-2xl font-semibold tracking-[-.03em]">{title}</h1><p className="mt-3 text-sm leading-6 text-[#68717a]">{detail}</p><div className="mt-6 flex flex-wrap gap-2"><Link href="/admin/crm" className="ops-button ops-button-primary">Customers</Link><Link href="/admin" className="ops-button ops-button-secondary">Enquiries</Link></div></section></main>;
}
