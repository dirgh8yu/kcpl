...
export function EdiWorkspace({ initialRows, initialSummary, initialConfigured, initialEligibleTenders, canQueue204 }: {
  ...
  return <div className="ops-content-wide py-5">
    ...
    <div className={`mt-4 rounded-[var(--app-radius)] border px-3 py-2.5 text-[length:var(--app-label-size)] ${configured ? "border-[#cfe1d3] bg-[#f4faf5] text-[#55755d]" : "border-[#eadcc2] bg-[#fffaf0] text-[#8d6b38]"}`}><strong>{configured ? "EDI transport authenticated" : "EDI transport not configured"}</strong> · {configured ? "VAN/middleware can poll outbound 204s and post inbound 990/214 messages." : "Set KCPL_EDI_SECRET in Firebase Secret Manager before external EDI transport can connect. Internal ledger and tender workflow remain available."}</div>
    {message ? <div className="mt-3 rounded-[var(--app-radius)] border border-[#cfe1d3] bg-[#f4faf5] px-3 py-2 text-[length:var(--app-label-size)] text-[#55755d]">{message}</div> : null}
    {error ? <div className="mt-3 rounded-[var(--app-radius)] border border-[#edc8c4] bg-[#fff4f2] px-3 py-2 text-[length:var(--app-label-size)] text-[#a6534d]">{error}</div> : null}
    ...
    {canQueue204 ? <section className="mt-5 rounded-[var(--app-radius)] border border-[#e2dbd5] bg-white p-4">
      <div className="flex items-center gap-2 text-[#b45f4b]"><Cable size={14}/><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em]">204 tender handoff</p></div>
      ...
      <div className="mt-3 grid gap-2 lg:grid-cols-2">{eligibleTenders.length ? eligibleTenders.slice(0, 12).map((tender) => <div key={tender.id} className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-3"><div className="flex items-start justify-between gap-3"><div><Link href={`/admin/tenders?tender=${encodeURIComponent(tender.id)}`} className="text-[11px] font-bold text-[#a65f4c] hover:underline">{tender.tender_reference}</Link><p className="mt-1 text-[length:var(--app-label-size)] text-[#877d75]">{tender.partner_name} · {tender.origin} → {tender.destination}</p></div><span className="rounded-full border border-[#e7ddd6] bg-white px-2 py-1 text-[length:var(--app-label-size)] font-bold uppercase text-[#877d75]">{tender.channel.replaceAll("_", " ")}</span></div><div className="mt-3 flex justify-end"><button className="ops-button" data-variant="primary" data-size="sm" disabled={busy === tender.id || tender.channel === "edi_204"} onClick={() => queue204(tender)}>{tender.channel === "edi_204" ? "204 queued" : busy === tender.id ? "Queueing…" : "Queue EDI 204"}</button></div></div>) : <p className="text-[length:var(--app-label-size)] text-[var(--admin-muted)]">No sent manual tenders are waiting for an EDI handoff.</p>}</div>
    </section> : null}

    <section className="mt-5 overflow-hidden rounded-[var(--app-radius)] border border-[#e2dbd5] bg-white">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#ebe5df] px-4 py-3"><div><p className="text-[11px] font-[730] text-[#4a423d]">EDI transaction ledger</p><p className="mt-1 text-[length:var(--app-label-size)] text-[#887e76]">Outbound and inbound message history without exposing raw X12 payloads to the browser.</p></div><div className="relative"><Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#9a9088]"/><input className="ops-input w-64 pl-8" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search reference, partner, set"/></div></div>
