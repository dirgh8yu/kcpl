...
export async function queueTenderAsEdi204(tenderIdValue: string, actor: Actor) {
  ...
  const branch = text(tender.get("branch"));
  const result = await queueEdi204({
    branch: branch as Parameters<typeof queueEdi204>[0]["branch"],
    tenderId,
