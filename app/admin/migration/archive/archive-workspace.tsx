    ...
    <div className="ops-content-wide ops-stack">
      ...
      <OpsSurface eyebrow="Stage 4B intake" title="Scan → identify → link → preserve" description="Upload one source document at a time so its provenance stays explicit. General records may remain unlinked; operational, customer, partner and finance papers should link to the matching KCPL record whenever possible.">
        <form onSubmit={upload} className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(320px,.7fr)]">
          ...
          <div className="rounded-[var(--app-radius)] border border-[#e4dcd5] bg-[var(--admin-surface-muted)] p-4">
            <label className="block cursor-pointer rounded-[var(--app-radius)] border border-dashed border-[#d5c9c0] bg-[var(--admin-surface)] p-6 text-center transition hover:border-[#c79c89]">
              ...
              <span className="mx-auto grid h-10 w-10 place-items-center rounded-[var(--app-radius)] bg-[#fff5ef] text-[#b8644e]"><Upload size={17}/></span>
              <strong className="mt-3 block text-[11px] text-[var(--admin-ink)]">{file ? file.name : "Choose scanned paper file"}</strong>
              <span className="mt-1 block text-[length:var(--app-label-size)] leading-4 text-[#8c827a]">PDF, image, Word, Excel, CSV or TXT · up to 20 MB</span>
            </label>
            {file ? <div className="mt-3 rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-3 text-[length:var(--app-label-size)] text-[#756b64]"><strong className="block text-[var(--admin-ink)]">{bytes(file.size)}</strong><span>{file.type || "Type inferred from extension"}</span></div> : null}
            <div className="mt-4 rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-3 text-[length:var(--app-label-size)] leading-4 text-[#7e746d]"><strong className="block text-[length:var(--app-label-size)] text-[var(--admin-ink)]">Archive integrity</strong>KCPL stores a SHA-256 fingerprint with every file. Stage 4C cannot erase archived evidence. If its linked source record is reversed, the archive keeps the original link metadata and moves its live link to the migration batch.</div>
      ...
      <OpsSurface eyebrow="Archive register" title="Historical evidence" description={`${filtered.length} of ${records.length} archived file${records.length === 1 ? "" : "s"} shown.`} action={<label className="relative block min-w-[240px]"><Search size={12} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[#9b9088]"/><input aria-label="Search paper archive" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search archive..." className="w-full rounded-[var(--app-radius)] border border-[#e3dbd4] bg-white py-2 pl-8 pr-3 text-[length:var(--app-label-size)] outline-none focus:border-[#c69b89]"/></label>} flush>
        {filtered.length ? <div className="ops-table-wrap"><table className="ops-table min-w-[1160px]"><thead><tr><th>Archive ID</th><th>Document</th><th>Linked record</th><th>Branch / source</th><th>Integrity</th><th>Archived by</th><th></th></tr></thead><tbody>{filtered.map((record) => {
          ...
          return <tr key={record.id}><td><OpsMono>{record.id}</OpsMono><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{dateTime(record.uploaded_at)} NPT</p>{record.recovery_id ? <p className="mt-1"><OpsBadge tone="warning">Recovery preserved</OpsBadge></p> : null}</td><td><strong className="text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{record.title}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{archiveCategoryLabels[record.category]} · {dateOnly(record.document_date)} · {bytes(record.size_bytes)}</p><p className="mt-1 max-w-[280px] truncate text-[length:var(--app-label-size)] text-[#a0968e]">{record.filename}</p></td><td>{record.entity_reference ? <>{href ? <Link href={href} className="font-bold text-[#b5654f] hover:underline">{record.entity_label || record.entity_reference}</Link> : <strong>{record.entity_label || record.entity_reference}</strong>}<p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{archiveEntityTypeLabels[record.entity_type]} · <OpsMono>{record.entity_reference}</OpsMono></p>{record.recovery_original_entity_reference ? <div className="mt-2 rounded-[var(--app-radius)] border border-[#eadfd8] bg-[#fff8f4] px-2 py-1.5 text-[length:var(--app-label-size)] leading-4 text-[#7d6f66]"><strong>Original pre-recovery link:</strong> {record.recovery_original_entity_label || record.recovery_original_entity_reference}{record.recovery_original_entity_type ? ` · ${archiveEntityTypeLabels[record.recovery_original_entity_type]}` : ""}<br/><OpsMono>{record.recovery_original_entity_reference}</OpsMono>{record.recovery_id ? <> · Recovery <OpsMono>{record.recovery_id}</OpsMono></> : null}</div> : null}</> : <OpsBadge tone="neutral">General archive</OpsBadge>}</td><td><strong className="text-[length:var(--app-label-size)] text-[#5b524b]">{record.branch}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{record.physical_reference || "No physical reference"}</p>{record.recovery_relinked_at ? <p className="mt-1 text-[length:var(--app-label-size)] text-[#9b8174]">Re-linked {dateTime(record.recovery_relinked_at)} NPT</p> : null}</td><td><OpsBadge tone="success"><ShieldCheck size={9}/>SHA-256</OpsBadge><p title={record.sha256} className="mt-1 max-w-[170px] truncate font-mono text-[7px] text-[#9d938b]">{record.sha256 || "Fingerprint unavailable"}</p></td><td><strong className="text-[length:var(--app-label-size)] text-[#5b524b]">{record.uploaded_by_name}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{record.uploaded_by_email}</p>{record.recovery_relinked_by_name ? <p className="mt-1 text-[length:var(--app-label-size)] text-[#9b8174]">Recovery: {record.recovery_relinked_by_name}</p> : null}</td><td><a href={`/api/admin/migration/archive/${encodeURIComponent(record.id)}/download`} className="ops-button" data-variant="secondary" data-size="sm"><Download size={11}/>Download</a></td></tr>;
