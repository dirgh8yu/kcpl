      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div>
          <label className="block rounded-[var(--app-radius)] border border-dashed border-[var(--admin-line)] bg-[var(--admin-surface-soft)] p-6 text-center transition hover:border-[var(--admin-accent-line)] hover:bg-[var(--admin-accent-bg)]">
            ...
            <span className="mx-auto grid h-10 w-10 place-items-center rounded-[var(--app-radius)] bg-white text-[var(--admin-crimson)] shadow-[0_5px_18px_rgba(80,55,40,.06)]"><Landmark size={17}/></span>
        ...
        <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-4">
          <p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em] text-[var(--admin-faint)]">Stage 3A rules</p>
...
function PreviewPanel({ preview, confirmed, busy, onConfirmed, onImport }: { preview: ReceivablesImportPreview; confirmed: boolean; busy: "preview" | "import" | ""; onConfirmed: (value: boolean) => void; onImport: () => void }) {
  return <OpsSurface eyebrow="Stage 3A safe preview" title="Reconcile every receivable before writing" description={`${preview.total} rows detected in ${preview.filename}. Only rows marked Ready will be created.`} flush>
    ...
    <div className="border-t border-[var(--admin-line)] bg-[var(--admin-surface)] p-4 sm:p-5">
      {preview.ready ? <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><label className="flex max-w-2xl cursor-pointer items-start gap-3 rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-3"><input type="checkbox" checked={confirmed} onChange={(event) => onConfirmed(event.target.checked)} className="mt-0.5"/><span className="text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]"><strong className="block text-[length:var(--app-label-size)] text-[var(--admin-ink)]">I reviewed and reconciled this finance preview.</strong>Import the {preview.ready} Ready receivable{preview.ready === 1 ? "" : "s"} only. Duplicate and invalid rows will remain untouched. Imported balances immediately affect KCPL Receivables and Customer 360.</span></label><OpsButton variant="primary" disabled={!confirmed || Boolean(busy)} onClick={onImport}>{busy === "import" ? <LoaderCircle size={12} className="animate-spin"/> : <Upload size={12}/>}Import {preview.ready} ready receivable{preview.ready === 1 ? "" : "s"}</OpsButton></div> : <OpsEmptyState compact icon={<AlertTriangle size={16}/>} title="Nothing is ready to import" description="Correct the invalid rows or review possible duplicates in the paper ledger, then preview the file again."/>}
    </div>
  </OpsSurface>;
}
