...
import { AlertTriangle, CheckCircle2, Download, FileSpreadsheet, LoaderCircle, Upload, UsersRound } from "lucide-react";
...
export function MigrationWorkspace() {
  ...
  return <OpsPage>
    <OpsPageHeader
      ...
      description="Move KCPL records into the operating system in controlled stages. Stage 1 is intentionally limited to customer master data so the import path can be proven before shipments and finance are touched."
      meta={<><span>Management only</span><span>Stage 1 of 4</span><span>No writes before preview</span></>}
      actions={<><Link href="/admin/crm" className="ops-button" data-variant="secondary" data-size="md">Customers</Link><a href="/api/admin/migration/customers" className="ops-button" data-variant="primary" data-size="md" download><Download size={13}/>Customer CSV template</a></>}
    />

    <OpsStatStrip>
      <OpsStat label="Stage 1" value="Customers" detail="Active now" icon={<UsersRound size={13}/>} tone="success"/>
      <OpsStat label="Stage 2" value="Shipments" detail="Not built yet" icon={<FileSpreadsheet size={13}/>} />
      <OpsStat label="Stage 3" value="Finance" detail="Not built yet" />
      <OpsStat label="Stage 4" value="Archive" detail="Historical jobs + rollback" />
    </OpsStatStrip>

    <div className="ops-content-wide ops-stack">
      <OpsSurface eyebrow="Stage plan" title="One migration layer at a time" description="This first release does not import shipments, receivables, payables or scanned job archives. Those stay isolated until customer migration is stable.">
        <div className="grid gap-2 md:grid-cols-4">
          <Stage number="01" title="Customer master" detail="CSV preview, validation, duplicate detection and confirmed import." active/>
          <Stage number="02" title="Shipment history" detail="Existing and historical movement records, after Stage 1 is proven."/>
      ...
      <OpsSurface eyebrow="Customer master import" title="CSV intake → validate → preview → confirm" description="Stage 1 uses a fixed template on purpose. This keeps the first migration path predictable before flexible spreadsheet mapping is introduced." action={<a href="/api/admin/migration/customers" className="ops-button" data-variant="secondary" data-size="sm" download><Download size={12}/>Download template</a>}>
...
function PreviewPanel({ preview, confirmed, busy, onConfirmed, onImport }: { preview: CustomerImportPreview; confirmed: boolean; busy: "preview" | "import" | ""; onConfirmed: (value: boolean) => void; onImport: () => void }) {
  return <OpsSurface eyebrow="Safe preview" title="Review before anything is written" description={`${preview.total} rows detected in ${preview.filename}. Only rows marked Ready can be created.`} flush>
    ...
    <div className="border-t border-[#e8e1db] bg-[#fffdfa] p-4 sm:p-5">
      {preview.ready ? <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><label className="flex max-w-2xl cursor-pointer items-start gap-3 rounded-[11px] border border-[#e7dfd8] bg-[#faf8f5] p-3"><input type="checkbox" checked={confirmed} onChange={(event) => onConfirmed(event.target.checked)} className="mt-0.5"/><span className="text-[9px] leading-4 text-[#6f665f]"><strong className="block text-[10px] text-[#514840]">I reviewed this preview.</strong>Import the {preview.ready} Ready row{preview.ready === 1 ? "" : "s"} only. Possible duplicates and invalid rows will remain untouched.</span></label><OpsButton variant="primary" disabled={!confirmed || Boolean(busy)} onClick={onImport}>{busy === "import" ? <LoaderCircle size={12} className="animate-spin"/> : <Upload size={12}/>}Import {preview.ready} ready customer{preview.ready === 1 ? "" : "s"}</OpsButton></div> : <OpsEmptyState compact icon={<AlertTriangle size={16}/>} title="Nothing is ready to import" description="Correct the invalid rows or review possible duplicates in the source CSV, then preview the file again."/>}
    </div>
  </OpsSurface>;
}
...
function Stage({ number, title, detail, active = false }: { number: string; title: string; detail: string; active?: boolean }) {
  return <div className={`rounded-[12px] border p-4 ${active ? "border-[#d7c2b8] bg-[#fff7f2]" : "border-[#e7dfd8] bg-[#faf8f5]"}`}><div className="flex items-center justify-between gap-2"><span className="text-[9px] font-black tracking-[.08em] text-[#b96650]">{number}</span>{active ? <OpsBadge tone="success">Active</OpsBadge> : <OpsBadge>Later</OpsBadge>}</div><strong className="mt-3 block text-[11px] text-[#514840]">{title}</strong><p className="mt-1.5 text-[8px] leading-4 text-[#8c827a]">{detail}</p></div>;
}
