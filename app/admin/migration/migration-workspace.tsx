        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div>
            <label className="block rounded-[var(--app-radius)] border border-dashed border-[#d8cec6] bg-[#fbf8f5] p-6 text-center transition hover:border-[#caa797] hover:bg-[#fffaf7]">
              ...
              <span className="mx-auto grid h-10 w-10 place-items-center rounded-[var(--app-radius)] bg-white text-[#bd654f] shadow-[0_5px_18px_rgba(80,55,40,.06)]"><Upload size={17}/></span>
          ...
          <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-4">
            <p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em] text-[#9a8e85]">Stage 1 rules</p>
...
function PreviewPanel({ preview, confirmed, busy, onConfirmed, onImport }: { preview: CustomerImportPreview; confirmed: boolean; busy: "preview" | "import" | ""; onConfirmed: (value: boolean) => void; onImport: () => void }) {
  return <OpsSurface eyebrow="Stage 1 safe preview" title="Review customers before anything is written" description={`${preview.total} rows detected in ${preview.filename}. Only rows marked Ready can be created.`} flush>
    <div className="grid grid-cols-2 gap-px border-b border-[var(--admin-line)] bg-[#e8e1db] sm:grid-cols-4">
    ...
    <div className="ops-table-wrap"><table className="ops-table min-w-[900px]"><thead><tr><th>Row</th><th>Customer</th><th>Branch</th><th>Contact</th><th>Status</th><th>Validation</th></tr></thead><tbody>{preview.rows.map((row) => <tr key={row.row_number}><td><OpsMono>{String(row.row_number)}</OpsMono></td><td><strong className="text-[11px] text-[#493f39]">{row.display_name}</strong>{row.tax_id ? <p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Tax ID {row.tax_id}</p> : null}</td><td>{row.primary_branch ?? <span className="text-[#a64d4f]">Invalid branch</span>}</td><td><span className="block text-[length:var(--app-label-size)]">{row.primary_email || "No email"}</span><span className="mt-1 block text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{row.primary_phone || "No phone"}</span></td><td><OpsBadge tone={statusTone(row.status)}>{statusLabel(row.status)}</OpsBadge></td><td><div className="max-w-[360px] text-[length:var(--app-label-size)] leading-4 text-[#7d736b]">{row.issues.length ? row.issues.join(" ") : row.duplicate_matches.length ? row.duplicate_matches.join(" · ") : <span className="inline-flex items-center gap-1 text-[#617765]"><CheckCircle2 size={10}/>No blocking issues</span>}</div></td></tr>)}</tbody></table></div>

    <div className="border-t border-[var(--admin-line)] bg-[var(--admin-surface)] p-4 sm:p-5">
      {preview.ready ? <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><label className="flex max-w-2xl cursor-pointer items-start gap-3 rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-3"><input type="checkbox" checked={confirmed} onChange={(event) => onConfirmed(event.target.checked)} className="mt-0.5"/><span className="text-[length:var(--app-label-size)] leading-4 text-[#6f665f]"><strong className="block text-[length:var(--app-label-size)] text-[var(--admin-ink)]">I reviewed this Stage 1 preview.</strong>Import the {preview.ready} Ready row{preview.ready === 1 ? "" : "s"} only. Possible duplicates and invalid rows will remain untouched.</span></label><OpsButton variant="primary" disabled={!confirmed || Boolean(busy)} onClick={onImport}>{busy === "import" ? <LoaderCircle size={12} className="animate-spin"/> : <Upload size={12}/>}Import {preview.ready} ready customer{preview.ready === 1 ? "" : "s"}</OpsButton></div> : <OpsEmptyState compact icon={<AlertTriangle size={16}/>} title="Nothing is ready to import" description="Correct the invalid rows or review possible duplicates in the source CSV, then preview the file again."/>}
    </div>
  </OpsSurface>;
}
...
function Stage({ number, title, detail, state = "later" }: { number: string; title: string; detail: string; state?: "active" | "complete" | "later" }) {
  ...
  return <div className={`rounded-[var(--app-radius)] border p-4 ${highlighted ? "border-[#d7c2b8] bg-[#fff7f2]" : "border-[var(--admin-line)] bg-[var(--admin-surface-muted)]"}`}><div className="flex items-center justify-between gap-2"><span className="text-[length:var(--app-label-size)] font-semibold tracking-[.08em] text-[#b96650]">{number}</span><OpsBadge tone={state === "active" ? "info" : state === "complete" ? "success" : "neutral"}>{state === "active" ? "Active" : state === "complete" ? "Complete" : "Later"}</OpsBadge></div><strong className="mt-3 block text-[11px] text-[var(--admin-ink)]">{title}</strong><p className="mt-1.5 text-[length:var(--app-label-size)] leading-4 text-[#8c827a]">{detail}</p></div>;
}
