...
function Rule({ icon, title, detail }: { icon: React.ReactNode; title: string; detail: string }) {
  return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-4"><strong className="flex items-center gap-2 text-[length:var(--app-label-size)] text-[var(--admin-ink)]">{icon}{title}</strong><p className="mt-2 text-[length:var(--app-label-size)] leading-4 text-[#857b73]">{detail}</p></div>;
}
