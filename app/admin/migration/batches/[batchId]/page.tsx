  ...
  return <OperationsShell userName={access.user.displayName} canManageStaff={staff.permissions.canManageStaff} canManageFinance={staff.permissions.canManageFinance} isManagement>
    <OpsPage>
      ...
      <div className="ops-content-wide ops-stack">
        ...
        {batch.detail_metrics.length ? <OpsSurface eyebrow="Batch composition" title="What this batch created"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{batch.detail_metrics.map((item) => <div key={item.label} className="rounded-[var(--app-radius)] border border-[#e9e1db] bg-[var(--admin-surface-muted)] p-4"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.07em] text-[#998f87]">{item.label}</p><strong className="mt-1 block text-[18px] text-[var(--admin-ink)]">{item.value}</strong></div>)}</div></OpsSurface> : null}
...
function Card({ icon, label, value, detail }: { icon: React.ReactNode; label: string; value: string; detail: string }) {
  return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-4"><div className="flex items-center gap-2 text-[#b5654f]">{icon}<span className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em]">{label}</span></div><strong className="mt-2 block text-[12px] capitalize text-[var(--admin-ink)]">{value}</strong><p className="mt-1 text-[length:var(--app-label-size)] leading-4 text-[var(--admin-muted)]">{detail}</p></div>;
}

function Safety({ title, detail }: { title: string; detail: string }) {
  return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-4"><strong className="flex items-center gap-2 text-[length:var(--app-label-size)] text-[var(--admin-ink)]"><AlertTriangle size={12} className="text-[#b5654f]"/>{title}</strong><p className="mt-2 text-[length:var(--app-label-size)] leading-4 text-[#857b73]">{detail}</p></div>;
}

function Gate({ title, detail }: { title: string; detail: string }) {
  return <main className="grid min-h-screen place-items-center bg-[var(--admin-canvas)] p-6 text-[var(--admin-ink)]"><section className="w-full max-w-xl rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface)] p-8"><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.13em] text-[#bd644e]">KCPL Migration Control Centre</p><h1 className="mt-3 text-[25px] font-[730] tracking-[-.04em]">{title}</h1><p className="mt-3 text-[11px] leading-6 text-[var(--admin-muted)]">{detail}</p><Link href="/admin/command-centre" className="ops-button mt-6" data-variant="primary" data-size="md">Operations Home</Link></section></main>;
}
