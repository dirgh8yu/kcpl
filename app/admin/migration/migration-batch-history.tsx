  ...
  return <div className="ops-stack">
    ...
    >
      {!dashboard ? <div className="p-5"><OpsEmptyState icon={<History size={17}/>} title="Migration history unavailable" description="Firebase migration batch storage could not be loaded."/></div> : <>
        <div className="grid grid-cols-2 gap-px border-b border-[var(--admin-line)] bg-[#e8e1db] md:grid-cols-5">
        ...
        {dashboard.batches.length ? <div className="ops-table-wrap"><table className="ops-table min-w-[1180px]"><thead><tr><th>Batch</th><th>Stage</th><th>Source</th><th>Rows</th><th>Imported</th><th>Actor</th><th>Completed</th><th>Status</th></tr></thead><tbody>{dashboard.batches.map((batch) => <tr key={batch.id}>
          <td><Link href={`/admin/migration/batches/${encodeURIComponent(batch.id)}`} className="font-bold text-[#b5654f]"><OpsMono>{batch.id}</OpsMono></Link><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Created {dateTime(batch.created_at)}</p></td>
          ...
          <td><span className="text-[length:var(--app-label-size)] text-[#5d554f]">{batch.source_filename || "No source filename"}</span></td>
          ...
          <td><OpsBadge tone={tone(batch.status)} dot>{label(batch.status)}</OpsBadge>{batch.status === "interrupted" ? <p className="mt-1 flex items-center gap-1 text-[length:var(--app-label-size)] text-[#94632f]"><Clock3 size={9}/>Running for more than 30 minutes</p> : null}{batch.error ? <p className="mt-1 max-w-[220px] text-[length:var(--app-label-size)] leading-4 text-[#a74d50]">{batch.error}</p> : null}</td>
        ...
        {dashboard.partial_failure_batches || dashboard.interrupted_batches ? <div className="border-t border-[var(--admin-line)] bg-[#fff9f4] p-4"><div className="flex items-start gap-2 text-[length:var(--app-label-size)] leading-4 text-[#775f50]"><AlertTriangle size={13} className="mt-0.5 shrink-0 text-[#a96c3e]"/><span><strong>Recovery attention required.</strong> Stage 4A only surfaces the affected batches. Controlled rollback and recovery actions will be added in Stage 4C after the archive layer is established.</span></div></div> : null}
      </>}
    </OpsSurface>
  </div>;
}
...
function Metric({ label, value, tone = "neutral" }: { label: string; value: number; tone?: "neutral" | "success" | "warning" | "danger" }) {
  const className = tone === "success" ? "text-[#5f7864]" : tone === "warning" ? "text-[#94632f]" : tone === "danger" ? "text-[#a74d50]" : "text-[var(--admin-ink)]";
  return <div className="bg-[var(--admin-surface)] p-4"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.07em] text-[#998f87]">{label}</p><strong className={`mt-1 block text-[20px] font-[740] ${className}`}>{value}</strong></div>;
}
