...
export function InvoiceWorkspace({ invoice, roleLabel }: { invoice: FinanceInvoice; roleLabel: string }) {
  ...
  return <OpsPage>
    ...
    <div className="ops-content-wide ops-stack">
      ...
      <section className="invoice-sheet overflow-hidden rounded-[var(--app-radius)] border border-[#e4ddd6] bg-white shadow-[0_14px_40px_rgba(78,59,45,.035)]">
        <div className="border-b border-[#ece5df] p-7 sm:p-10"><div className="flex flex-wrap items-start justify-between gap-8"><div><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.14em] text-[#b8654f]">Kapileshwor Cargo Pvt. Ltd.</p><h2 className="mt-2 text-[36px] font-[740] tracking-[-.055em] text-[#342e2a]">Invoice</h2><p className="mt-3 text-[length:var(--app-label-size)] leading-5 text-[#887e76]">Pragatipath Finance Complex, 2nd Floor<br/>Mhepi Road, Sorakhutte, Kathmandu, Nepal</p></div><div className="min-w-[260px] text-[length:var(--app-label-size)]"><Info label="Invoice" value={invoice.reference} mono/><Info label="Issue date" value={dateLabel(invoice.issue_date)}/><Info label="Due date" value={dateLabel(invoice.due_date)}/><Info label="Status" value={financeInvoiceStatusLabels[invoice.status]}/></div></div></div>
        <div className="grid border-b border-[#ece5df] md:grid-cols-2"><div className="p-7 sm:p-10"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.11em] text-[var(--admin-muted)]">Bill to</p><h3 className="mt-2 text-[19px] font-[730] tracking-[-.025em] text-[#443b35]">{invoice.customer_name}</h3><p className="mt-2 text-[length:var(--app-label-size)] text-[#8f857d]">Customer <OpsMono>{invoice.customer_id}</OpsMono></p></div><div className="border-t border-[#ece5df] p-7 sm:p-10 md:border-l md:border-t-0"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.11em] text-[var(--admin-muted)]">Operational reference</p><p className="mt-2 text-[11px] font-bold text-[var(--admin-ink)]">{invoice.shipment_reference ? <OpsMono>{invoice.shipment_reference}</OpsMono> : "No shipment linked"}</p><p className="mt-2 text-[length:var(--app-label-size)] text-[#8f857d]">Quote {invoice.quote_reference ? <OpsMono>{invoice.quote_reference}</OpsMono> : "not linked"} · {invoice.branch}</p></div></div>
        <div className="p-7 sm:p-10"><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-[length:var(--app-label-size)]"><thead className="border-b border-[#dcd3cc] text-[length:var(--app-label-size)] font-bold uppercase tracking-[.1em] text-[var(--admin-muted)]"><tr><th className="py-3">Description</th><th className="py-3 text-right">Qty</th><th className="py-3 text-right">Unit price</th><th className="py-3 text-right">Tax</th><th className="py-3 text-right">Total</th></tr></thead><tbody>{invoice.line_items.map((line) => <tr key={line.id} className="border-b border-[var(--admin-line)]"><td className="py-4 pr-4 font-semibold text-[var(--admin-ink)]">{line.description}</td><td className="py-4 text-right text-[#71675f]">{line.quantity}</td><td className="py-4 text-right text-[#71675f]">{money(line.unit_price, invoice.currency)}</td><td className="py-4 text-right text-[#71675f]">{line.tax_rate}%</td><td className="py-4 text-right font-bold text-[var(--admin-ink)]">{money(line.total, invoice.currency)}</td></tr>)}</tbody></table></div>
