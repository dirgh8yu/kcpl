  ...
  return <OpsPage>
    ...
    <div className="ops-content-wide ops-stack">
      ...
      <OpsSurface eyebrow="Receivable register" title="Receivables ledger" description={`${filtered.length} of ${dashboard.invoices.length} receivable records shown.`} flush>
        ...
        <div className="ops-table-wrap"><table className="ops-table min-w-[1120px]"><thead><tr><th>Receivable</th><th>Customer</th><th>Job / branch</th><th>Date / due</th><th>Total / opening</th><th>Balance</th><th>Status</th></tr></thead><tbody>{filtered.length ? filtered.map((invoice) => <tr key={invoice.reference}><td><Link href={`/admin/finance/invoices/${encodeURIComponent(invoice.reference)}`} className="font-bold text-[#514840] hover:underline">{invoice.record_type === "opening_balance" ? "Opening balance" : invoice.external_invoice_number || invoice.reference}</Link><div className="mt-1 flex flex-wrap items-center gap-1.5"><OpsMono>{invoice.reference}</OpsMono>{invoice.record_type === "opening_balance" ? <OpsBadge tone="violet">Opening</OpsBadge> : invoice.migration_batch_id ? <OpsBadge tone="info">Imported invoice</OpsBadge> : null}</div>{invoice.migration_batch_id ? <p className="mt-1 text-[8px] text-[#9c928a]"><OpsMono>{invoice.migration_batch_id}</OpsMono></p> : null}</td><td><strong>{invoice.customer_name}</strong><p className="mt-1 text-[8px] text-[#9c928a]"><OpsMono>{invoice.customer_id}</OpsMono></p></td><td><span>{invoice.shipment_reference ? <OpsMono>{invoice.shipment_reference}</OpsMono> : "No shipment"}</span><p className="mt-1 flex items-center gap-1 text-[8px] text-[#9c928a]"><Landmark size={9}/>{invoice.branch}</p></td><td><span>{invoice.record_type === "opening_balance" ? `As at ${dateLabel(invoice.migration_as_of_date || invoice.issue_date)}` : dateLabel(invoice.issue_date)}</span><p className={`mt-1 text-[8px] ${invoice.status === "overdue" ? "font-bold text-[#b65355]" : "text-[#9c928a]"}`}>Due {dateLabel(invoice.due_date)}</p></td><td className="font-bold text-[#514840]">{money(invoice.total, invoice.currency)}</td><td className={`font-bold ${invoice.balance_due > 0 ? "text-[#514840]" : "text-[#66806b]"}`}>{money(invoice.balance_due, invoice.currency)}</td><td><OpsBadge tone={statusTone(invoice.status)} dot>{financeInvoiceStatusLabels[invoice.status]}</OpsBadge></td></tr>) : <tr><td colSpan={7}><OpsEmptyState kind="search" title="No receivables match" description="Change the filters or create a new invoice draft."/></td></tr>}</tbody></table></div>
      </OpsSurface>
    </div>
  </OpsPage>;
}
