...
export function FinanceWorkspace({ dashboard, roleLabel }: { dashboard: FinanceDashboard; roleLabel: string }) {
  ...
  return <OpsPage>
    ...
    <OpsStatStrip>
      ...
      <OpsStat label="Opening balances" value={dashboard.opening_balance_count} icon={<Landmark size={13}/>} tone={dashboard.opening_balance_count ? "warning" : "neutral"}/>
    ...
    <div className="ops-content-wide ops-stack">
      ...
      <OpsSurface eyebrow="Receivable register" title="Receivables ledger" description={`${filtered.length} of ${dashboard.invoices.length} receivable records shown.`} flush>
        <div className="ops-toolbar"><OpsSearch value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search invoice, opening balance, customer, shipment or branch"/><select className="ops-select" value={status} onChange={(event) => setStatus(event.target.value as "all" | FinanceInvoiceStatus)}><option value="all">All statuses</option>{financeInvoiceStatuses.map((item) => <option key={item} value={item}>{financeInvoiceStatusLabels[item]}</option>)}</select><OpsButton variant="ghost" size="sm" onClick={() => { setQuery(""); setStatus("all"); }}>Reset</OpsButton></div>
        <div className="ops-table-wrap"><table className="ops-table min-w-[1120px]"><thead><tr><th>Receivable</th><th>Customer</th><th>Job / branch</th><th>Date / due</th><th>Total / opening</th><th>Balance</th><th>Status</th></tr></thead><tbody>{filtered.length ? filtered.map((invoice) => <tr key={invoice.reference}><td><Link href={`/admin/finance/invoices/${encodeURIComponent(invoice.reference)}`} className="font-bold text-[var(--admin-ink)] hover:underline">{invoice.record_type === "opening_balance" ? "Opening balance" : invoice.external_invoice_number || invoice.reference}</Link><div className="mt-1 flex flex-wrap items-center gap-1.5"><OpsMono>{invoice.reference}</OpsMono>{invoice.record_type === "opening_balance" ? <OpsBadge tone="violet">Opening</OpsBadge> : invoice.migration_batch_id ? <OpsBadge tone="info">Imported invoice</OpsBadge> : null}</div>{invoice.migration_batch_id ? <p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]"><OpsMono>{invoice.migration_batch_id}</OpsMono></p> : null}</td><td><strong>{invoice.customer_name}</strong><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]"><OpsMono>{invoice.customer_id}</OpsMono></p></td><td><span>{invoice.shipment_reference ? <OpsMono>{invoice.shipment_reference}</OpsMono> : "No shipment"}</span><p className="mt-1 flex items-center gap-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]"><Landmark size={9}/>{invoice.branch}</p></td><td><span>{invoice.record_type === "opening_balance" ? `As at ${dateLabel(invoice.migration_as_of_date || invoice.issue_date)}` : dateLabel(invoice.issue_date)}</span><p className={`mt-1 text-[length:var(--app-label-size)] ${invoice.status === "overdue" ? "font-bold text-[#b65355]" : "text-[var(--admin-muted)]"}`}>Due {dateLabel(invoice.due_date)}</p></td><td className="font-bold text-[var(--admin-ink)]">{money(invoice.total, invoice.currency)}</td><td className={`font-bold ${invoice.balance_due > 0 ? "text-[var(--admin-ink)]" : "text-[#66806b]"}`}>{money(invoice.balance_due, invoice.currency)}</td><td><OpsBadge tone={statusTone(invoice.status)} dot>{financeInvoiceStatusLabels[invoice.status]}</OpsBadge></td></tr>) : <tr><td colSpan={7}><OpsEmptyState kind="search" title="No receivables match" description="Change the filters or create a new invoice draft."/></td></tr>}</tbody></table></div>
      </OpsSurface>
    </div>
  </OpsPage>;
}

function Mini({ label, value, tone = "neutral" }: { label: string; value: string; tone?: "neutral" | "success" | "danger" }) { return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[var(--admin-surface-muted)] p-3"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.07em] text-[var(--admin-muted)]">{label}</p><strong className={`mt-1.5 block text-[length:var(--app-label-size)] ${tone === "success" ? "text-[#66806b]" : tone === "danger" ? "text-[#b65355]" : "text-[var(--admin-ink)]"}`}>{value}</strong></div>; }
function Age({ label, value, total, currency, danger = false }: { label: string; value: number; total: number; currency: string; danger?: boolean }) { return <div><div className="flex items-center justify-between gap-2"><span className="text-[length:var(--app-label-size)] font-bold text-[#8e847c]">{label}</span><span className={`text-[length:var(--app-label-size)] font-semibold ${danger ? "text-[#b65355]" : "text-[#8e847c]"}`}>{money(value, currency)}</span></div><div className="mt-2"><OpsProgress value={value} max={Math.max(total, 1)} tone={danger ? "danger" : value > 0 ? "warning" : "accent"}/></div></div>; }
