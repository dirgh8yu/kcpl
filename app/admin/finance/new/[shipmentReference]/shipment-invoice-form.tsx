...
export function ShipmentInvoiceForm({
  ...
  return <main className="min-h-screen bg-[var(--admin-canvas)] text-[var(--admin-ink)]">
    <header className="bg-[var(--admin-crimson)] px-5 py-6 text-white lg:px-8"><div className="mx-auto flex max-w-[1300px] flex-wrap items-start justify-between gap-5"><div className="flex items-start gap-4"><Link href={`/admin/jobs/${encodeURIComponent(shipmentReference)}`} className="grid h-10 w-10 place-items-center rounded-[var(--app-radius)] border border-white/15 text-white/65 hover:bg-white/10"><ArrowLeft size={16}/></Link><div><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.22em] text-[#d4ad62]">KCPL Finance</p><h1 className="mt-1 text-3xl font-semibold tracking-[-.045em]">Create shipment invoice</h1><p className="mt-2 text-xs text-white/45">{shipmentReference}</p></div></div><Link href="/admin/finance" className="rounded-[var(--app-radius)] border border-white/15 px-4 py-2.5 text-xs font-semibold">Accounts Receivable</Link></div></header>
    ...
    <div className="mx-auto max-w-[1300px] p-5 lg:p-8">
      ...
      <section className={`rounded-[var(--app-radius)] border border-black/10 bg-white p-6 shadow-sm sm:p-8 ${customerId ? "" : "opacity-60"}`}><div className="mb-6 flex items-start gap-3"><span className="rounded-[var(--app-radius)] bg-[var(--admin-crimson)] p-2.5 text-white"><FilePlus2 size={17}/></span><div><p className="text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.15em] text-[#b78a3e]">New receivable</p><h2 className="mt-1 text-2xl font-semibold">Invoice draft</h2><p className="mt-1 text-xs text-black/45">{customerId ? "Customer confirmed. Complete the commercial details below." : "Invoice creation unlocks after the CRM customer is confirmed or created above."}</p></div></div>
        <form onSubmit={submit} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="md:col-span-2 xl:col-span-4"><button disabled={busy || !customerId} className="rounded-[var(--app-radius)] bg-[var(--admin-crimson)] px-5 py-3 text-sm font-semibold text-white disabled:opacity-40">{busy ? "Creating…" : "Create invoice draft"}</button></div>
        </form>
      </section>
    </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label><span className="mb-1.5 block text-[length:var(--app-label-size)] font-semibold uppercase tracking-[.12em] text-black/40">{label}</span>{children}</label>;
}
