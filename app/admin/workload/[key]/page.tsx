          ...
          <div className="ops-grid-main">
            <OpsSurface eyebrow="Ownership" title="Assigned shipments" description={`${assignedJobs.length} active movement${assignedJobs.length === 1 ? "" : "s"} currently owned by ${targetName}.`} flush>
              {assignedJobs.length ? <div className="ops-table-wrap overflow-x-auto"><table className="ops-table min-w-[980px] w-full"><thead><tr><th>Route</th><th>Shipment</th><th>Status</th><th>Branch</th><th>ETA</th><th>Tasks</th><th>Customs</th><th></th></tr></thead><tbody>{assignedJobs.map((job) => <tr key={job.reference}>
                ...
                <td className={job.overdue_tasks ? "font-bold text-[#ae434a]" : ""}>{job.open_tasks}{job.overdue_tasks ? <span className="ml-1 text-[length:var(--app-label-size)]">({job.overdue_tasks} overdue)</span> : null}</td>
...
function Gate({ title, detail }: { title: string; detail: string }) {
  return <main className="grid min-h-screen place-items-center bg-[#f3f1ee] p-6 text-[#26221f]"><section className="w-full max-w-xl rounded-[var(--app-radius)] border border-[#ddd8d2] bg-white p-8 shadow-[0_12px_36px_rgba(54,43,34,.06)]"><span className="grid h-10 w-10 place-items-center rounded-[var(--app-radius)] bg-[#fbebe6] text-[#b45c47]"><UserRound size={17}/></span><p className="mt-5 text-[length:var(--app-label-size)] font-bold text-[#8f8179]">KCPL Staff workload</p><h1 className="mt-2 text-[28px] font-[730] leading-tight tracking-[-.04em] text-[#26221f]">{title}</h1><p className="mt-3 text-[13px] leading-6 text-[#736d67]">{detail}</p><div className="mt-6 flex flex-wrap gap-2"><Link href="/admin/command-centre" className="ops-button" data-variant="primary" data-size="md">Operations home</Link><Link href="/admin/shipments" className="ops-button" data-variant="secondary" data-size="md">Shipments</Link></div></section></main>;
}
