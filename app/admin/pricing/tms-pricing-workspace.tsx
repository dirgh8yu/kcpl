    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
      <OpsSurface eyebrow="Transport orders" title="Buy costs ready for sell pricing" description="Only non-master orders with a selected procurement cost appear here.">
        <div className="grid max-h-[720px] gap-2 overflow-auto">
          {orders.length ? orders.map((order) => <button key={order.id} type="button" onClick={() => loadDefaults(order)} className={`rounded-[var(--app-radius)] border p-3 text-left ${selectedOrderId === order.id ? "border-[#dca99d] bg-[#fff8f5]" : "border-[var(--admin-line)] bg-white hover:border-[#d9cec6]"}`}>
            ...
            <span className="mt-1 block text-[length:var(--app-label-size)] font-bold text-[#62584f]">Buy {money(order.buy_cost, order.buy_currency)}</span>
      ...
      {selectedOrder ? <div className="grid gap-4">
        <OpsSurface eyebrow="Sell calculation" title={`${selectedOrder.origin} → ${selectedOrder.destination}`} description="Pricing uses the most specific active rule. Customer markup is the fallback, followed by KCPL default controls.">
          ...
          {customer?.pricing_notes ? <p className="mt-3 rounded-[var(--app-radius)] bg-[#faf7f3] p-3 text-[length:var(--app-label-size)] leading-5 text-[#766b63]">Customer pricing note: {customer.pricing_notes}</p> : null}
        ...
        {preview ? <OpsSurface eyebrow="Price result" title={money(preview.result.sell_price, preview.input.sell_currency)} description="This snapshot freezes the cost, FX, rule and margin decision used for approval and quote release.">
          ...
          {preview.result.approval_required ? <div className="mt-4 rounded-[var(--app-radius)] border border-[#ead7b6] bg-[#fffaf0] p-3 text-[length:var(--app-label-size)] text-[#795e32]"><div className="flex items-center gap-2 font-bold"><CircleAlert size={13}/> Management approval required</div><ul className="mt-2 list-disc space-y-1 pl-5">{preview.result.approval_reasons.map((reason) => <li key={reason}>{reason}</li>)}</ul>{preview.approval_status === "approved" ? <p className="mt-2 font-bold text-[#55705c]">Approved by Management.</p> : canApprove ? <div className="mt-3"><OpsButton size="sm" onClick={approve} disabled={busy}><ShieldCheck size={12}/> Approve this snapshot</OpsButton></div> : <p className="mt-2">Quote release is locked until Management approves this exact snapshot.</p>}</div> : <div className="mt-4 rounded-[var(--app-radius)] border border-[#d9e5dc] bg-[#f6faf7] p-3 text-[length:var(--app-label-size)] text-[#56675a]"><strong>Margin controls passed.</strong> This snapshot is cleared for quote release.</div>}
...
function Mini({ label, value }: { label: string; value: string }) {
  return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-3"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em] text-[#958a82]">{label}</p><strong className="mt-1 block text-[12px] text-[var(--admin-ink)]">{value}</strong></div>;
}
