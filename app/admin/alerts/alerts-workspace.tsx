...
export function AlertsWorkspace({ initialAlerts, roleLabel }: { initialAlerts: AutomationAlert[]; roleLabel: string }) {
  ...
  return <OpsPage><main className="min-h-[calc(100vh-64px)] bg-[#F6F6F3] text-[#101010]"><div className="mx-auto w-full max-w-[1320px] px-4 pb-16 pt-8 sm:px-6 lg:px-8">
    <header className="grid gap-6 border-b border-[#101010] pb-7 md:grid-cols-[minmax(0,1fr)_auto] md:items-end"><div><p className="text-[10px] uppercase tracking-[0.11em] text-[#DC143C]">Operations · Attention desk</p><h1 className="mt-3 text-[clamp(36px,4vw,52px)] font-normal leading-[1.04] tracking-[-0.04em]">Tasks & Alerts</h1><p className="mt-3 max-w-2xl text-[14px] leading-6 text-[#5B5B57]">One exception ledger for overdue work, shipment risk, customs blockers and finance escalation. Acknowledge means reviewed; resolve only after the underlying condition is handled.</p><p className="mt-3 text-[10px] uppercase tracking-[0.05em] text-[#8A8A84]">{roleLabel} · Nepal time</p></div><div className="flex flex-wrap gap-2"><Link href="/admin/command-centre" className="inline-flex h-10 items-center border border-[#A5A5A0] px-3 text-[12px] hover:border-[#101010] hover:bg-[#EEEEE8]">Overview</Link><button type="button" onClick={() => action("evaluate")} disabled={evaluating} className="inline-flex h-10 items-center gap-2 border border-[#DC143C] bg-[#DC143C] px-4 text-[12px] font-medium text-white hover:border-[#B61032] hover:bg-[#B61032] disabled:opacity-50"><RefreshCw size={13} className={evaluating ? "animate-spin" : ""}/>{evaluating ? "Checking…" : "Check now"}</button></div></header>
    ...
    <section className="mt-6 border-y border-[#D6D6D0]">
      <div className="grid gap-3 border-b border-[#101010] py-4 md:grid-cols-[minmax(260px,1fr)_180px_180px_auto]"><label className="flex h-10 items-center border border-[#BDBDB6] bg-white px-3"><SearchIcon/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search shipment, owner, branch, exception…" className="min-w-0 flex-1 bg-transparent text-[12px] outline-none placeholder:text-[#8A8A84]"/></label><select className="ops-select" value={severity} onChange={(event) => setSeverity(event.target.value as "all" | AutomationAlertSeverity)}><option value="all">All severities</option><option value="critical">Critical</option><option value="warning">Warning</option><option value="info">Info</option></select><select className="ops-select" value={status} onChange={(event) => setStatus(event.target.value as StatusFilter)}><option value="active">Active alerts</option><option value="open">Open</option><option value="acknowledged">Acknowledged</option><option value="resolved">Resolved</option><option value="all">All history</option></select><button type="button" onClick={reset} className="h-10 border border-[#A5A5A0] px-3 text-[12px] hover:border-[#101010] hover:bg-[#EEEEE8]">Reset</button></div>
      <div className="hidden min-h-11 grid-cols-[92px_170px_minmax(260px,1fr)_190px_180px_auto] items-center gap-4 border-b border-[#101010] px-3 text-[10px] uppercase tracking-[0.06em] text-[#5B5B57] lg:grid"><span>Severity</span><span>Type / state</span><span>Issue</span><span>Ownership</span><span>Timing</span><span className="text-right">Actions</span></div>
      {visible.length ? <div>{visible.map((alert) => { const busy = busyId === alert.id; const resolved = alert.status === "resolved"; return <article key={alert.id} className={`relative grid gap-4 border-b border-[#D6D6D0] px-3 py-4 last:border-b-0 lg:grid-cols-[92px_170px_minmax(260px,1fr)_190px_180px_auto] lg:items-start ${resolved ? "opacity-65" : "hover:bg-[#EEEEE8]"}`}>{!resolved && alert.severity === "critical" ? <span className="absolute bottom-2 left-0 top-2 w-[2px] bg-[#DC143C]"/> : null}<div><OpsBadge tone={severityTone(alert.severity)} dot>{alert.severity}</OpsBadge>{alert.escalated_at ? <div className="mt-2"><OpsBadge tone="danger">Escalated</OpsBadge></div> : null}</div><div><p className="text-[10px] font-medium uppercase tracking-[0.04em] text-[#5B5B57]">{automationAlertTypeLabels[alert.type]}</p><div className="mt-2"><OpsBadge tone={statusTone(alert.status)}>{alert.status}</OpsBadge></div></div><div className="min-w-0"><h2 className="text-[13px] font-medium tracking-[-0.015em]">{alert.title}</h2><p className="mt-1 text-[11px] leading-5 text-[#666660]">{alert.detail}</p><p className="mt-2 text-[9px] text-[#8A8A84]">Record <OpsMono>{alert.entity_id}</OpsMono>{alert.parent_reference ? <> · Parent <OpsMono>{alert.parent_reference}</OpsMono></> : null}</p></div><div className="text-[10px] leading-5 text-[#666660]"><p>{alert.branch || "Branch not attached"}</p><p>{alert.assigned_to_name || alert.assigned_to_email || "No assigned owner"}</p></div><div className="text-[9px] leading-4 text-[#777771]"><p>First {dateTime(alert.first_triggered_at)}</p><p className="mt-1">Last {dateTime(alert.last_triggered_at)}</p>{alert.acknowledged_at ? <p className="mt-1">Ack {dateTime(alert.acknowledged_at)}</p> : null}{alert.resolved_at ? <p className="mt-1">Resolved {dateTime(alert.resolved_at)}</p> : null}</div><div className="flex flex-wrap gap-2 lg:justify-end"><Link href={alert.action_path} className="ops-button" data-variant={resolved ? "secondary" : "primary"} data-size="sm">Open record</Link>{alert.status === "open" ? <OpsButton variant="secondary" size="sm" disabled={busy} onClick={() => action("acknowledge", alert.id)}>{busy ? "Working…" : "Acknowledge"}</OpsButton> : null}{!resolved ? <OpsButton variant="ghost" size="sm" disabled={busy} onClick={() => action("resolve", alert.id)}><CheckCircle2 size={12}/>{busy ? "Working…" : "Resolve"}</OpsButton> : null}</div></article>; })}</div> : <div className="py-12">{emptyState}</div>}
    </section>

    <div className="mt-5 border-l-2 border-[#DC143C] bg-[#EEEEE8] px-4 py-3 text-[11px] leading-5 text-[#5B5B57]"><div className="flex items-start gap-2"><ShieldAlert size={14} className="mt-0.5 shrink-0 text-[#DC143C]"/><p><strong className="font-medium text-[#101010]">Control rule:</strong> acknowledging an alert records review but does not transfer ownership. Resolving closes the current alert; if the condition persists, the next automation evaluation can reopen it.</p></div></div>
  </div></main></OpsPage>;
}

function SearchIcon() { return <span className="mr-2 text-[#777771]"><AlertTriangle size={13} aria-hidden="true"/></span>; }
