    ...
    <OpsPage>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(280px,.8fr)_minmax(0,1.7fr)]">
        <OpsSurface eyebrow="Order queue" title="Transport orders" description="Rate an order whenever cargo facts or supplier pricing changes.">
          <div className="space-y-2">{orders.length ? orders.map((order) => <button key={order.id} type="button" onClick={() => { setSelectedOrderId(order.id); setResults([]); }} className={`w-full rounded-[var(--app-radius)] border p-3 text-left transition ${selectedOrderId === order.id ? "border-[#df8a75] bg-[#fff8f4]" : "border-[var(--admin-line)] bg-white hover:border-[#d8cec5]"}`}>
            ...
            <p className="mt-2 text-[length:var(--app-label-size)] text-[#847a72]">{modeLabel(order.mode)} · {order.weight_kg.toLocaleString()} kg · {order.volume_cbm.toLocaleString()} CBM · pickup {dateLabel(order.pickup_date)}</p>
            {order.selected_cost !== null && order.selected_currency ? <p className="mt-2 text-[length:var(--app-label-size)] font-bold text-[#55705b]">Selected {money(order.selected_cost, order.selected_currency)}</p> : null}
