    ...
    <OpsPage>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(280px,.8fr)_minmax(0,1.7fr)]">
        <OpsSurface eyebrow="Order queue" title="Transport orders" description="Rate an order whenever cargo facts or supplier pricing changes.">
          <div className="space-y-2">{orders.length ? orders.map((order) => <button key={order.id} type="button" onClick={() => { setSelectedOrderId(order.id); setResults([]); }} className={`w-full rounded-[var(--app-radius)] border p-3 text-left transition ${selectedOrderId === order.id ? "border-[var(--admin-accent-line)] bg-[var(--admin-accent-bg)]" : "border-[var(--admin-line)] bg-white hover:border-[var(--admin-line-strong)]"}`}>
            <div className="flex items-start justify-between gap-2"><div><OpsMono>{order.id}</OpsMono><p className="mt-1 text-[12px] font-bold text-[var(--admin-ink)]">{order.origin} <ArrowRight className="inline" size={12}/> {order.destination}</p></div><OpsBadge tone={order.status === "selected" ? "success" : order.status === "rated" ? "info" : "neutral"}>{order.status}</OpsBadge></div>
            <p className="mt-2 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{modeLabel(order.mode)} · {order.weight_kg.toLocaleString()} kg · {order.volume_cbm.toLocaleString()} CBM · pickup {dateLabel(order.pickup_date)}</p>
            {order.selected_cost !== null && order.selected_currency ? <p className="mt-2 text-[length:var(--app-label-size)] font-bold text-[var(--admin-success)]">Selected {money(order.selected_cost, order.selected_currency)}</p> : null}
        ...
        <OpsSurface eyebrow="Rate comparison" title={selectedOrder ? `${selectedOrder.origin} → ${selectedOrder.destination}` : "Select an order"} description={selectedOrder ? `${modeLabel(selectedOrder.mode)} · ${selectedOrder.weight_kg.toLocaleString()} kg · ${selectedOrder.volume_cbm.toLocaleString()} CBM. Results are grouped by currency without hidden FX conversion.` : "Choose a transport order from the queue."} action={selectedOrder ? <OpsButton size="sm" variant="primary" onClick={() => rateSelectedOrder()} disabled={busy}><Calculator size={13}/> Rate order</OpsButton> : undefined}>
          {!selectedOrder ? <OpsEmptyState compact title="No order selected" description="Choose an order to compare Partner procurement rates."/> : results.length ? <div className="ops-scroll-x overflow-x-auto"><table className="w-full min-w-[760px] text-left text-[length:var(--app-label-size)]"><thead><tr className="border-b border-[var(--admin-line)] text-[var(--admin-muted)]"><th className="px-3 py-2">Partner</th><th className="px-3 py-2">Service</th><th className="px-3 py-2">Basis</th><th className="px-3 py-2">Linehaul</th><th className="px-3 py-2">Fuel</th><th className="px-3 py-2">Accessorials</th><th className="px-3 py-2">Total</th><th className="px-3 py-2">Transit</th><th className="px-3 py-2"></th></tr></thead><tbody>{results.map((result) => <tr key={result.rate_card_id} className="border-b border-[var(--admin-line)] last:border-0"><td className="px-3 py-3"><strong className="block text-[11px] text-[var(--admin-ink)]">{result.partner_name}</strong><OpsMono>{result.partner_id}</OpsMono></td><td className="px-3 py-3">{result.service || modeLabel(result.mode)}{result.equipment ? <span className="block text-[var(--admin-muted)]">{result.equipment}</span> : null}</td><td className="px-3 py-3">{tmsRateUnitLabels[result.unit]} × {result.quantity.toLocaleString(undefined, { maximumFractionDigits: 3 })}{result.minimum_applied ? <span className="block text-[var(--admin-warning)]">minimum applied</span> : null}</td><td className="px-3 py-3">{money(result.linehaul, result.currency)}</td><td className="px-3 py-3">{money(result.fuel_surcharge, result.currency)}</td><td className="px-3 py-3">{money(result.accessorials, result.currency)}</td><td className="px-3 py-3 text-[12px] font-semibold text-[var(--admin-ink)]">{money(result.total_cost, result.currency)}</td><td className="px-3 py-3">{result.transit_days_min === null ? "Not set" : result.transit_days_max && result.transit_days_max !== result.transit_days_min ? `${result.transit_days_min}–${result.transit_days_max} days` : `${result.transit_days_min} days`}</td><td className="px-3 py-3"><OpsButton size="sm" onClick={() => selectRate(result)} disabled={busy || selectedOrder.selected_rate_card_id === result.rate_card_id}>{selectedOrder.selected_rate_card_id === result.rate_card_id ? "Selected" : "Select"}</OpsButton></td></tr>)}</tbody></table></div> : <OpsEmptyState compact icon={<Calculator size={18}/>} title="Rate this order" description={`KCPL will compare it against ${activeRateCards} active Partner buy rate${activeRateCards === 1 ? "" : "s"}, respecting lane, mode, equipment, validity, minimum charge and surcharges.`} action={<OpsButton size="sm" variant="primary" onClick={() => rateSelectedOrder()} disabled={busy}>Find rates</OpsButton>}/>} 
        </OpsSurface>
      </div>
    </OpsPage>
  );
