    ...
    <div className={`kcpl-ops min-h-screen bg-[var(--ops-bg)] text-[var(--ops-text)] transition-[padding] duration-200 ${collapsed ? "lg:pl-[72px]" : "lg:pl-[244px]"}`}>
      <aside className={`ops-sidebar fixed inset-y-0 left-0 z-50 hidden flex-col text-white transition-[width] duration-200 lg:flex ${collapsed ? "w-[72px]" : "w-[244px]"}`}>
        <div className={`flex h-14 items-center border-b border-white/[.07] ${collapsed ? "justify-center px-2" : "justify-between px-3"}`}>
          ...
          {!collapsed ? <button type="button" onClick={() => setCollapsed(true)} className="grid h-7 w-7 place-items-center rounded-md text-white/35 transition hover:bg-white/[.06] hover:text-white/75" aria-label="Collapse sidebar"><PanelLeftClose size={14}/></button> : null}
        </div>

        {collapsed ? <button type="button" onClick={() => setCollapsed(false)} className="mx-auto mt-2 grid h-8 w-8 place-items-center rounded-md text-white/35 transition hover:bg-white/[.06] hover:text-white/75" aria-label="Expand sidebar"><PanelLeftOpen size={14}/></button> : null}
        ...
        <nav className={`flex-1 overflow-y-auto pb-4 ${collapsed ? "px-2 pt-2" : "px-2.5 pt-3"}`} aria-label="KCPL Operations navigation">
          {groups.map((group, groupIndex) => <div key={group.label} className={groupIndex ? "mt-4" : ""}>
            {!collapsed ? <p className="px-2 pb-1.5 text-[9px] font-semibold uppercase tracking-[.11em] text-white/27">{group.label}</p> : groupIndex ? <div className="mx-2 mb-2 border-t border-white/[.06]"/> : null}
        ...
        <div className={`border-t border-white/[.07] ${collapsed ? "p-2" : "p-2.5"}`}>
          <div className={`rounded-lg ${collapsed ? "flex justify-center py-2" : "bg-white/[.035] p-2.5"}`}>
            <div className={`flex items-center ${collapsed ? "justify-center" : "gap-2.5"}`}>
              <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full border border-white/10 bg-white/[.07] text-[10px] font-semibold text-white/80">{initial}</span>
              {!collapsed ? <div className="min-w-0 flex-1"><p className="truncate text-[11px] font-medium text-white/78">{userName}</p><p className="mt-0.5 truncate text-[9px] text-white/30">{resolvedRole}</p></div> : null}
            </div>
            {!collapsed ? <a href={signOutPath} className="mt-2 flex h-7 items-center gap-2 rounded-md px-1.5 text-[10px] text-white/35 transition hover:bg-white/[.05] hover:text-white/70"><LogOut size={12}/>Sign out</a> : null}
          </div>
        </div>
      </aside>

      <header className="ops-topbar sticky top-0 z-40 flex h-12 items-center gap-3 border-b border-[#e5e7ea] bg-white/95 px-3 backdrop-blur-xl sm:px-4 lg:px-5">
        <button type="button" onClick={() => setMobileOpen(true)} className="grid h-8 w-8 shrink-0 place-items-center rounded-md text-[#626a73] hover:bg-[#f2f3f4] lg:hidden" aria-label="Open navigation"><Menu size={16}/></button>
        <div className="flex min-w-0 flex-1 items-center gap-1.5 overflow-hidden text-[11px] text-[#8a9198]">
          {breadcrumbs.map((item, index) => <span key={`${item}-${index}`} className="flex min-w-0 items-center gap-1.5">{index ? <ChevronRight size={11} className="shrink-0 text-[#c0c4c8]"/> : null}<span className={`truncate ${index === breadcrumbs.length - 1 ? "font-medium text-[#444a51]" : ""}`}>{item}</span></span>)}
        </div>
        <button type="button" onClick={() => setPaletteOpen(true)} className="hidden h-8 min-w-[210px] items-center gap-2 rounded-lg border border-[#e1e4e7] bg-[#fafafa] px-2.5 text-left text-[11px] text-[#8a9198] transition hover:border-[#d3d7dc] hover:bg-white md:flex">
        ...
        <button type="button" onClick={() => setPaletteOpen(true)} className="grid h-8 w-8 place-items-center rounded-md text-[#68717a] hover:bg-[#f2f3f4] md:hidden" aria-label="Search workspaces"><Command size={15}/></button>
        <Link href="/admin/alerts" className="grid h-8 w-8 place-items-center rounded-md text-[#68717a] transition hover:bg-[#f2f3f4] hover:text-[#30363d]" aria-label="Open alerts" title="Alerts"><BellRing size={15}/></Link>
      ...
      {mobileOpen ? <div className="fixed inset-0 z-[80] lg:hidden">
        <button type="button" className="absolute inset-0 bg-[#0d1117]/35 backdrop-blur-[1px]" onClick={() => setMobileOpen(false)} aria-label="Close navigation overlay"/>
        <aside className="ops-sidebar absolute inset-y-0 left-0 flex w-[284px] flex-col text-white shadow-2xl">
          <div className="flex h-14 items-center justify-between border-b border-white/[.07] px-3.5"><Link href="/admin/command-centre" onClick={() => setMobileOpen(false)} className="flex items-center gap-2.5"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[#eef0ff] text-[11px] font-black text-[#3445a3]">K</span><span><strong className="block text-[12px] font-semibold">KCPL Operations</strong><span className="text-[9px] text-white/35">OPERATING SYSTEM</span></span></Link><button type="button" onClick={() => setMobileOpen(false)} className="grid h-8 w-8 place-items-center rounded-md text-white/50 hover:bg-white/[.06] hover:text-white" aria-label="Close navigation"><X size={15}/></button></div>
          <nav className="flex-1 overflow-y-auto px-2.5 py-3">{groups.map((group, groupIndex) => <div key={group.label} className={groupIndex ? "mt-4" : ""}><p className="px-2 pb-1.5 text-[9px] font-semibold uppercase tracking-[.11em] text-white/27">{group.label}</p><div className="space-y-0.5">{group.items.map((item) => { const active = item.match(pathname); const Icon = item.icon; return <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={`flex h-10 items-center gap-2.5 rounded-lg px-2.5 text-[12px] font-medium ${active ? "bg-white/[.09] text-white" : "text-white/55 hover:bg-white/[.055] hover:text-white/85"}`}><Icon size={16}/>{item.label}</Link>; })}</div></div>)}</nav>
          <div className="border-t border-white/[.07] p-3"><div className="flex items-center gap-2.5"><span className="grid h-8 w-8 place-items-center rounded-full bg-white/[.08] text-[10px] font-semibold">{initial}</span><div className="min-w-0 flex-1"><p className="truncate text-[11px] text-white/80">{userName}</p><p className="text-[9px] text-white/35">{resolvedRole}</p></div><a href={signOutPath} className="grid h-8 w-8 place-items-center rounded-md text-white/40 hover:bg-white/[.05] hover:text-white" aria-label="Sign out"><LogOut size={14}/></a></div></div>
        </aside>
      </div> : null}
