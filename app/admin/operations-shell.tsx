...
type StaffRole = "management" | "accounts" | "commercial" | "operations";
type NavigationScope = { allBranches: boolean; branches: string[] };
type MacroNavItem = {
  label: string;
  href: string;
  visible?: boolean;
...
function breadcrumbFor(pathname: string, activeLabel?: string) {
  ...
  if (pathname.startsWith("/admin/alerts")) return "Control Tower / Tasks & Alerts";
...
export function OperationsShell({
  ...
  const [role, setRole] = useState<StaffRole | null>(isManagement ? "management" : null);
  const [scope, setScope] = useState<NavigationScope>({ allBranches: isManagement, branches: [] });

  useEffect(() => {
    let cancelled = false;
    fetch("/api/admin/navigation", { cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<{ role?: StaffRole; capabilities?: NavigationCapabilities; scope?: NavigationScope }> : null)
      .then((data) => {
  ...
  const homeHref = capabilities.canManageJobFile ? "/admin/command-centre" : "/admin/notifications";
  ...
  const roleLabel = role ? roleLabels[role] : isManagement ? "Management" : "KCPL staff";
  const scopePrimary = scope.allBranches
    ? "All branches"
    : scope.branches.length === 1
      ? scope.branches[0]
      : scope.branches.length > 1
        ? `${scope.branches.length} assigned branches`
        : "Assigned branches";
  const scopeDetail = scope.allBranches
    ? `${roleLabel} · Global scope`
    : scope.branches.length
      ? `${roleLabel} · ${scope.branches.join(" · ")}`
      : `${roleLabel} · Role scope`;

  const macroNav = useMemo<MacroNavItem[]>(() => [
    { label: "Home", href: "/admin/command-centre", visible: capabilities.canManageJobFile, active: (path) => path.startsWith("/admin/command-centre") },
    { label: "Operations", href: "/admin/shipments", visible: capabilities.canManageJobFile, active: (path) => ["/admin/shipments", "/admin/jobs/", "/admin/pickups", "/admin/freight-documents", "/admin/visibility", "/admin/customs", "/admin/documents", "/admin/delivery"].some((prefix) => path.startsWith(prefix)) },
    ...
    { label: "Control Tower", href: "/admin/alerts", visible: capabilities.canManageJobFile, active: (path) => path.startsWith("/admin/alerts") },
  ], [capabilities]);
  ...
  return (
    <div className="min-h-screen bg-[#f6f6f3] text-[#141414] [font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe_UI',sans-serif]">
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-[232px] flex-col bg-black px-[18px] pb-4 pt-5 lg:flex">
        <Link href={homeHref} className="flex h-7 items-center text-[15px] font-semibold leading-[22px] text-white" aria-label="KCPL workspace home">
          <span>KCPL</span><span className="ml-auto h-1.5 w-1.5 rounded-full" style={{ backgroundColor: V4_RED }}/>
        </Link>
        <div className="flex h-[62px] flex-col justify-center gap-[2px]" title={scope.branches.length ? scope.branches.join(", ") : scopePrimary}>
          <p className="truncate text-[13px] font-medium leading-[19px] text-white">{scopePrimary}</p>
          <p className="truncate text-[11px] font-medium leading-[15px] text-[#737373]">{scopeDetail}</p>
        </div>
        ...
        <nav className="space-y-0.5" aria-label="KCPL workspaces">
          {macroNav.filter((item) => item.visible !== false).map((item) => {
            ...
            return <Link key={item.label} href={item.href} aria-current={active ? "page" : undefined} className={`relative flex h-[34px] items-center rounded-[6px] px-[10px] text-[13px] font-medium leading-[19px] transition ${active ? "bg-[#242424] text-white" : "text-[#737373] hover:bg-[#171717] hover:text-white"}`}>
              {active ? <span className="mr-2 h-4 w-0.5 rounded-[2px]" style={{ backgroundColor: V4_RED }}/> : null}{item.label}
            </Link>;
          })}
        </nav>
        <div className="mt-auto">
          ...
          <Link href={systemHref} aria-current={systemActive ? "page" : undefined} className={`relative flex h-[34px] items-center rounded-[6px] px-[10px] text-[13px] font-medium transition ${systemActive ? "bg-[#242424] text-white" : "text-[#737373] hover:bg-[#171717] hover:text-white"}`}>
            {systemActive ? <span className="mr-2 h-4 w-0.5 rounded-[2px]" style={{ backgroundColor: V4_RED }}/> : null}System
          </Link>
          <div className="flex min-h-[54px] items-end border-t border-[#171717] pt-3">
            <div className="min-w-0 flex-1"><p className="truncate text-[13px] font-medium leading-[19px] text-white">{userName}</p><p className="text-[11px] font-medium leading-[15px] text-[#737373]">{roleLabel}</p></div>
      ...
      <header className="fixed inset-x-0 top-0 z-40 h-[54px] border-b border-[#e2e2e2] bg-[#f6f6f3] lg:left-[232px]">
        <div className="flex h-full items-center px-4 lg:px-7">
          ...
          <Link href="/admin/notifications" className="ml-[14px] grid h-7 w-7 place-items-center text-[#5b5b5b] hover:text-[#141414]" aria-label="Open notifications"><Bell size={14}/></Link>
      ...
      {mobileOpen ? <div className="fixed inset-x-0 bottom-0 top-[54px] z-50 overflow-y-auto bg-black px-4 py-5 lg:hidden">
        <div className="mx-auto max-w-xl">
          ...
          <div className="mb-4 border-b border-[#242424] pb-4"><p className="text-[13px] font-medium text-white">{scopePrimary}</p><p className="mt-1 text-[11px] text-[#737373]">{scopeDetail}</p></div>
          ...
          <nav className="space-y-1">
            {macroNav.filter((item) => item.visible !== false).map((item) => {
              const active = item.active(pathname);
              return <Link key={item.label} href={item.href} onClick={() => setMobileOpen(false)} className={`flex min-h-11 items-center rounded-[7px] px-3 text-[13px] font-medium ${active ? "bg-[#242424] text-white" : "text-[#8a8a8a]"}`}>
                {active ? <span className="mr-2 h-4 w-0.5 rounded" style={{ backgroundColor: V4_RED }}/> : null}{item.label}
              </Link>;
            })}
            <Link href={systemHref} onClick={() => setMobileOpen(false)} className={`flex min-h-11 items-center rounded-[7px] px-3 text-[13px] font-medium ${systemActive ? "bg-[#242424] text-white" : "text-[#8a8a8a]"}`}>
              {systemActive ? <span className="mr-2 h-4 w-0.5 rounded" style={{ backgroundColor: V4_RED }}/> : null}System
            </Link>
          </nav>
          <div className="mt-6 border-t border-[#242424] pt-4"><p className="text-[13px] font-medium text-white">{userName}</p><p className="mt-1 text-[11px] text-[#737373]">{roleLabel}</p><a href={signOutPath} className="mt-4 inline-flex items-center gap-2 text-[12px] font-medium text-[#b5b5b5]"><LogOut size={13}/>Sign out</a></div>
        </div>
      </div> : null}
