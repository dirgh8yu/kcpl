...
export function OperationsShell({
  ...
  useEffect(() => {
    ...
    const next = [current, ...safeStoredWorkspaces("kcpl-ops-recents").filter((item) => item.href !== current.href)].slice(0, 6);
  ...
  const favoriteItems = favorites.map((favorite) => workspaces.find((item) => item.href === favorite.href)).filter((item): item is WorkflowWorkspace => Boolean(item));
  const recentItems = recents.map((recent) => workspaces.find((item) => item.href === recent.href)).filter((item): item is WorkflowWorkspace => Boolean(item)).slice(0, 4);
    ...
    <div className="kcpl-ops min-h-screen">
      <aside className={`fixed inset-y-0 left-0 z-50 hidden flex-col border-r border-[#dcd7d1] bg-[#f1efec]/96 backdrop-blur-xl transition-[width] duration-200 lg:flex ${collapsed ? "w-[68px]" : "w-[244px]"}`}>
        ...
        <div className={`px-2.5 pt-2 ${collapsed ? "flex justify-center" : ""}`}>
          <button type="button" onClick={() => setPaletteOpen(true)} className={`flex h-9 items-center rounded-[9px] border border-[#d9d4ce] bg-white text-[#716a64] shadow-[0_1px_2px_rgba(45,35,29,.03)] hover:border-[#c7c0b9] ${collapsed ? "w-9 justify-center" : "w-full gap-2.5 px-2.5"}`} aria-label="Open command palette">
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto px-2.5 py-3">
          {!collapsed && favoriteItems.length ? <SidebarSection label="Pinned">{favoriteItems.map((item) => <NavLink key={`favorite-${item.href}`} item={item} active={activeItem?.id === item.id} collapsed={false}/>)}</SidebarSection> : null}
          {groups.map((group, groupIndex) => <div key={group.group} className={groupIndex || favoriteItems.length ? "mt-4" : ""}>
            {!collapsed ? <p className="mb-1 px-2 text-[9px] font-bold uppercase tracking-[.08em] text-[#928b84]">{group.group}</p> : groupIndex ? <div className="mx-2 mb-3 border-t border-[#ded9d4]"/> : null}
            <div className="space-y-0.5">{group.items.map((item) => <NavLink key={item.href} item={item} active={activeItem?.id === item.id} collapsed={collapsed}/>)}</div>
          </div>)}
      ...
      <header className={`fixed inset-x-0 top-0 z-40 h-[58px] border-b border-[#ded9d4] bg-[#f8f7f5]/94 backdrop-blur-xl transition-[left] duration-200 ${collapsed ? "lg:left-[68px]" : "lg:left-[244px]"}`}>
        <div className="flex h-full items-center gap-2.5 px-3 sm:px-4 lg:px-5">
          ...
          <div className="min-w-0 flex-1"><div className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8e8780]"><span>KCPL</span>{activeItem ? <><ChevronRight size={10}/><span>{activeItem.group}</span><ChevronRight size={10}/><span className="truncate text-[#4f4944]">{activeItem.label}</span></> : <><ChevronRight size={10}/><span className="truncate text-[#4f4944]">Operations</span></>}</div></div>
          ...
          <button type="button" onClick={() => setPaletteOpen(true)} className="hidden h-9 min-w-[280px] items-center gap-2 rounded-[9px] border border-[#d9d4ce] bg-white px-3 text-[#716a64] shadow-[0_1px_2px_rgba(45,35,29,.03)] hover:border-[#c7c0b9] md:flex" aria-label="Open command palette"><Search size={13}/><span className="flex-1 text-left text-[11px] font-semibold">Search KCPL…</span><span className="text-[9px] font-bold text-[#918a83]">⌘K</span></button>
          ...
          <div className="relative">
            ...
            {createOpen ? <div className="absolute right-0 top-11 w-72 overflow-hidden rounded-[12px] border border-[#ddd7d1] bg-white p-1.5 shadow-[0_22px_62px_rgba(54,43,34,.16)]">
              ...
              {capabilities.canManageJobFile ? <MenuLink href="/admin/delivery" icon={<PackageCheck size={14}/>} label="Delivery / POD" detail="Work final-mile attempts and proof" close={() => setCreateOpen(false)}/> : null}
              {capabilities.canManageFinance ? <MenuLink href="/admin/payables?create=1" icon={<HandCoins size={14}/>} label="New supplier bill" detail="Record payable, then Match-Pay it" close={() => setCreateOpen(false)}/> : null}
              {capabilities.canManageFinance ? <MenuLink href="/admin/finance/new" icon={<ReceiptText size={14}/>} label="New receivable" detail="Create a customer invoice" close={() => setCreateOpen(false)}/> : null}
            </div> : null}
          </div>
        </div>
      </header>
