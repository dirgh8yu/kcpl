    ...
    {eligible.length ? <div className="grid gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <div className="grid content-start gap-2">
        {eligible.map((load) => {
          ...
          return <button key={load.id} type="button" onClick={() => setSelectedLoadId(load.id)} className={`rounded-[var(--app-radius)] border p-3 text-left ${selectedLoadId === load.id ? "border-[#dca99d] bg-[#fff8f5]" : "border-[var(--admin-line)] bg-white"}`}>
      ...
      {selectedLoad ? <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[#fcfbf9] p-4">
        ...
        {!allocation ? <div className="mt-4 rounded-[var(--app-radius)] border border-[#eadfd7] bg-white p-3 text-[length:var(--app-label-size)] leading-5 text-[#776e67]">After the master tender is accepted or countered, prepare the commercial allocation here. Booking will fail closed until this exact package exists.</div> : <>
          ...
          </div> : <div className="mt-4 rounded-[var(--app-radius)] border border-[#d9e5dc] bg-[#f6faf7] p-3 text-[length:var(--app-label-size)] text-[#56675a]">No new Management approval is required for the staged house economics.</div>}
          {allocation.status === "ready" ? <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-[var(--app-radius)] border border-[#d9e5dc] bg-[#f6faf7] p-3 text-[length:var(--app-label-size)] text-[#56675a]"><span><strong>Ready to book.</strong> Final booking will consume these same derived version IDs and fingerprints.</span><Link href="/admin/tenders" className="ops-button" data-size="sm" data-variant="primary">Tender Desk <ArrowRight size={11}/></Link></div> : null}
        </>}
      </div> : null}
...
function Mini({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 rounded-[var(--app-radius)] border border-[#ece5df] bg-white p-2"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em] text-[var(--admin-muted)]">{label}</p><p className="mt-1 truncate text-[length:var(--app-label-size)] font-semibold text-[var(--admin-ink)]" title={value}>{value}</p></div>;
}
