...
import {
  loadPreparedConsolidationAllocationForBookingInTransaction,
...
/**
 * Consolidation final booking consumes a previously staged allocation package.
 * It never derives or persists a new house commercial version.
 */
export async function confirmConsolidatedLoadBookingWithPreparedAllocation(input: ConsolidatedBookingInput, actor: Actor, staff: KcplStaffContext) {
  ...
  try {
    const result = await db.runTransaction(async (transaction) => {
      // READ PHASE begins. Every authority read finishes before any write below.
      ...
      if (text(load.get("status")) === "booked") {
        ...
        const houseOrders = await Promise.all(houseIds.map((id) => transaction.get(db.collection("transport_orders").doc(id))));
        if (houseOrders.some((doc) => !doc.exists)) return { kind: "state_conflict" as const };
        if (!await validateBookedConsolidationAllocationForRetryInTransaction(transaction, load, houseOrders)) return { kind: "state_conflict" as const };
        ...
        const [masterShipment, masterQuote, ...children] = await Promise.all([
          transaction.get(db.collection("shipments").doc(masterShipmentReference)), transaction.get(db.collection("quotes").doc(masterBridgeQuoteReference(load.id))),
      ...
      const houseOrders = await Promise.all(members.map((member) => transaction.get(db.collection("transport_orders").doc(member.order_id))));
      ...
      const allocationResult = await loadPreparedConsolidationAllocationForBookingInTransaction(transaction, { load, branch, masterOrder, tender, masterVersion, houseOrders });
      if (allocationResult.kind !== "ready") return { kind: allocationResult.kind };
      const prepared = allocationResult.prepared;
      const sourceVersions = prepared.sourceVersions;
      const bookedHouseVersions = prepared.derivedVersions;
      ...
      // WRITE PHASE begins. No transaction.get/query call appears below this point.
      ...
      for (let index = 0; index < houseOrders.length; index += 1) {
        ...
        transaction.set(quoteRef, {
          ...
          consolidation_allocation_package_id: prepared.package.package_id,
        ...
        transaction.create(db.collection("shipments").doc(reference), {
          ...
          transport_order_id: order.id, consolidation_load_id: load.id, consolidation_allocation_package_id: prepared.package.package_id,
        ...
        transaction.update(order.ref, {
          ...
          booked_allocation_package_id: prepared.package.package_id,
        ...
        commercialAllocations.push({ order_id: order.id, source_commercial_version_id: source.id, source_commercial_fingerprint: source.fingerprint, booked_commercial_version_id: version.id, booked_commercial_fingerprint: version.fingerprint, allocated_cost: allocation, currency: commercials.currency, allocation_package_id: prepared.package.package_id });
      }
      ...
      transaction.update(prepared.packageRef, {
        status: "booked", booked_at: now, booked_by_name: actor.name, booked_by_email: actor.email, booking_reference: bookingReference,
      });
      transaction.update(loadRef, {
        ...
        commercial_allocations: commercialAllocations, booked_allocation_package_id: prepared.package.package_id,
        commercial_allocation_status: "booked", booking_operation_id: operationId, updated_at: now,
      });
      transaction.create(loadRef.collection("events").doc(`booking-${masterShipmentReference}`), {
        ...commercialEventPayload(masterVersion, "load_booked_with_commercial_lineage", actor, `${masterShipmentReference} · ${houseShipmentReferences.length} house shipments · package ${prepared.package.package_id}`),
        allocation_package_id: prepared.package.package_id,
