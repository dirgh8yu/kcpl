    ...
    <OpsPage>
      ...
      {showCreate && canManage ? <OpsSurface className="mt-4" eyebrow="Consolidation planning" title="Create master load" description="Choose at least two compatible orders. Same-branch, mode, equipment, temperature and capacity rules are enforced server-side.">
        <form onSubmit={createLoad} className="grid gap-4">
          ...
          <div>
            <p className="mb-2 text-[length:var(--app-label-size)] font-bold uppercase tracking-[.09em] text-[#7a7069]">House orders</p>
            <div className="grid max-h-[280px] gap-2 overflow-auto rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-[#fcfbf9] p-3 md:grid-cols-2">
              {eligibleOrders.length ? eligibleOrders.map((order) => <div key={order.id} className="flex items-start gap-2 rounded-[var(--app-radius)] border border-[#ece5df] bg-white p-3 text-[length:var(--app-label-size)]"><input type="checkbox" aria-label={`Select ${order.id} for consolidation`} checked={selectedOrderIds.includes(order.id)} onChange={(event) => setSelectedOrderIds((current) => event.target.checked ? [...current, order.id] : current.filter((id) => id !== order.id))}/><span><OpsMono>{order.id}</OpsMono><strong className="mt-1 block text-[#4c433d]">{order.origin} → {order.destination}</strong><span className="mt-1 block text-[var(--admin-muted)]">{order.branch} · {modeLabel(order.mode)} · {order.weight_kg.toFixed(1)} kg · {order.volume_cbm.toFixed(3)} CBM{order.customer_name ? ` · ${order.customer_name}` : " · customer not linked"}</span></span></div>) : <OpsEmptyState title="No eligible orders" description="Create transport orders or resolve their current tender/booking state before consolidating."/>}
            </div>
          </div>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[330px_minmax(0,1fr)]">
        <OpsSurface eyebrow="Master loads" title="Consolidation register" description="Select a load to plan its house orders and stop sequence.">
          <div className="grid gap-2">
            {loads.length ? loads.map((load) => <button key={load.id} type="button" onClick={() => setSelectedLoadId(load.id)} className={`rounded-[var(--app-radius)] border p-3 text-left transition ${selectedLoadId === load.id ? "border-[#dca99d] bg-[#fff8f5]" : "border-[var(--admin-line)] bg-white hover:border-[#d9cec6]"}`}>
...
function Mini({ label, value, sub }: { label: string; value: string; sub: string }) {
  return <div className="rounded-[var(--app-radius)] border border-[var(--admin-line)] bg-white p-3"><p className="text-[length:var(--app-label-size)] font-bold uppercase tracking-[.08em] text-[#958a82]">{label}</p><strong className="mt-1 block text-[12px] text-[var(--admin-ink)]">{value}</strong><span className="mt-1 block text-[length:var(--app-label-size)] text-[#9a9089]">{sub}</span></div>;
}
