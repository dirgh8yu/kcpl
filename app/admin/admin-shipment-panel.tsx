...
function ShipmentWorkspace({
  ...
  return <article className="rounded-3xl border border-black/10 bg-white p-5 sm:p-7">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-3 py-1.5 text-[10px] font-black uppercase tracking-[.1em] ${shipmentStatusStyles[draft.status]}`}>{shipmentStatusLabels[draft.status]}</span><a href={`/admin/jobs/${encodeURIComponent(draft.reference)}`} className="flex items-center gap-1.5 rounded-xl bg-[#10263f] px-3 py-2 text-xs font-black text-white transition hover:bg-[#193653]">Digital Job File <ExternalLink size={13}/></a><a href={`/tracking?reference=${encodeURIComponent(draft.reference)}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 rounded-xl border border-black/10 px-3 py-2 text-xs font-black text-[#10263f] transition hover:bg-[#f8f7f2]">Public tracking <ExternalLink size={13}/></a></div>
    </div>
    ...
    <div className="mt-8 border-t border-black/10 pt-7">
      <div className="flex flex-wrap items-start justify-between gap-3"><div className="flex gap-3"><span className="rounded-xl bg-[#b78a3e]/10 p-2.5 text-[#b78a3e]"><FileText size={19}/></span><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-[#b78a3e]">Document vault</p><h4 className="mt-1 text-lg font-black">Private shipment files</h4><p className="mt-1 text-xs leading-5 text-black/45">Stored privately in Firebase Storage. These files never appear on public tracking.</p></div></div><div className="flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-[.1em] text-emerald-700"><ShieldCheck size={13}/> Admin only</div></div>

      {!documentStorageAvailable && <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-semibold text-amber-800">Firebase Storage is unavailable for this deployment. Existing metadata remains safe, but uploads and downloads are unavailable until storage access is restored.</div>}
