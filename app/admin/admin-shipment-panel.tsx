  ...
  async function persistShipment(overrideReason = "") {
    ...
    if (!response.ok) {
      if (response.status === 409 && data.canOverride) {
        const reason = window.prompt(`Workflow guard blocked this status change:\n\n${(data.blockers ?? [data.error ?? "Controlled milestone not ready."]).join("\n")}\n\nManagement override reason:`)?.trim() ?? "";
        if (reason.length >= 8) return persistShipment(reason);
        if (reason) throw new Error("Management override reasons must be at least 8 characters.");
      }
