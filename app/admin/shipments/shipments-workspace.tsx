...
export function ShipmentsWorkspace({ data, roleLabel }: { data: CommandCentreData; roleLabel: string }) {
  ...
  return <OpsPage>
    <OpsPageHeader eyebrow="Operations" title="Shipments" description="The live movement register. Scan route, milestone, ownership, ETA and open work, then enter the Digital Job File when the movement needs action." meta={<><span>{roleLabel}</span><span>Nepal operational date {dateOnly(data.operational_date)}</span><span>{data.accessible_branches.length} accessible branches</span></>} actions={<><Link href="/admin/command-centre" className="ops-button" data-variant="secondary" data-size="md">Operations home</Link><Link href="/admin/alerts" className="ops-button" data-variant="primary" data-size="md"><ShieldAlert size={13}/>Tasks & alerts</Link></>}/>
    ...
    {selected ? <><button type="button" className="ops-drawer-backdrop" aria-label="Close shipment preview" onClick={() => setSelectedReference(null)}/><aside className="ops-drawer" aria-label={`Shipment preview ${selected.reference}`}><div className="ops-drawer-header"><div className="min-w-0"><p className="ops-eyebrow">Shipment preview</p><h2 className="mt-1 truncate text-[20px] font-[730] tracking-[-.035em] text-[#342f2b]">{selected.origin || "Origin not set"} <ArrowRight size={14} className="inline text-[#c47a62]"/> {selected.destination || "Destination not set"}</h2><p className="mt-1 text-[11px] text-[#827970]">{selected.customer_name} · <OpsMono>{selected.reference}</OpsMono></p><p className="mt-1 text-[10px] text-[#978d84]">Updated {dateTime(selected.updated_at)}</p></div><button type="button" onClick={() => setSelectedReference(null)} className="grid h-8 w-8 place-items-center rounded-[9px] text-[#8f857d] hover:bg-[#f5efea]" aria-label="Close preview"><X size={14}/></button></div><div className="ops-drawer-body ops-stack"><div className="flex flex-wrap items-center gap-2"><OpsBadge tone={statusTone(selected.status)} dot>{shipmentStatusLabels[selected.status]}</OpsBadge><OpsBadge tone={attentionScore(selected, data.operational_date) > 0 ? "warning" : "success"}>{attentionScore(selected, data.operational_date) > 0 ? "Action required" : "Operationally clear"}</OpsBadge><OpsBadge tone={selected.priority === "urgent" || selected.priority === "high" ? "warning" : "neutral"}>{selected.priority} priority</OpsBadge></div><ShipmentMilestoneRail job={selected}/><div className="grid grid-cols-2 gap-x-5 gap-y-4 border-y border-[#e9e2dc] py-4"><PreviewFact icon={<MapPin size={13}/>} label="Current location" value={selected.current_location || "Not updated"}/><PreviewFact icon={<Landmark size={13}/>} label="Primary branch" value={selected.primary_branch}/><PreviewFact icon={<UserRound size={13}/>} label="Owner" value={selected.assigned_to_name || selected.assigned_to_email || "Unassigned"}/><PreviewFact icon={<CalendarDays size={13}/>} label="ETA" value={dateOnly(selected.eta)}/><PreviewFact label="Carrier" value={selected.carrier || "Not assigned"}/><PreviewFact label="Quote" value={selected.quote_reference || "Not linked"} mono/></div><div className="grid grid-cols-3 gap-2"><DrawerMetric label="Open tasks" value={selected.open_tasks} tone={selected.overdue_tasks ? "danger" : "neutral"}/><DrawerMetric label="Overdue" value={selected.overdue_tasks} tone={selected.overdue_tasks ? "danger" : "neutral"}/><DrawerMetric label="Customs open" value={selected.required_customs_open} tone={customsBlocking(selected, data.operational_date) ? "warning" : "neutral"}/></div>{!selected.assigned_to_name && !selected.assigned_to_email ? <div className="rounded-[10px] border border-[#ecd8b5] bg-[#fff9ee] p-3 text-[11px] text-[#74582d]"><strong className="block">No shipment owner</strong><span className="mt-1 block">Assign ownership in the Digital Job File before the movement becomes difficult to hand over.</span></div> : null}{selected.handling_branches.length ? <div><p className="text-[10px] font-semibold text-[#8a8179]">Handling branches</p><div className="mt-2 flex flex-wrap gap-1.5">{selected.handling_branches.map((item) => <OpsBadge key={item}>{item}</OpsBadge>)}</div></div> : null}<div className="flex flex-wrap gap-2 pt-1"><Link href={`/admin/jobs/${encodeURIComponent(selected.reference)}`} className="ops-button" data-variant="primary" data-size="md">Open Digital Job File <ArrowRight size={12}/></Link>{selected.customer_id ? <Link href={`/admin/crm/${encodeURIComponent(selected.customer_id)}`} className="ops-button" data-variant="secondary" data-size="md">Customer 360</Link> : null}</div></div></aside></> : null}
  </OpsPage>;
}
...
function PreviewFact({ icon, label, value, mono = false }: { icon?: React.ReactNode; label: string; value: string; mono?: boolean }) { return <div><p className="flex items-center gap-1.5 text-[10px] font-semibold text-[#8b827a]">{icon}{label}</p><p className="mt-1.5 break-words text-[12px] font-semibold text-[#514840]">{mono ? <OpsMono>{value}</OpsMono> : value}</p></div>; }
function DrawerMetric({ label, value, tone }: { label: string; value: number; tone: "neutral" | "warning" | "danger" }) { return <div className="border-l-2 border-[#e5ded8] pl-3"><p className="text-[10px] text-[#8d847c]">{label}</p><p className={`mt-1 text-[18px] font-[730] ${tone === "danger" ? "text-[#ad464d]" : tone === "warning" ? "text-[#94622d]" : "text-[#443b35]"}`}>{value}</p></div>; }
