...
import { AlertTriangle, CheckCircle2, Circle, Search, ShieldAlert, X } from "lucide-react";
...
export function CustomsWorkspace({ initialRows, customsAgents }: { initialRows: CustomsDeskRow[]; customsAgents: CustomsAgentOption[] }) {
  ...
  const visible = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const haystack = [row.reference, row.customer_name, row.origin, row.destination, row.mode, row.branch ?? "", row.clearance.entry_point ?? "", row.clearance.declaration_reference ?? "", row.clearance.agent_name ?? ""].join(" ").toLowerCase();
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, height: "var(--app-control-height)", padding: "0 12px", maxWidth: 300, flex: "1 1 260px", border: "1px solid var(--admin-line)", borderRadius: "var(--app-radius)", background: "var(--admin-surface)" }}>
              ...
              <input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search shipment, customer, border…" style={{ flex: 1, minWidth: 0, border: 0, outline: 0, background: "transparent", color: "var(--admin-ink)", font: "inherit", fontSize: 13.5 }}/>
          ...
          <div style={{ border: "1px solid var(--admin-line)", borderRadius: "var(--app-surface-radius)", overflow: "hidden", background: "var(--admin-surface)" }}>
            {visible.length ? <div style={{ overflowX: "auto" }}><table className="ops-table" style={{ minWidth: 980 }}><thead><tr><th>Shipment</th><th>Branch · Mode</th><th>Border point</th><th>Clearance</th><th>Desk state</th><th>Risk</th><th>Checklist</th><th>Owner</th></tr></thead><tbody>{visible.map((row) => {
            ...
            })}</tbody></table></div> : <OpsEmptyState kind="search" icon={<ShieldAlert size={18}/>} title={query || branch !== "all" ? "No results" : "No customs entries"} description={query || branch !== "all" ? "Try changing the filter." : "No shipments are currently in Customs clearance."}/>} 
          </div>
        </div>
