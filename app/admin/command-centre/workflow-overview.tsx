...
function Signal({ label, value, danger = false, warning = false }: { label: string; value: number; danger?: boolean; warning?: boolean }) {
  return <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[length:var(--app-label-size)] font-bold ${danger && value > 0 ? "border-[#edc8c4] bg-[#fff4f2] text-[#a6534d]" : warning && value > 0 ? "border-[#eadcc2] bg-[#fffaf0] text-[#8d6b38]" : "border-[#e5ded8] bg-white text-[#756c65]"}`}><strong>{value}</strong>{label}</span>;
}

function FlowLink({ href, children }: { href: string; children: React.ReactNode }) {
  return <Link href={href} className="rounded-[var(--app-radius)] border border-[#e6dfd9] bg-white px-2.5 py-1.5 text-[length:var(--app-label-size)] font-bold text-[#5e5650] hover:border-[#d5c8bf] hover:text-[#a85f4b]">{children}</Link>;
}
...
export function WorkflowOverviewStrip({ overview }: { overview: WorkflowOverview }) {
  ...
  return <section className="ops-content-wide pt-5">
    <div className="overflow-hidden rounded-[var(--app-radius)] border border-[#ded8d2] bg-[var(--admin-surface)] shadow-[0_8px_28px_rgba(54,43,34,.04)]">
      <div className="border-b border-[#e9e3dd] px-4 py-3.5 sm:px-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="ops-eyebrow">End-to-end workflow</p><h2 className="mt-1 text-[15px] font-[740] tracking-[-.02em] text-[#443d38]">One operating pipeline</h2><p className="mt-1 text-[length:var(--app-label-size)] leading-5 text-[#867c74]">Move work forward from enquiry to procurement, pickup, freight documents, execution, delivery and Match-Pay without hunting for the next workspace.</p></div><Link href="/admin/alerts" className="ops-button" data-variant="secondary" data-size="sm">Open blockers</Link></div>
