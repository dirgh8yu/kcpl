    ...
    <div className="overflow-hidden rounded-[15px] border border-[#ded8d2] bg-[#fffdfa] shadow-[0_8px_28px_rgba(54,43,34,.04)]">
      <div className="border-b border-[#e9e3dd] px-4 py-3.5 sm:px-5">
        ...
        <div className="mt-3 flex items-center gap-1.5 overflow-x-auto pb-1">
          ...
          <FlowLink href="/admin/tenders">Tender / book</FlowLink><ArrowRight size={10} className="shrink-0 text-[#aaa099]"/>
          <FlowLink href="/admin/pickups">Pickup</FlowLink><ArrowRight size={10} className="shrink-0 text-[#aaa099]"/>
          <FlowLink href="/admin/freight-documents">Documents</FlowLink><ArrowRight size={10} className="shrink-0 text-[#aaa099]"/>
      ...
      <div className="grid divide-y divide-[#ece6e0] md:grid-cols-2 xl:grid-cols-7 xl:divide-x xl:divide-y-0">
        ...
        {overview.pickup ? <Link href="/admin/pickups" className="group p-4 hover:bg-[#fcf9f6]"><div className="flex items-center gap-2 text-[#b45f4b]"><CalendarClock size={14}/><span className="text-[9px] font-bold uppercase tracking-[.08em]">Pickup</span></div><div className="mt-3 flex flex-wrap gap-1.5"><Signal label="unscheduled" value={overview.pickup.unscheduled} warning/><Signal label="missed" value={overview.pickup.missed} danger/><Signal label="confirmed" value={overview.pickup.confirmed}/></div><p className="mt-3 text-[9px] leading-4 text-[#8b8179]">Booked cargo waiting for collection windows, confirmation or recovery.</p></Link> : null}
        {overview.documents ? <Link href="/admin/freight-documents" className="group p-4 hover:bg-[#fcf9f6]"><div className="flex items-center gap-2 text-[#b45f4b]"><FileText size={14}/><span className="text-[9px] font-bold uppercase tracking-[.08em]">Documents</span></div><div className="mt-3 flex flex-wrap gap-1.5"><Signal label="carriage missing" value={overview.documents.missing_primary} danger/><Signal label="review" value={overview.documents.review_pending} warning/><Signal label="generated" value={overview.documents.generated_current}/></div><p className="mt-3 text-[9px] leading-4 text-[#8b8179]">Generate and review BL, AWB, consignment, manifest and execution documents.</p></Link> : null}
