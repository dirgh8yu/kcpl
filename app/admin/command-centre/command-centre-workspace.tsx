        ...
        <div className="relative z-10 mx-auto flex max-w-[1720px] flex-wrap items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              {customizeOpen ? <div id="home-section-controls" className="absolute right-0 top-11 z-30 w-64 rounded-[var(--app-radius)] border border-[#d9d3cd] bg-white p-2 shadow-[0_20px_60px_rgba(54,43,34,.15)]">
                <p className="px-2 pb-2 pt-1 text-[length:var(--app-label-size)] font-bold text-[var(--admin-muted)]">Home sections</p>
                {(Object.keys(defaultSections) as SectionKey[]).map((key) => <button key={key} type="button" onClick={() => toggleSection(key)} className="flex w-full items-center gap-3 rounded-[var(--app-radius)] px-2.5 py-2 text-left text-[11px] font-semibold capitalize text-[#504a45] hover:bg-[#f7f5f2]"><span className={`grid h-5 w-5 place-items-center rounded-[var(--app-radius)] border ${sections[key] ? "border-[#d99b88] bg-[#fbe9e3] text-[#ad5844]" : "border-[#dcd6d0] bg-white text-transparent"}`}><Check size={12}/></span>{key === "branch" ? "Branch pressure" : key === "staff" ? "Staff workload" : key === "recent" ? "Recent movement" : key}</button>)}
                <button type="button" onClick={() => { setSections(defaultSections); window.localStorage.setItem(`kcpl-ops-home-sections:${roleLabel.toLowerCase()}`, JSON.stringify(defaultSections)); }} className="mt-1 w-full rounded-[var(--app-radius)] px-2.5 py-2 text-left text-[length:var(--app-label-size)] font-semibold text-[#8b6257] hover:bg-[#f7f5f2]">Reset view</button>
              </div> : null}
            </div>
      ...
      <div className="ops-command-grid">
        ...
        {(sections.branch || sections.staff) ? <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(390px,.75fr)]">
          ...
          {sections.staff ? <HomePanel title="Ownership load" eyebrow="Staff workload">
            {staffWithWork.length ? <div className="overflow-x-auto"><table className="ops-table min-w-[500px] w-full"><thead><tr><th>Staff</th><th>Jobs</th><th>Open tasks</th><th>Overdue</th><th>Urgent</th></tr></thead><tbody>{staffWithWork.slice(0, 12).map((staff) => <tr key={staff.key}><td><Link href={`/admin/workload/${encodeURIComponent(staff.key)}`} aria-label={`Open workload for ${staff.name}`} className="group flex min-w-0 items-center justify-between gap-2 rounded-[var(--app-radius)] py-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#df7159]/30"><span className="min-w-0"><strong className="block max-w-[180px] truncate group-hover:text-[#9e5948]">{staff.name}</strong>{staff.email ? <span className="mt-0.5 block max-w-[180px] truncate text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{staff.email}</span> : null}</span><ArrowUpRight size={12} className="shrink-0 text-[#9b948d] group-hover:text-[#9e5948]"/></Link></td><td>{staff.active_jobs}</td><td>{staff.open_tasks}</td><td className={staff.overdue_tasks ? "font-bold text-[#ae434a]" : "text-[var(--admin-muted)]"}>{staff.overdue_tasks}</td><td className={staff.urgent_jobs ? "font-bold text-[#9b682b]" : "text-[var(--admin-muted)]"}>{staff.urgent_jobs}</td></tr>)}</tbody></table></div> : <OpsEmptyState compact kind="healthy" title="No staff pressure" description="No accessible active jobs or open tasks are currently assigned to staff."/>}
          </HomePanel> : null}
        </div> : null}

        {sections.recent ? <HomePanel title="Recently updated movements" eyebrow="What changed" action={<span className="text-[length:var(--app-label-size)] text-[var(--admin-muted)]">Snapshot {dateTimeNepal(data.generated_at)} NPT</span>}>
          {recent.length ? <div className="ops-activity-feed">{recent.map((job) => <Link key={job.reference} href={`/admin/jobs/${encodeURIComponent(job.reference)}`} className="ops-activity-item hover:bg-[#faf9f7]"><span className={`mt-1.5 h-2 w-2 rounded-full ${job.status === "exception" ? "bg-[#ae434a]" : "bg-[#3f7295]"}`}/><span className="min-w-0"><strong className="block truncate text-[12px] text-[var(--admin-ink)]">{job.customer_name} · {job.origin || "Origin"} → {job.destination || "Destination"}</strong><span className="mt-0.5 block truncate text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{shipmentStatusLabels[job.status]} · {job.current_location || job.primary_branch} · <OpsMono>{job.reference}</OpsMono></span></span><span className="whitespace-nowrap text-[length:var(--app-label-size)] text-[#8e8780]">{timeOnlyNepal(job.updated_at)} NPT</span></Link>)}</div> : <OpsEmptyState compact kind="neutral" title="No movement updates yet" description="Recent shipment changes will appear here as operational records are updated."/>}
        </HomePanel> : null}
      </div>
    </main>
  );
