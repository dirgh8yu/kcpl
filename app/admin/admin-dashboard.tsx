      ...
      <div className="ops-split">
        <aside className="ops-split-list flex max-h-[48vh] min-h-0 flex-col lg:max-h-[calc(100vh-58px)]">
          <div className="border-b border-[#e2ddd8] bg-white/70 p-4">
            <div className="flex items-end justify-between gap-3"><div><p className="ops-eyebrow">Enquiry desk</p><h1 className="mt-1 text-[20px] font-[730] tracking-[-.035em] text-[#2f2a26]">Freight enquiries</h1></div><span className="text-[length:var(--app-label-size)] font-semibold text-[var(--admin-muted)]">{filtered.length} of {quotes.length}</span></div>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {filtered.length ? filtered.map((quote) => {
              ...
              return <button key={quote.reference} type="button" onClick={() => selectQuote(quote.reference)} className="ops-record-row block w-full border-b border-[#e8e3de] px-4 py-3.5 text-left" data-selected={selected || undefined}>
                ...
                <p className="mt-1.5 text-[length:var(--app-label-size)] text-[#9a928b]">Received {formatDate(quote.created_at)}</p>
              </button>;
        ...
        <section className="ops-split-detail min-h-0 overflow-y-auto">
          ...
          {loading ? <div className="grid min-h-[55vh] place-items-center" role="status"><div className="text-center"><span className="mx-auto block h-5 w-5 animate-spin rounded-full border-2 border-[#dfd8d2] border-t-[#df7159]"/><p className="mt-3 text-[11px] font-semibold text-[var(--admin-muted)]">Loading enquiry…</p></div></div> : null}
              ...
              {activeTab === "overview" ? <div className="ops-grid-main">
                <div className="ops-stack">
                  <OpsSurface eyebrow="Request" title="Cargo & route" description="The customer's original freight requirement.">
                    ...
                    {detail.requirements ? <div className="mt-5 border-t border-[#e8e3de] pt-4"><p className="text-[length:var(--app-label-size)] font-semibold text-[var(--admin-muted)]">Customer requirements</p><p className="mt-2 whitespace-pre-wrap text-[12px] leading-6 text-[#625b55]">{detail.requirements}</p></div> : null}
                  </OpsSurface>
                ...
                <aside className="ops-stack xl:sticky xl:top-[132px]">
                  ...
                  {canViewCommercial ? <OpsSurface eyebrow="Commercial" title="Quote snapshot"><div className="grid grid-cols-2 gap-px overflow-hidden rounded-[var(--app-radius)] border border-[#e5e0db] bg-[#e5e0db]"><Snapshot label="Customer price" value={detail.quoted_amount ? formatMoney(detail.quoted_amount, detail.quote_currency) : "Not quoted"}/><Snapshot label="Margin" value={metrics ? `${metrics.margin.toFixed(1)}%` : "—"}/></div><OpsButton variant="ghost" size="sm" className="mt-3" onClick={() => setActiveTab("pricing")}>Open pricing <ArrowRight size={11}/></OpsButton></OpsSurface> : null}
...
function CustomerControl({ detail, saving, manualCustomerId, onManualCustomerId, onLink, onCreate }: { detail: QuoteDetail; saving: boolean; manualCustomerId: string; onManualCustomerId: (value: string) => void; onLink: (customerId: string) => void; onCreate: () => void }) {
  ...
  return <OpsSurface priority="warning" eyebrow="CRM customer" title="Confirm customer before marking Won" description="Confirm a suggested account, enter a known KCPL customer reference, or create a new prospect only when no duplicate exists.">
    {detail.crm_matches.length ? <div className="grid gap-2">{detail.crm_matches.slice(0, 4).map((match) => <button type="button" disabled={saving} key={match.id} onClick={() => onLink(match.id)} className="flex items-center justify-between gap-3 rounded-[var(--app-radius)] border border-[#e2d9d2] bg-white p-3 text-left transition hover:border-[#d9c2b7] hover:bg-[#fff8f4]"><div className="min-w-0"><p className="truncate text-[11px] font-bold text-[var(--admin-ink)]">{match.display_name}</p><p className="mt-1 text-[length:var(--app-label-size)] text-[var(--admin-muted)]">{match.reason || "Existing CRM details match"} · <OpsMono>{match.id}</OpsMono></p></div><span className="shrink-0 text-[length:var(--app-label-size)] font-bold text-[#b36a55]">Confirm</span></button>)}</div> : <div className="border-l-2 border-[#d7b479] bg-[#fffaf4] px-3 py-2.5 text-[11px] leading-5 text-[#756555]">No existing CRM match was found for this enquiry.</div>}
...
function PricingMetric({ label, value, tone = "neutral" }: { label: string; value: string; tone?: "neutral" | "success" | "warning" | "danger" }) {
  return <div className="bg-white p-4"><p className="text-[length:var(--app-label-size)] font-semibold text-[var(--admin-muted)]">{label}</p><strong className={`mt-1.5 block text-[18px] tracking-[-.035em] ${tone === "success" ? "text-[#47795a]" : tone === "warning" ? "text-[#9b682b]" : tone === "danger" ? "text-[#ae434a]" : "text-[var(--admin-ink)]"}`}>{value}</strong></div>;
}