...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const guide = guides.zh[slug as GuideSlug];
  return createPageMetadata({ title: guide.title, description: guide.description, path: `/zh/guides/${slug}` });
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  if (!guideSlugs.includes(slug as GuideSlug)) notFound();
  return <FreightGuidePage locale="zh" slug={slug as GuideSlug}/>;
}
