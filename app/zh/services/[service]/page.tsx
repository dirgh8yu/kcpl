...
export async function generateMetadata({ params }: { params: Promise<{ service: string }> }) {
  ...
  return createPageMetadata({
    title: siteText("zh", `svc.${found.key}.title` as SiteTextKey),
    description: siteText("zh", `svc.${found.key}.summary` as SiteTextKey),
    path: `/zh/services/${found.slug}`,
  });
}
...
export default async function Page({ params }: { params: Promise<{ service: string }> }) {
  ...
  return <ServiceDetailPage locale="zh" slug={service as ServiceSlug}/>;
}
