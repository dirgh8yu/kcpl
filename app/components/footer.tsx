  ...
  return (
    <footer className="bg-ink text-white">
      <Container className="grid gap-12 py-16 md:grid-cols-2 lg:grid-cols-[1.35fr_0.75fr_0.75fr_1fr] lg:py-20">
        ...
        <div><h3 className="footer-title">Company</h3><div className="footer-links"><Link href="/about">About</Link><Link href="/services">Services</Link><Link href="/network">Network</Link><Link href="/tracking">Tracking</Link><Link href="/quote">Request Quote</Link><Link href="/contact">Contact</Link><Link href="/privacy">Privacy Policy</Link></div></div>
      ...
      <Container className="flex flex-col gap-4 border-t border-white/10 py-6 text-[0.63rem] uppercase tracking-[0.14em] text-white/35 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-5"><Link href="/privacy" className="transition-colors hover:text-white">Privacy</Link><Link href="/quote" className="inline-flex items-center gap-2 text-gold" data-analytics-event="footer_quote_click">Request a quote <ArrowUpRight size={13}/></Link></div>
      </Container>
    </footer>
  );
}
