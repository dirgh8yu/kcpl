...
import { AlertTriangle, ArrowRight, CheckCircle2, Package, Truck } from "lucide-react";
import {
  OpsBadge,
  OpsButton,
  OpsEmptyState,
...
export function PortalShipmentsWorkspace({ shipments }: { shipments: PortalShipmentView[] }) {
  ...
  const counts = useMemo(() => ({
    ...
    attention: shipments.filter((shipment) => shipment.status === "exception").length,
    ...
    <OpsPage>
      ...
      <div className="ops-content">
        <div className="ops-stack portal-stack">
          <PortalKpiStrip
            items={[
              { key: "active", icon: Package, label: "Active shipments", value: counts.active, tone: "accent" },
              { key: "in-transit", icon: Truck, label: "In transit", value: counts.inTransit, tone: "info" },
              {
                key: "attention",
                icon: AlertTriangle,
                ...
                detail: counts.attention > 0 ? "KCPL is working on these" : "No exceptions raised",
              },
              {
                key: "delivered",
                icon: CheckCircle2,
                ...
                tone: "success",
                detail: "Completed movements on your account",
              },
            ]}
          />
