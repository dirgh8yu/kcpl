...
import type { PortalCapabilities, PortalDenialReason, PortalIdentity, PortalRole } from "./portal-access-policy";
...
export async function authorizePortalIdentity(identity: PortalIdentity): Promise<
  | { kind: "authorized"; session: PortalSession }
  | { kind: "denied"; reason: PortalDenialReason }
  | { kind: "unavailable" }
> {
  const decision = await resolvePortalAccount(identity);
