  ...
  try {
    ...
    const allRows = billDocs.map((doc) => {
      ...
      return {
        ...
        invoiceTax: numberOrNull(bill.tax_total),
        invoiceTotal: numberOrNull(bill.total),
        auditStatus: status,
        paymentAllowed: freightAuditPaymentAllowed(status) && !stale,
        auditStale: stale,
        bookedPartnerName: nullable(audit?.booked_partner_name),
        bookedCurrency: nullable(audit?.booked_currency),
        bookedCost: numberOrNull(audit?.booked_cost),
        expectedLinehaul: numberOrNull(audit?.expected_linehaul),
        expectedFuelSurcharge: numberOrNull(audit?.expected_fuel_surcharge),
        expectedAccessorials: numberOrNull(audit?.expected_accessorials),
        expectedRateUnit: nullable(audit?.expected_rate_unit),
        expectedQuantity: numberOrNull(audit?.expected_quantity),
        minimumApplied: typeof audit?.minimum_applied === "boolean" ? audit.minimum_applied : null,
