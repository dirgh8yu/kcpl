...
/** Marks a task done or not done, through the same guarded function the web
 * Job File uses. */
export async function POST(request: Request, context: { params: Promise<{ reference: string; id: string }> }) {
  return withStaffSession(request, async ({ user, staff }) => {
    ...
    const result = await toggleJobChild(reference, "job_tasks", id.slice(0, 180), completed, { name: user.displayName, email: user.email }, staff);
