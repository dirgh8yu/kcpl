...
import {
  importPayablesCsv,
  payablesCsvTemplate,
  payablesImportLimits,
  preparePayablesImport,
} from "../../../../admin/migration/payables-import.server";
...
export async function GET() {
  ...
  return new Response(payablesCsvTemplate(), {
    status: 200,
    headers: {
      "content-type": "text/csv; charset=utf-8",
      "content-disposition": 'attachment; filename="kcpl-stage-3b-payables-template.csv"',
...
export async function POST(request: Request) {
  ...
  if (!(file instanceof File)) return json({ ok: false, error: "Choose a payables CSV file." }, 400);
  if (action !== "preview" && action !== "import") return json({ ok: false, error: "Choose a valid migration action." }, 400);

  const limits = payablesImportLimits();
  if (!file.name.toLowerCase().endsWith(".csv")) return json({ ok: false, error: "Stage 3B accepts CSV files only." }, 415);
  if (file.size <= 0) return json({ ok: false, error: "The selected CSV is empty." }, 400);
  if (file.size > limits.maxFileBytes) return json({ ok: false, error: "Stage 3B payables CSV files must be 2 MB or smaller." }, 413);
  ...
  try {
    ...
    console.error("KCPL Stage 3B payables migration failed", error);
    return json({ ok: false, error: error instanceof Error ? error.message : "Payables migration failed." }, 500);
  }
}
