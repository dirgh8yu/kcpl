...
import { customerCsvTemplate, customerImportLimits, importCustomerCsv, prepareCustomerImport } from "../../../../admin/migration/customer-import.server";
...
export async function GET() {
  ...
  return new Response(customerCsvTemplate(), {
    status: 200,
    headers: {
      "content-type": "text/csv; charset=utf-8",
      "content-disposition": 'attachment; filename="kcpl-customer-import-template.csv"',
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return json({ ok: false, error: "The migration upload could not be read." }, 400);
  }

  const file = form.get("file");
  const action = String(form.get("action") ?? "preview").trim();
  if (!(file instanceof File)) return json({ ok: false, error: "Choose a customer CSV file." }, 400);
  if (action !== "preview" && action !== "import") return json({ ok: false, error: "Choose a valid migration action." }, 400);

  const limits = customerImportLimits();
  if (!file.name.toLowerCase().endsWith(".csv")) return json({ ok: false, error: "Stage 1 accepts CSV files only." }, 415);
  if (file.size <= 0) return json({ ok: false, error: "The selected CSV is empty." }, 400);
  if (file.size > limits.maxFileBytes) return json({ ok: false, error: "Stage 1 customer CSV files must be 2 MB or smaller." }, 413);

  let csv: string;
  try {
    csv = await file.text();
  } catch {
    return json({ ok: false, error: "The CSV could not be decoded as text." }, 400);
  }
  ...
  try {
    ...
    console.error("KCPL customer migration failed", error);
    return json({ ok: false, error: error instanceof Error ? error.message : "Customer migration failed." }, 500);
  }
}
