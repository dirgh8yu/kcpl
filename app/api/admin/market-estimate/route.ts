...
let providerHealthCache: { expiresAt: number; healthy: boolean } | null = null;
...
function providerErrorText(value: unknown): string {
  ...
  if (value && typeof value === "object") return Object.values(value as Record<string, unknown>).map(providerErrorText).filter(Boolean).join(" ");
  return "";
}
function friendlyProviderError(raw: string) {
  if (/invalid url parameters/i.test(raw)) return "The external provider could not understand these shipment details. Select a suggested location or use an IATA / UN location code, then confirm the cargo measurements.";
  if (/location|origin|destination|address|geocod/i.test(raw)) return "The external provider could not resolve this route. Select a suggested location, a three-letter IATA airport code, or a five-letter UN seaport code.";
  if (/dimension|width|length|height|volume|load/i.test(raw)) return "The external provider needs complete cargo dimensions for this load type. Enter length, width and height for each box, crate or pallet.";
...
function rateRows(response: Record<string, unknown>) {
  ...
  const rows = Array.isArray(rawMode)
...
async function fetchFreightos(params: URLSearchParams): Promise<ProviderAttempt> {
  ...
  try {
    ...
    return { ok: false, status: 0, response: {}, payload: {}, error: "The external freight estimate provider could not be reached." };
  }
  let payload: Record<string, unknown>;
  try {
    payload = await upstream.json() as Record<string, unknown>;
  } catch {
    return { ok: false, status: upstream.status, response: {}, payload: {}, error: "The external freight estimate provider returned an unreadable response." };
  }
  const response = payload.response && typeof payload.response === "object" ? payload.response as Record<string, unknown> : {};
  const raw = providerErrorText(response.errors || payload.errors);
  return { ok: upstream.ok, status: upstream.status, response, payload, error: friendlyProviderError(raw) };
}
async function providerHealthy() {
  if (providerHealthCache && providerHealthCache.expiresAt > Date.now()) return providerHealthCache.healthy;
  ...
  const attempt = await fetchFreightos(test);
  const healthy = Boolean(rateRows(attempt.response).rows.length);
  providerHealthCache = { expiresAt: Date.now() + CACHE_TTL_MS, healthy };
  return healthy;
}
...
export async function POST(request: Request) {
  ...
  if (!isContainer && (!width || !length || !height)) return json({ ok: false, error: "Enter length, width and height for each box, crate or pallet." }, 400);
  ...
  if (weight) base.set("weight", `${weight}${weightUnit(body.weightUnit)}`);
  const dimUnit = dimensionUnit(body.dimensionUnit);
  if (width) base.set("width", `${width}${dimUnit}`);
  if (length) base.set("length", `${length}${dimUnit}`);
  if (height) base.set("height", `${height}${dimUnit}`);

  const cacheKey = `${mode}|${base.toString()}`;
  pruneCache();
  const cached = estimateCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return json(cached.value);

  // Ask broadly first. Freightos' own public examples do not require a mode filter,
  // and broad requests can expose coverage that an exact-mode request hides.
  let attempt = await fetchFreightos(base);
  let { rates, rows } = rateRows(attempt.response);

  // Fallback to Freightos' documented public-calculator shape if the fast estimator
  // returns no coverage. This removes estimate/currency while keeping the same shipment.
  if (!rows.length && attempt.ok) {
    const fallback = new URLSearchParams(base);
    fallback.delete("estimate");
    fallback.delete("currency");
    attempt = await fetchFreightos(fallback);
    ({ rates, rows } = rateRows(attempt.response));
  }

  if (!attempt.ok && !rows.length) {
    return json({ ok: false, code: "provider_error", error: attempt.error }, 502);
  }

  const requested = rows.find((row) => text(row.mode, 12).toLowerCase() === mode.toLowerCase());
  if (!requested) {
    const availableModes = rows.map((row) => text(row.mode, 12)).filter(Boolean);
    if (availableModes.length) {
      return json({
        ok: false,
        code: "mode_unavailable",
        available_modes: availableModes,
        error: `${mode.toUpperCase()} is not available from Freightos for this shipment. Available external modes: ${availableModes.join(", ")}.`,
      }, 404);
    }
    const healthy = await providerHealthy();
    return json({
      ok: false,
      code: healthy ? "coverage_unavailable" : "provider_unavailable",
      error: healthy
        ? "Freightos is responding, but its public marketplace has no estimate coverage for this specific route and load. KCPL can still price it from partner/vendor rates."
        : "Freightos' public estimator did not return its own documented test rate, so the external provider is currently unavailable or has changed its public API.",
    }, healthy ? 404 : 502);
  }

  const price = requested.price && typeof requested.price === "object" ? requested.price as Record<string, unknown> : {};
  ...
  if (!min || !max || min.currency !== max.currency) return json({ ok: false, code: "provider_error", error: "The provider returned an incomplete price range." }, 502);

  const transit = requested.transitTimes && typeof requested.transitTimes === "object" ? requested.transitTimes as Record<string, unknown> : {};
  ...
  const value = {
    ...
    estimate: {
      ...
      mode: text(requested.mode, 12) || mode,
      min: min.amount,
      max: max.amount,
      midpoint,
