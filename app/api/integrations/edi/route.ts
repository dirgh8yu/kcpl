...
import { acknowledgeOutboundEdi, ingestEdiPayload, listOutboundEdiQueue } from "../../../admin/edi/edi-gateway.server";
...
export async function POST(request: Request) {
  ...
  if (contentType.includes("application/json")) {
    ...
    return inbound(payload, partner, providerEventId);
  }
...
async function inbound(payload: string, partner: string, providerEventId: string) {
  const result = await ingestEdiPayload(payload, partner, providerEventId || null);
