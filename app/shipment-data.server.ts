...
async function ensureShipmentSchema(db: D1Database) {
  if (!shipmentSchemaReady) {
    shipmentSchemaReady = (async () => {
      const existing = await db.prepare(`
        SELECT name
        FROM sqlite_master
        WHERE type = 'table'
          AND name IN ('shipments', 'shipment_events')
      `).all<{ name: string }>();
      const names = new Set((existing.results ?? []).map((row) => row.name));

      if (!names.has("shipments")) {
        await db.prepare(`
      ...
      await db.prepare("CREATE INDEX IF NOT EXISTS idx_shipments_quote_reference ON shipments(quote_reference)").run();
      await db.prepare("CREATE INDEX IF NOT EXISTS idx_shipments_status ON shipments(status)").run();
      await db.prepare("CREATE INDEX IF NOT EXISTS idx_shipment_events_reference_time ON shipment_events(shipment_reference, event_time DESC, id DESC)").run();
    })().catch((error) => {
